...
*/
    ...
    public static function saveParameters($config)
    {
        ...
        if (!empty($multiple_conf_keys)) {
            foreach ($multiple_conf_keys as $key) {
                ...
                self::setConfig($key, json_encode($liste));
            }
        }
        ...
        if (!empty($array_conf_keys)) {
            foreach ($array_conf_keys as $key) {
                ...
                self::setConfig($key, json_encode($value));
            }
        }
	...
	echo '<pre>';
#	var_dump(   'SELECT eo.id_order AS id_order, o.payment AS payment
...
#	);
	...
	var_dump($list_orders);
		...
		$details = $order->getOrderDetailList();
            foreach ($details as $detail) {
                if(substr($detail['product_reference'], 0, 3) == "AMB"){
	...
	foreach ($list_orders as $order) {
		if($isContremarque)
			list($flag, $msg) = $this->sendOrder($order['id_order'], $output_dir . $output_name, null);
		else
			list($flag, $msg) = $this->sendOrder($order['id_order'], $output_dir . $output_name, null); // $order['sellermania_ref']
            //else
              //  list($flag, $msg) = [true, ''];
	...
	// send file by email
        ...
        if (!$ok_orders) {
            ...
            return json_encode(array('mc' => 31, 'suxs' => 'nosuccess'));
            ...
            return json_encode(array('mc' => 30, 'suxs' => 'success'));
        }
    }
     ...
     * add order to file
     */
    public function sendOrder($id_order, $filename, $refMarketplace)
    {
...
*/
	...
	$id_command = ($refMarketplace!=null)?$refMarketplace:$id_order;
        $lines = array();
        $tapis_sur_mesure = 0;
        // headline
        $line1 = 'A                  ????????';
        ...
        if($order->payment == "CDISCOUNT.FR"){
            $line1 = self::addField($line1, "3012810000020", 190);
        } else if($order->payment == "AMAZON.FR"){
            $line1 = self::addField($line1, "3012810000030", 190);
        } else{
            $line1 = self::addField($line1, (isset($customer->pro_gln) ? $customer->pro_gln : '3012816900105'), 190); // 3012816900105 is the non-pro client code
        }
        $lines[] = $line1;
        ...
        $line2 = self::addField($line2, trim($address_default->address1).' '.trim($address_default->address2), 37, 60);
        ...
        foreach ($details as $detail) {
            ...
            //filtrer les produits dématérialisés
            /*
            ...
            */
            if($detail['product_ean13'] == '3281690249031' && $detail['product_reference'] == '249030'){ // Si le produit fait office de frais de port (specif Dumortier)
                ...
                var_dump('Produit frais de port ' . $detail['product_ean13']);
                continue;
            }
        ...
        //freight
        if (0 < $order->total_shipping_tax_excl || $shippingCosts > 0) {
            $shippingCosts = ($shippingCosts == 0)?$order->total_shipping_tax_excl:$shippingCosts; // Si les frais de port sont déjà renseignés à cause d'un produit "Frais de port" (plus haut)
            $nLine++;
            $line = 'C'.$nLine;
            $line = self::addField($line, '????????', 5);
            ...
            $unit_price_value = Tools::substr(rtrim(round(abs($shippingCosts), 2), '0'), 0, 9);
