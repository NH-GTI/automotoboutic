    ...
    { 
        ...
        if ($new_sm_status > 0) 
        {
            ...
            {                
                ...
                // Ignore status update through import process(cron/automatic)
                global $argv;
                $argument_key = '';
                if (isset($argv[0])) {
                ...
                if (Tools::getValue('k') == Configuration::get('SELLERMANIA_KEY') || $argument_key == Configuration::get('SELLERMANIA_KEY')) {
                    if ($new_sm_status == \Sellermania\OrderConfirmClient::STATUS_DISPATCHED || $new_sm_status == \Sellermania\OrderConfirmClient::STATUS_AVAILABLE_IN_STORE) {
                        ...
                        Configuration::updateValue('SM_SLEEPING_ORDERS_UPDATES', json_encode($order_items));
                    } 
                    return '';
                }  
                ...
                if (!empty($order_items)) {
                    ...
                    if (!empty($result['OrderItemConfirmationStatus'])) {
