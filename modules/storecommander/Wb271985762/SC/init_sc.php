...
define('SC_CSSSTYLE', 'lib/js/material_custom_018.css');
define('SC_CSS_FONTAWESOME', 'lib/css/fontawesome/all.css');
define('SC_JQUERY', 'lib/js/jquery-3.7.1.min.js');
define('SC_JQUERY_UI', 'lib/js/jquery-ui/jquery-ui.min.js');
define('SC_JSFUNCTIONS', 'lib/js/functions_055.js');
define('SC_JSDHTMLX', 'lib/js/dhtmlx_011.js');
define('SC_JSDHTMLX_CUSTOM','lib/js/dhtmlx_custom_08.js');
define('SC_JS_COOKIE','lib/js/js-cookie.min.js');
...
ob_end_clean();
require_once SC_DIR.'lib/php/ScCleanup.php';
(new ScCleanup())->removeOldFiles();
...
require_once SC_DIR.'lib/php/service/autoload.php';
...
require_once SC_DIR.'lib/php/ScUsage.php';
...
{
    $utfFile = SC_DIR.'lib/php/utf16.php';
    if(function_exists('opcache_invalidate') && ini_get('opcache.restrict_api') === ""){
        opcache_invalidate($utfFile);
    }
    require_once $utfFile;
}

define('SC_EXTERNAL_URL',Tools::getShopDomainSsl(true).__PS_BASE_URI__.'modules/'.SC_MODULE_FOLDER_NAME.'/');
define('SC_ORK_EXTERNAL_URL', SC_EXTERNAL_URL.'ork/');
...
{
    require_once SC_DIR.'../../'.SC_MODULE_FOLDER_NAME.'.php';
    if (IS_RBM)
    {
    ...
    else
    {
        ...
        if (empty($actual_version) || $actual_version != $sc_module->version)
        {
            ...
            if (file_exists(SC_DIR.'tmp'))
            {
                ...
                {
                    $good = Tools::ZipExtract(SC_DIR.'tmp/'.$sc_zip_filename, _PS_MODULE_DIR_);
                    if ($good)
                    {
...
{
    require_once _PS_MODULE_DIR_.'creativeelements/classes/wrappers/UId.php'; // needed to generate uid param for CreativeElements module CE\UId()
    require_once SC_DIR.'lib/php/extension/ScCreativeElements.php'; // helper for creating links
}
