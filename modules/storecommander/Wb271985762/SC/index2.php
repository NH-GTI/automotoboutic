...
if ((defined('SC_UkooParts_ACTIVE') && SC_UkooParts_ACTIVE == 1) && SCI::moduleIsInstalled('ukooparts')) {
    require_once 'lib/php/service/UkooParts/UkooPartsService.php';
}
...
if ($ajax)
{
    ...
    if ($action)
    {
        ...
        if (strpos($action, '_update') !== false && isset($_POST['ids']) && !is_array($_POST['ids']))
        {
...
{
    $page = SC_Agent::getInstance()->getPageByPermissions(Tools::getValue('page', 'cat_tree'));
    if(!$page)
    {
        echo _l('No interface allowed for this employee. Please contact an administrator.');
        exit;
    }
...
<head>
    ...
    <script src="<?php echo SC_JS_COOKIE; ?>"></script>
    <script src="https://clippingmagic.com/api/v1/ClippingMagic.js"></script>
    <script>
        ClippingMagic.initialize({apiId: <?php echo CutOut::getApiId(); ?>});
    </script>
    <script src="<?php echo SC_JSDHTMLX_CUSTOM; ?>"></script>
</head>
<body>
<script>
    const defaultSuccessClipBoardMessage = '<?php echo _l('Data successfully copied to clipboard', 1); ?>';
    const errorClipBoardMessage = '<?php echo _l('Error while copying to clipboard', 1); ?>';
    const defaultCookieOptions = {
        expires: 30,
        path: '<?php echo constant('COOKIE_PATH'); ?>',
        SameSite : 'Lax'
    };
    ...
    */
    ...
    function modalMessage(n, value)
    {
        const html = [];
        if(!Boolean(value.class)) {
            console.error('class missing')
        }

        html.push(`<div class="message ${value.class}">`)

        if(!Boolean(value.title)) {
            console.error('title missing')
        }
        html.push(`<h1>${value.title}</h1>`)

        if(Boolean(value.content)) {
            html.push(`<div class="paragraph">${value.content}</div>`)
        }
    ...
    function modalInputNumberMinMax(n, value)
    {
        if(!Boolean(value.formObjectJs)) {
            console.log('please set form object identifier')
        }
        ...
        if(value.hasOwnProperty('min')) {
            attributes.push(`min="${value.min}"`)
        }
        if(value.hasOwnProperty('max')) {
            attributes.push(`max="${value.max}"`)
        }

        attributes = attributes.join(' ')
        return `<input type="number" class="dhxform_textarea" ${attributes} onchange="${value.formObjectJs}.callEvent('onChange', ['${value.name}', this.value]);"/>`
    }
    ...
    const lang_translation = {
        alau: {
            inactive_customer: '<?php echo _l('Inactive customer'); ?>',
            guest_customer: '<?php echo _l('This customer is a guest'); ?>',
        }
    }
    <?php
    ...
    function checkIdShopBetweenBrowserTabs()
    {
        ...
        var cookie_selected_shop = Cookies.get('sc_shop_selected');
...
elseif ((defined('OPEN_UPDATE_WINDOW') && OPEN_UPDATE_WINDOW == '1') && _r('INT_HELP_SC_UPDATE')) {
?>
<script>
    dhxMenu.callEvent('onClick', ['forceNewVersion']);
</script>
<?php
}
