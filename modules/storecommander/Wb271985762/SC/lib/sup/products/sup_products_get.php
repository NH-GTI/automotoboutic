...
if (!defined('STORE_COMMANDER'))
{
    ...
    $supplier_ids = Tools::getValue('supplier_list');
    ...
    if ($without_sup)
    {
        if ($nb_shop_entry > 1)
        {
            $sql = 'SELECT p.id_product,p.id_category_default,p.id_supplier,p.reference,p.active,"" as product_supplier_reference, pl.name
                    ...
                    LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (pl.id_product = p.id_product AND pl.id_shop = p.id_shop_default AND pl.id_lang='.(int) $id_lang.')
        ...
        else
        {
            $sql = 'SELECT p.id_product,p.id_category_default,p.id_supplier,p.reference,p.active,"" as product_supplier_reference,pl.name
                    FROM '._DB_PREFIX_.'product p
                    RIGHT JOIN '._DB_PREFIX_.'product_shop ps ON (ps.id_product = p.id_product AND ps.id_shop='.(int) $id_shop.'
                    LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (pl.id_product = p.id_product AND pl.id_shop = '.(int) $id_shop.' AND pl.id_lang='.(int) $id_lang.')
                    WHERE p.id_supplier = 0 OR p.id_product NOT IN (SELECT id_product FROM '._DB_PREFIX_.'product_supplier)';
        }
    ...
    else
    {
        foreach ($supplier_ids as $id_supplier)
        {
            ...
            else
            {
                $sql = 'SELECT p.id_product,p.id_category_default,p.id_supplier,p.reference,ps.active,psup.product_supplier_reference,pl.name,s.name as supplier_name
                        FROM '._DB_PREFIX_.'product p
                        RIGHT JOIN '._DB_PREFIX_.'product_shop ps ON (ps.id_product = p.id_product)
                        LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (pl.id_product = p.id_product AND pl.id_shop = ps.id_shop AND pl.id_lang='.(int) $id_lang.')
                        LEFT JOIN '._DB_PREFIX_.'supplier s ON s.id_supplier = '.(int) $id_supplier.'
                        LEFT JOIN '._DB_PREFIX_.'product_supplier psup ON psup.id_supplier = '.(int) $id_supplier.' AND psup.id_product = p.id_product
    ...
    function generateValue($products_by_sup)
    {
        ...
        foreach ($products_by_sup as $id_supplier => $product_list)
        {
            foreach ($product_list as $product)
            {
                ...
                $cols = ['id_product', 'reference', 'reference_supplier', 'id_supplier', 'product_name', 'active'];
                foreach ($cols as $col)
                {
                    switch ($col){
                        case 'id_product':
