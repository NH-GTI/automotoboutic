...
if (_r('GRI_SUP_PROPERTIES_GRID_IMG')) { ?>
        ...
        case 'image_add':
            ...
            if (suppliers_ids) {
                if (!dhxWins.isWindow("wProductImages" + suppliers_ids)) {
                    ...
                    ll_toolbar.setItemState('auto_upload', (Cookies.get('sc_sup_img_auto_upload') == 1 ? 1 : 0));
                    ...
                    ll_toolbar.attachEvent("onStateChange", function (id, state) {
                        if (id == 'auto_upload') {
                            ...
                            Cookies.set('sc_sup_img_auto_upload', auto_upload, defaultCookieOptions);
                        }
                    });

                    ll.cells('a').attachURL("index.php?ajax=1&act=sup_image_upload&id_lang=" + SC_ID_LANG, null, {supplier_list:suppliers_ids});
            ...
            if (prop_tb._imagesGrid.getSelectedRowId() == null) {
                ...
                if (last_supplierID != 0) {
                    if (confirm('<?php echo _l('Are you sure you want to delete the selected items?', 1); ?>')) {
                        $.post('index.php?ajax=1&act=sup_image_update&action=delete',
                        {
                            id_lang: SC_ID_LANG,
                            supplier_list: castListId(last_supplierID),
                            list_id_image: castListId(prop_tb._imagesGrid.getSelectedRowId())
                        }, function () {
                            displayImages();
                            displaySuppliers();
                        });
...
function displayImages(callback) {
    ...
    {
        supplier_list: castListId(sup_grid.getSelectedRowId()),
