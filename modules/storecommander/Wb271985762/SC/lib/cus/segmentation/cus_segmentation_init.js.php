...
if (SCSG)
{
    ...
    function setPropertiesPanel_segments(id){
            ...
            prop_tb._segmentsGrid.attachEvent("onEditCell",function(stage,rId,cInd,nValue,oValue)
            {
                    ...
                    if (cInd == idxUsed){
                        if(stage==1)
                        {
                            ...
                            $.post('index.php?ajax=1&act=cus_segmentation_update&action=present',
                            {
                                id_lang: SC_ID_LANG,
                                value: value,
                                id_segment: Number(rId),
                                ids: castListId(getListIdFromUserData(cus_grid, 'id_customer'))
                            },function(){
                                displaySegments()
                            })
                        }
        ...
        if (id=='segment_multi_add')
        {
            ...
            {
                let segments = prop_tb._segmentsGrid.getSelectedRowId();
                $.post('index.php?ajax=1&act=cus_segmentation_update&action=mass_present',
                {
                    id_lang: SC_ID_LANG,
                    value: 1,
                    segments: castListId(segments),
                    ids: castListId(getListIdFromUserData(cus_grid, 'id_customer'))
                },function(data){
                    displaySegments()
                })
    ...
    function displaySegments(force_refresh, callback)
    {
        ...
        {
            $.post('index.php?ajax=1&act=cus_segmentation_relation_get',
            {
        ...
        else if(force_refresh==true)
        {
            ...
            $.post('index.php?ajax=1&act=cus_segmentation_get',
            {
                id_lang: SC_ID_LANG
            },function(response)
            {
                prop_tb._segmentsGrid.parse(response)
