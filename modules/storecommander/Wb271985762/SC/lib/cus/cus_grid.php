...
<script>
    ...
    getTbSettingsButton(cus_grid_tb, {'grideditor':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':<?php echo (int)_r('MEN_TOO_SETTINGS'); ?>}, '', 100);
    ...
    function gridToolBarOnClick(id){
            ...
            if (id=='user_go'){
                let sel=getListIdFromUserData(cus_grid, 'id_customer');
                if (sel)
                {
                    let tabId=sel.split(',');
                    if (tabId.length === 1){
...
{ ?>
            ...
            if (id=='send_mail')
            {
                ...
                const params = {
                    ...
                    id_lang: SC_ID_LANG,
                    selectedIds: String(getListIdFromUserData(cus_grid, 'id_customer'))
                };
                    ...
                    dhtmlx.message({
                        ...
                        callback: function (res) {
                            ...
                            dhtmlx.message({
                                ...
                                callback: function (res) {
                                    full_delete = (res ? 1 : 0);
                                    let customer_ids = getListIdFromUserData(cus_grid, 'id_customer');
...
?>
        ...
        if(stage==1 && coltype === 'coro') // only for ed type
        {
    ...
    function onGridCusContextButtonClick(itemId){
        ...
        if(itemId==="delete_from_manual_segment"){
            if (confirm('<?php echo _l('Do you really want to remove data from current segment?', 1); ?>')){
                $.post("index.php?ajax=1&act=all_win-segmentation_element_update",{action:'delete_from_id_element',ids:getListIdFromUserData(cus_grid, 'id_customer'), segmentId: id_selected_segment},function(){
                    displayCustomers();
                });
            }
        }
