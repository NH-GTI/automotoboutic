...
echo '<script>'; ?>
    ...
    wImportCus.tbOptions.addInput("filter_name", 0,"",100);
    wImportCus.tbOptions.addText('txt_filter_name',0,'<?php echo _l('Filter by name'); ?>');
    wImportCus.tbOptions.setItemToolTip('filter_name','<?php echo _l('Filter by name'); ?>');
    ...
    wImportCus.tbOptions.attachEvent("onClick",
        function(id){
            ...
            if (id=='download')
            {
                window.open(wImportCus.gridFiles.getUserData(wImportCus.gridFiles.getSelectedRowId(),'fileUrl'));
            }
            if (id=='delete')
            {
...
//                    wImportCus._uploadWindow = dhxWins.createWindow("wImportCusUpload", 50, 50, 396, 568);
    ...
    {
        let inputFilterName = wImportCus.tbOptions.getInput('filter_name');
        inputFilterName.onkeyup=function(){
            let search = $(this).val();
            wImportCus.gridFiles.filterBy(1,search);
        };
    }
    ...
    wImportCus.gridFilesDataProcessor.attachEvent("onBeforeDataSending", function(id, state, data){
...
<?php
    ...
    wImportCus.gridFilesDataProcessor.attachEvent("onBeforeUpdate",function(id,status){
            if (wImportCus.gridFiles.cells(id,wImportCus.gridFiles.getColIndexById('fieldsep')).getValue() === wImportCus.gridFiles.cells(id,wImportCus.gridFiles.getColIndexById('valuesep')).getValue())
            {
...
function displayProcessCus()
{
    ...
    let jqhxhr_process = $.post('index.php?ajax=1&act=cus_win-import_process&action=mapping_process',{'mapping':mappingCus,'id_lang_sc':SC_ID_LANG,'filename':lastCSVFileCus,'importlimit':wImportCus.tbProcess.getValue('importlimit'),'create_categories':(1*create_categories)},function(data){
        ...
        dhxlImportCus.cells('c').attachHTMLString(data.responseText);
    });
