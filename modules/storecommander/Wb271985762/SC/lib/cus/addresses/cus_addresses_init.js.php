...
?>
    ...
    function initCustomerAddress(){
        if (needinitCustomerAddress)
        {
            ...
            customerAddressDataProcessor.attachEvent("onAfterUpdate", function(id,action,tid,response){
                if(!!response.error) {
                    dhtmlx.message({text: response.error, type: 'error', expire: 10000});
                }
            });
    ...
    function displayCustomerAddresses()
    {
        ...
        $.post("index.php?ajax=1&act=cus_addresses_get&id_lang="+SC_ID_LANG,
        {
