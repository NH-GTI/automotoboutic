...
function loadGridUISettings(gridObj, orderColumns = true, hideColumns = true, sizeColumns = true, sortColumns = true)
{
...
//console.log("LOAD: "+ui_settings[grid_name]);
        ...
        if (uidata.length==4)
        {
            ...
            // ORDER
            if (orderColumns && orderData !== undefined && orderData !== null && orderData !== '') {
            ...
            // HIDDEN
            if (hideColumns && hiddenData !== undefined && hiddenData !== null && hiddenData !== '') {
            ...
            // SIZE
            if (sizeColumns && sizeData !== undefined && sizeData !== null && sizeData !== '') {
            ...
            // SORT
            if (sortColumns && sortData !== undefined && sortData !== null && sortData !== '') {
...
function openFixMyPsWindow(id){
    ...
    if (!dhxWins.isWindow('wFixmyprestashop')) {
        ...
        $.get('index.php?ajax=1&act=all_win-fixmyprestashop_init&defaultSolution=' + id, function (data) {
            $('#jsExecute').html(data);
        });
        wFixmyprestashop.show();
        wFixmyprestashop.center()
        dhxWins.window('wFixmyprestashop').bringToTop();
...
function openSettingsWindow(tools,section,item)
{
    ...
    if (!dhxWins.isWindow('scWindowCoreSettings'))
    {
        window.scWindowCoreSettings = dhxWins.createWindow('scWindowCoreSettings', 50, 50, 1000, $(window).height() - 75);
        scWindowCoreSettings.cell.offsetParent.id = 'scWindowCoreSettings'
    }

    $.post('index.php?ajax=1&act=core_settings_init', {
        urlparams: params
    }, function (data) {
        $('#jsExecute').html(data);
    });
    scWindowCoreSettings.show();
    scWindowCoreSettings.center()
    dhxWins.window('scWindowCoreSettings').bringToTop();
}
...
/**
 * If field is not in grid, may be userData exist
 * @param grid
 * @param column
 * @returns {*|string}
 */
function getListIdFromUserData(grid, column)
{
    let idList = grid.getSelectedRowId();
    if(!Boolean(idList)) {
        return idList;
    }

    let temp = idList.split(',');
    if(!Boolean(grid.getUserData(temp[0], column))) {
        return idList;
    }

    idList = [];
    for(const rId of temp)
    {
        idList.push(grid.getUserData(rId, column));
    }
...
/**
 * cast each id of the string into Number
 * @param str
 * @param separator
 * @returns {*}
 */
function castListId(str, separator = ',') {
    if(str === null || str.length === 0) {
        return str
    }

    let cleanedData= str.split(separator).map(function(value) {
        return Number(value)
    });
