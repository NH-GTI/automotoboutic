...
if (SCSG)
{
    ...
    function setPropertiesPanel_segments(id){
            ...
            prop_tb._segmentsGrid.attachEvent("onEditCell",function(stage,rId,cInd,nValue,oValue)
            {
                    ...
                    if (cInd == idxUsed){
                        if(stage==1)
                        {
                            ...
                            $.post('index.php?ajax=1&act=ord_segmentation_update',
                                {
                                    action: 'present',
                                    value: value,
                                    id_lang: SC_ID_LANG,
                                    id_segment: Number(rId),
                                    ids: castListId(getListIdFromUserData(ord_grid, 'id_order'))
                                }, function () {
                                    displaySegments();
                                });
                        }
        ...
        if (id=='segment_multi_add')
        {
            ...
            {
                $.post('index.php?ajax=1&act=ord_segmentation_update&action=mass_present',
                {
                    value: 1,
                    id_lang : SC_ID_LANG,
                    segments: castListId(prop_tb._segmentsGrid.getSelectedRowId()),
                    ids: castListId(getListIdFromUserData(ord_grid, 'id_order'))
                },
                function(){
                    displaySegments();
    ...
    function displaySegments(force_refresh, callback)
    {
        ...
        {
            $.post("index.php?ajax=1&act=ord_segmentation_relation_get&type=order&idlist="+castListId(getListIdFromUserData(ord_grid, 'id_order'))+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(data){
                selArray=data.split(',');
