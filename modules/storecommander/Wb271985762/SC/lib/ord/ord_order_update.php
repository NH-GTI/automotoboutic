    ...
    if (isset($_POST['!nativeeditor_status']) && trim($_POST['!nativeeditor_status']) == 'updated')
    {
        ...
        foreach ($fields as $field)
        {
            ...
            if (isset($_POST[$field]) && $field == 'shipping_number')
            {
                ...
                if (Tools::getValue($field) && _s('ORD_ORDPROP_SEND_TRACKING_MAIL') && SCI::getConfigurationValue('PS_MAIL_METHOD') != 3)
                {
                    $order = new Order($id_order);
                    $customer = new Customer((int) $order->id_customer);
                    ...
                    if (Validate::isLoadedObject($customer) && Validate::isLoadedObject($carrier))
                    {
                        if (version_compare(_PS_VERSION_, '1.7.1.0', '>='))
                        {
                            ...
                            SCI::hookExec('actionAdminOrdersTrackingNumberUpdate', [
                                'order' => $order,
                                'customer' => $customer,
                                'carrier' => $carrier,
                            ]);
