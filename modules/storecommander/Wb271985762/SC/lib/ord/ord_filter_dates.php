...
?>
    ...
    dhxlOrdFilterFromTo.block_width= 300;
    dhxlOrdFilterFromTo.date_form = [
        {type: "settings", offsetLeft:0, position: "label-left", labelWidth: 30, inputWidth: 200, labelAlign: "left"},
        {type: "block",offsetLeft:0,width: dhxlOrdFilterFromTo.block_width,list:[
            {
                type: "calendar",
                name: "from",
        ...
        {type: "block",offsetTop:20,width: dhxlOrdFilterFromTo.block_width,list:[
                {
                    type: "calendar",
                    name: "entire_day",
                    dateFormat: "%Y-%m-%d",
    ...
    var date_filter_form = dhxlOrdFilterFromTo.cells('a').attachForm(dhxlOrdFilterFromTo.date_form);
    date_filter_form.format_info = '<?php echo _l('yyyy-mm-dd h:m:s'); ?>';
    date_filter_form.getInput("from").setAttribute("autocomplete","off");
    date_filter_form.getInput("from").setAttribute("placeholder",<?php echo _s('INTERFACE_CALENDAR_FORCE_DAY') ? '"'._l('yyyy-mm-dd 00:00:00').'"' : 'date_filter_form.format_info'; ?>);
    date_filter_form.getInput("to").setAttribute("autocomplete","off");
    date_filter_form.getInput("to").setAttribute("placeholder",<?php echo _s('INTERFACE_CALENDAR_FORCE_DAY') ? '"'._l('yyyy-mm-dd 23:59:59').'"' : 'date_filter_form.format_info'; ?>);
...
<?php if (!empty($invoice)) { ?>
        Cookies.set('sc_ord_inv_fromto_dates',date_from+"_"+date_to, defaultCookieOptions);
        parent.periodselection = "inv_from_to_"+date_from+"_"+date_to;
        Cookies.set('sc_ord_periodselection',parent.periodselection, defaultCookieOptions);
        parent.ord_filter.setItemText('inv_from_to','<?php echo _l('Inv from'); ?> '+date_from+" <?php echo _l('to'); ?> "+date_to);
<?php }
else
{ ?>
        Cookies.set('sc_ord_fromto_dates',date_from+"_"+date_to, defaultCookieOptions);
