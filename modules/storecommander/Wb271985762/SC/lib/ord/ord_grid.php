...
<script>
    ...
    getTbSettingsButton(ord_grid_tb, {'grideditor':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':<?php echo (int)_r('MEN_TOO_SETTINGS'); ?>}, '', 100);
...
ord_grid.getSelectedOrderId = function()
{
    const orderList = [];
    for (let i = 0; i < this.selectedRows.length; i++) {
        const idRow = this.selectedRows[i].idd;
        const idOrder = this.getUserData(idRow,'id_order');
    ...
    if (orderList.length === 0) {
        ...
        return orderList.join(this.delim);
    }
}
