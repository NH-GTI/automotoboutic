...
$only_active = (bool) Tools::getValue('only_active',0);
$currentContextShopId = Context::getContext()->shop->id;
Shop::setContext(Shop::CONTEXT_SHOP, (int) $id_shop);


function getRowsFromDB()
{
    global $id_lang,$only_active, $sc_agent,$id_shop,$filter_params,$id_customer;
...
//    exit;
    ...
    if (!empty($res))
    {
        foreach ($res as $row)
        {
            ...
            $taxrate = (isset($tax[(int) $row['id_tax']]) && $tax[(int) $row['id_tax']] ? $tax[(int) $row['id_tax']] : 0);
...
$xml = getRowsFromDB();
Shop::setContext(Shop::CONTEXT_SHOP, (int) $currentContextShopId);
?>
<rows id="0">
    <head>
        <beforeInit>
            <call command="attachHeader"><param><![CDATA[#numeric_filter,#numeric_filter,#text_filter,#text_filter,#text_filter,#numeric_filter,#numeric_filter,,]]></param></call>
        </beforeInit>
