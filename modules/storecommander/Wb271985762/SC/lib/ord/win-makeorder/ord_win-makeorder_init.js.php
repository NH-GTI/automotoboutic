...
?>
<?php echo '<script>'; ?>
    var preselected_id_customer = <?php echo $cus_id_customer; ?>;
    var preselected_id_shop_customer = <?php echo $cus_id_shop_customer; ?>;
    var cus_default_data = (preselected_id_customer !== null && preselected_id_shop_customer !== null);
    ...
    <?php if (SCMS)
{
    ...
    foreach ($shops as $shop) { ?>
        ...
        var winMakeOrderDefaultShopName = '';
        opts.forEach(function(opt_item){
            ...
            if(current_id_shop==makeOrder_shop) {
                winMakeOrderDefaultShopName = opt_item[2];
            }
        });
        makeOrder_customer_tb.addButtonSelect("makeOrder_shop", 100, winMakeOrderDefaultShopName, opts, "","",false,true);
    ...
    var makeOrder_cell_customerInfo_tabs_stats_tb = makeOrder_cell_customerInfo_tabs.tabs("notes").attachToolbar();
    ...
    makeOrder_searchProduct_tb.addButtonTwoState("only_active", 100, "", "fad fa-flag", "fad fa-flag");
    makeOrder_searchProduct_tb.setItemToolTip('only_active','<?php echo _l('If enabled: display active products only', 1);?>');

    makeOrder_searchProduct_tb.attachEvent("onStateChange", function (id, state) {
        makeOrder_searchProduct_tb.callEvent("onClick", ['makeOrder_searchProduct_refresh']);
    })
    ...
    function displayMOsearchProduct()
    {
        if(makeOrder_selected_customer!=undefined && makeOrder_selected_customer!=null && makeOrder_selected_customer!='' && makeOrder_selected_customer!=0)
        {
            ...
            $.post("index.php?ajax=1&act=ord_win-makeorder_product_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{"id_customer": makeOrder_selected_customer,"id_shop": makeOrder_shop, 'only_active': Number(makeOrder_searchProduct_tb.getItemState('only_active')), 'filter_params': makeOrder_searchProduct_filter_params},function(data)
            {
    ...
    function displayMOcrossSelling()
    {
        ...
        if(selectedIds!=undefined && selectedIds!=null && selectedIds!='' && selectedIds!=0)
        {
            ...
            makeOrder_crossSelling_grid.clearAll(true);
            $.post("index.php?ajax=1&act=ord_win-makeorder_crossselling_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{"id_shop": makeOrder_shop, "id_product_id_product_attribute":selectedIds},function(data)
            {
                ...
                if(makeOrder_selected_customer!=undefined && makeOrder_selected_customer!=null && makeOrder_selected_customer!='' && makeOrder_selected_customer!=0)
                {
                    if(makeOrder_selected_cart!=undefined && makeOrder_selected_cart!=null && makeOrder_selected_cart!='' && makeOrder_selected_cart!=0)
                    {
                        ...
                        $.post("index.php?ajax=1&act=ord_win-makeorder_cart_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),
                        {
                            ...
                            'id_product_id_product_attribute_list': selectedIds
                        },
                        function(data)
                        {
                    ...
                    $.post("index.php?ajax=1&act=ord_win-makeorder_cart_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),
                        {
                            ...
                            'quantity': nValue,
                            'action': 'update_qty',
                            'id_product_id_product_attribute': rId
                        },
                        function(error)
                    {
                        makeOrder_cell_cart.progressOff();
                    ...
                    $.post("index.php?ajax=1&act=ord_win-makeorder_cart_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),
                        {
                            ...
                            'id_shop': makeOrder_shop,
                            'action': 'reset_price',
                            'id_product_id_product_attribute_list':selection
                        },
                        function(error)
                        {
                            if (error){
    ...
    var winMakeOrdercontextMenuXML='<menu absolutePosition="auto" mode="popup" maxItems="8"  globalCss="contextMenu" globalSecondCss="contextMenu" globalTextCss="contextMenuItem">'+
    ...
    '</menu>';
    makeOrder_cart_cmenu.loadStruct(winMakeOrdercontextMenuXML);
                    ...
                    $.post("index.php?ajax=1&act=ord_win-makeorder_cart_update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),
                        {
                            ...
                            'quantity': quantity,
                            'action': 'add_product',
                            'id_product_id_product_attribute_list': ids_product
                        },
                        function(error)
                        {
                            makeOrder_cell_cart.progressOff();
