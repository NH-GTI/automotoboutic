    ...
    if (!empty($last_stats) && (!empty($last_stats['last_scan']) && $last_analysis == $today))
    {
        ...
        if (!empty($biggest_format))
        {
            ...
            if (!empty($five_best_seller))
            {
                ...
                $response = Requests::post('https://imgcompress.storecommander.net/demo.php', array('k3y' => COMPRESS_K), $data, $options);
                if ($response->success)
                {
...
function returnHtmlForComparison($data_for_html)
{
    ...
    foreach ($data_for_html as $row)
    {
        ...
        $html .= '<li>'._l('Image from product:').' '.$row['product_name'].
            ...
            '<br/><a  style="text-align:left;" href="https://imgcompress.storecommander.net/comparison.php'.$comparison_url_params.'" target="_blank">'._l('Check the difference visually').'</a>'.
            '</li>';
    }
    $html .= '</ol>';
    $html .= '<ul>
