...
if (!defined('STORE_COMMANDER'))
{
    ...
    if (!empty($CRON))
    {
        ...
        $sc_agent->setEmployee((int) Tools::getValue('id_employee'));
    }
    ...
    switch ($action){
        ...
        case 'export_process':
            ...
            $fields_lang = ['name', 'available_now', 'link_rewrite', 'meta_title', 'meta_description', 'meta_keywords', 'description_short', 'description', 'name_with_attributes'];
            // fields specific to combination, should not be read in Product object
            $combination_fields = ['name_with_attributes'];

            $not_auto_combination_fields = [];
            if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
            {
            ...
            if ($AUTO_EXPORT || ($CRON && $CRONVERSION >= 2))
            {
                ...
                else
                {
                    ...
                    if (!empty($temp_id))
                    {
                        $sc_export = ['id_sc_export' => $temp_id, 'name' => $auto_filename, 'last_export' => null, 'exporting' => 0, 'id_next' => 0, 'id_combination_next' => 0, 'total_lines' => 0];
                    }
                    else
                    {
                        if ($AUTO_EXPORT)
            ...
            if ($exportConfig['mapping'] != '' && $feed = @simplexml_load_file(SC_TOOLS_DIR.'cat_export/'.$exportConfig['mapping']))
            {
                foreach ($feed->field as $mfield)
                {
                    if ((int) $mfield->used)
                    {
                        $mapping[] = ['name' => (string) $mfield->name,
                ...
                else
                {
                    ...
                    foreach ($fieldList as $k => $mfield)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                $tmpFieldName = stripslashes($fname[$mfield]).($mapping[$k]['lang'] != '' ? ' '.strtoupper($mapping[$k]['lang']) : '');
                                if (!in_array($tmpFieldName, $names))
                                {
                                    $names[$k] = $tmpFieldName;
                                }
                                unset($tmpFieldName);
                            }
            ...
            $fullProduct = count($fieldList) != count(array_diff($fieldList, ['supplier', 'manufacturer', 'vat', 'link_to_product', 'link_to_combination', 'unity', 'tags']));
            ...
            foreach ($products as $idp)
            {
                    ...
                    || sc_in_array('id_feature_value', $fieldList, 'catWinExportProcess_fieldList'))
                {
                    ...
                    $sql->select('fl.name, fp.`id_feature`')
                        ...
                        ->leftJoin('feature_lang', 'fl', 'fp.`id_feature` = fl.`id_feature`')
                        ->where('fp.`id_product` = '.(int) $idp['id_product'])
                        ;
                    if ($selected_shops_id > 0)
                    {
                        $sql->innerJoin('feature_shop', 'fs', 'fp.`id_feature` = fs.`id_feature` AND fs.`id_shop` = '.(int) $selected_shops_id);
                    }
                    if (!empty($featuresListNameDefault))
                    {
                        $sql->where('fp.`id_feature` NOT IN ('.pInSQL(implode(',', array_unique(array_values($featuresListNameDefault)))).')');
                    }
                    $features = Db::getInstance()->executeS($sql);
                    foreach ($features as $feature)
                    {
                        $featuresListNameDefault[$feature['name']] = $feature['id_feature'];
                    }
                    ...
                    foreach ($langs as $lang)
                    {
                        ...
                        $features = Db::getInstance()->executeS($sql);
                        $featuresListByLang[$lang['id_lang']] = [];
                        foreach ($features as $feature)
                        {
                            $featuresListByLang[$lang['id_lang']][$feature['id_feature']]['id_feature_value'] = $feature['id_feature_value'];
                            if ($multipleFeatureEnabled && !empty($featuresListByLang[$lang['id_lang']][$feature['id_feature']]))
                            {
                                if (!empty($featuresListByLang[$lang['id_lang']][$feature['id_feature']]['value']))
                                {
                                    $featuresListByLang[$lang['id_lang']][$feature['id_feature']]['value'] .= $featureSseparator.$feature['value'];
                                }
                                else
                                {
                                    $featuresListByLang[$lang['id_lang']][$feature['id_feature']]['value'] = $feature['value'];
                                }
                            }
                            else
                            {
                ...
                if ($exportConfig['exportcombinations'])
                {
                    ...
                    $standardFields = ['price', 'wholesale_price', 'weight', 'reference', 'upc', 'unit_price_impact', 'supplier_reference', 'ean13', 'default_on', 'ecotax'];
                    if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
                    {
                    ...
                    if (is_array($combinaisons))
                    {
                        ...
                        foreach ($combinaisons as $k => $combinaison)
                        {
                            ...
                            $combArray[$combinaison['id_product_attribute']]['attributes'][$combinaison['id_lang']][] = [$combinaison['group_name'], $combinaison['attribute_name'], $combinaison['id_attribute'], $combinaison['id_attribute_group'], $combinaison['is_color_group'], $combinaison['color'], (version_compare(_PS_VERSION_, '1.5.0.1', '>=') ? $combinaison['group_position'] : 0)];
                            ...
                            ## supplier data cache
                            $combArray[$combinaison['id_product_attribute']]['all_supplier_data'] = [];
                            $sql = (new DbQuery())
                                ->select('`id_supplier`, `product_supplier_reference`, `product_supplier_price_te`')
                                ->from('product_supplier')
                                ->where('`id_product` = '.(int) $p->id)
                                ->where('`id_product_attribute` = '.(int) $combinaison['id_product_attribute'])
                            ;
                            ...
                            if ($data)
                            {
                                ...
                                {
                                    $combArray[$combinaison['id_product_attribute']]['all_supplier_data'][$supplierData['id_supplier']] = [
                                        'reference' => $supplierData['product_supplier_reference'],
                                        'wholesale_price' => (float) $supplierData['product_supplier_price_te'],
                                    ];
                                }
                            }
                            ...
                            if ($id_product_attribute != 0)
                            {
                                foreach ($attributes['attributes'] as $id_lang => $values)
                                {
                                    foreach ($values as $v)
                                    {
                                        ...
                                        {
                                            $combArray[$id_product_attribute]['attributeByGroup'][$id_lang] = [];
                                        }
                    ...
                    if (
                        ...
                        foreach ($mapping as $n => $f)
                        {
                                    ...
                                    switch ($f['name']) {
                                        ...
                                        case 'wholesale_price':
                                            ...
                                            if (!empty($f['options']) && !empty($suppliersListByLang[$f['options']]))
                                            {
                                                ...
                                                if (!empty($product_attribute['all_supplier_data'][$id_supplier]) && $switchObjectOptionTwo != 'product_value')
                                                {
                                                    $field = $product_attribute['all_supplier_data'][$id_supplier]['wholesale_price'];
                                                }
                                                else
                                                {
                                            ...
                                            elseif (empty($f['options']))
                                            {
                                                ...
                                                {
                                                    $field = (float) $product_attribute['wholesale_price'];
                                                }
                                                else
                                                {
                                    ...
                                    switch ($f['name']) {
                                        ...
                                        case 'price_exctax_without_reduction':
                                            if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
                                            {
                                                $specific_price = [];
                                                $field = $p->priceCalculation((int) $selected_shops_id, $p->id, ($switchObjectOptionTwo == 'product_value' ? false : (int) $id_product_attribute), (int) SCI::getDefaultCountryId() /*$id_country*/ , 0 /*$id_state*/ , 0 /*$zipcode*/ , (int) Configuration::get('PS_CURRENCY_DEFAULT') /*$id_currency*/ ,
                                                        1 /*$id_group*/ , 1 /*$quantity*/ , 0 /* use tax */ , 6 /*$decimals*/ , 0 /*$only_reduc*/ , 0 /*$use_reduc*/ , 0 /*$with_ecotax*/ , $specific_price, 0 /*$use_group_reduction*/ ,
                                                        0 /*$id_customer*/ , 0 /*$use_customer_price*/ , 0 /*$id_cart*/ , 1 /*$real_quantity*/);
                                            }
                                            else
                                            {
                                        ...
                                        case 'image_link':
                                            ...
                                            {
                                                $cacheProductImages[$p->id.'_'.$id_product_attribute] = [];
                                        ...
                                        case 'links_to_all_images':
                                            ...
                                            $res = createQueriesCache('SELECT i.`cover`, i.`id_image`, il.`legend`, i.`position`
                                                    ...
                                                    WHERE i.`id_product` = '.(int) $p->id.'
                                                    ORDER BY `position`');
                                            $imgs = $cacheProductImages[$p->id.'_'.$id_product_attribute] = [];
                                            foreach ($res as $k => $v)
                                            {
                                                $position_image = count($cacheProductImages[$p->id.'_'.$id_product_attribute]) + 1;
                                                $cacheProductImages[$p->id.'_'.$id_product_attribute][$position_image] = $v['id_image'];
                                        ...
                                        case 'feature':
                                            $field = '';
                                            if (!empty($featuresListByLang[$getIDlangByISO[$f['lang']]]) && sc_array_key_exists($f['options'], $featuresListNameDefault))
                                            {// has feature
                                                $field = $featuresListByLang[$getIDlangByISO[$f['lang']]][$featuresListNameDefault[$f['options']]]['value'];
                                            }
                                            break;
                                        case 'id_feature_value':
                                            ...
                                            if (!empty($featuresListByLang[$getIDlangByISO[$f['lang']]]) && sc_array_key_exists($f['options'], $featuresListNameDefault))
                                            {// has feature
                                                $field = $featuresListByLang[$getIDlangByISO[$f['lang']]][$featuresListNameDefault[$f['options']]]['id_feature_value'];
                                            }
                                            break;
                                        case 'attribute':
                                        ...
                                        case 'reference':
                                            switch ($switchObjectOptionTwo){
                                                ...
                                                default:
                                                    $field = $p->{$f['name']};
                                                    if ($exportConfig['exportcombinations'] && !empty($product_attribute[$f['name']]))
                                                    {
                                                        $field = $product_attribute[$f['name']];
                                                    }
                                            }
                            ...
                            if ($f['modifications'] != '')
                            {
                                ...
                                foreach ($tasks as $t)
                                {
                                    if ($t != '' && (sc_in_array($t, ['strip_tags', 'strtolower', 'strtoupper', 'html_entity_decode'], 'catWinExportProcess_tasksfunctions') || substr($t, 0, 1) == '='))
                                    {
                                        ...
                                        elseif (substr($t, 0, 1) == '=')
                                        {
                                            if (is_numeric($field))
                                            {
                                                if (strpos($t, '(x)'))
                                                {
                                                    $t = str_replace(["'(x)'", '"(x)"'], (is_int($field) ? '(int)$field' : '(float)$field'), $t);
                                                }
                                                else
                                                {
            ...
            if (file_exists(SC_CSV_EXPORT_DIR.$exportConfig['exportfilename']))
            {
                ...
                $urlParams = [
                    'source' => 'export',
                    'detail' => 'catalog',
                    'retrieve' => generateToken(SC_CSV_EXPORT_DIR.$exportConfig['exportfilename']),
                ];
                $exportedFileUrl = SC_ORK_EXTERNAL_URL.'cron/cron_init.php?'.http_build_query($urlParams).'&'.time();
                $message_return = _l('Export:').' <a href="'.$exportedFileUrl.'" target="_blank">/modules/'.SC_MODULE_FOLDER_NAME.'/export/'.$exportConfig['exportfilename'].'</a> - '.$nb_lines.' '._l('lines').' - '.date('Y-m-d H:i:s');
