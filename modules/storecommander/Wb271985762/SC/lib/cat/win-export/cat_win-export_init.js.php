...
echo '<script>'; ?>
    ...
    getTbSettingsButton(wExport.tbOptions, {'grideditor':0,'settings':<?php echo (int)_r('MEN_TOO_SETTINGS'); ?>}, 'win_export_', 50);
    ...
    // UISettings
    wExport.gridFiles._uisettings_prefix='cat_win_export';
    wExport.gridFiles._uisettings_name = wExport.gridFiles._uisettings_prefix;
    wExport.gridFiles._first_loading = 1;
    initGridUISettings(wExport.gridFiles);
    ...
    wExport.gridFilesDataProcessor.attachEvent("onBeforeUpdate", function(id, state, data){
        if(data.exportfilename !== undefined) {
            let rowId = wExport.gridFiles.getSelectedRowId();
            if(data.exportfilename.length <= 0)
            {
                dhtmlx.message({text:'<?php echo _l('Invalid filename', 1); ?>',type:'error'});
                return false;
            }

            if(!data.exportfilename.includes('.csv')) {
                data.exportfilename = data.exportfilename + '.csv';
                wExport.gridFiles.cells(rowId,wExport.gridFiles.getColIndexById('exportfilename')).setValue(data.exportfilename);
            }
        }
    ...
    function setExportOptionsBGColor()
    {
        ...
        wExport.gridMapping.forEachRow(function(rId){
            ...
            if(arrayProductAndCombinationCommonFields.includes(wExport.gridMapping.cells(rId,idxDBField).getValue())){
                ...
                if(wExport.gridMapping.cells(rId,idxOptionsTwo).getValue() == ''){
...
function displayExportLastGrid()
{
    ...
    wExport.gridResult.load("index.php?ajax=1&act=cat_win-export_files_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(), function(){
        filterScriptName();
        wExport.gridResult.sortRows(wExport.gridResult.getColIndexById('date'),"date","des");
    });
...
function displayExportOptions(callback)
{
    ...
    {
        // UISettings
        loadGridUISettings(wExport.gridFiles);
        wExport.gridFiles._first_loading = 0;
...
function displayExportCategories(categselection,callback)
{
        ...
        wExport.treeCategories.load("index.php?ajax=1&act=cat_category_get&id_lang="+SC_ID_LANG+"&with_segment=0&"+new Date().getTime(),function(){
        <?php }
else
{ ?>
        wExport.treeCategories.load("index.php?ajax=1&act=cat_category_get&forceDisplayAllCategories=1&id_lang="+SC_ID_LANG+"&with_segment=0&"+new Date().getTime(),function(){
        <?php } ?>
        if (categselection)
