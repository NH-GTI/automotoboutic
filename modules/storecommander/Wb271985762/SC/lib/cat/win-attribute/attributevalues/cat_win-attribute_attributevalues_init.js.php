...
$propname = _l('Attribute values', 1); ?>
    ...
    if (id=='img_add')
    {
        if (wAttributes.gridAttributes.getSelectedRowId()==null || wAttributes.gridAttributes.getSelectedRowId().split(',').length!=1)
        {
            ...
            if (!dhxWins.isWindow("wAttributeTexture"))
            {
                ...
                ll.cells('a').attachURL('index.php?ajax=1&act=cat_win-attribute_attributevalues_texture&action=add&id_lang='+SC_ID_LANG, null, {id_attribute:wAttributes.gridAttributes.getSelectedRowId()});
				// cell b
