            ...
            if (Tools::isSubmit($field))
            {
                ...
                foreach ($languages as $lang)
                {
                    if(isset($dataLang[$lang['iso_code']]))
                    {
                        ...
                        if (!$exists)
                        {
                            $sql = 'INSERT INTO ' . _DB_PREFIX_ . 'attribute_lang ('.(int) $id_attribute.', '.(int) $lang['id_lang'].',"' . bqSQL($dataLang[$lang['iso_code']]) . '")';
                        }
                        else
                        {
                        ...
                        {
                            addToHistory('attribute', 'modification', $field, (int) $id_attribute, $lang['id_lang'], _DB_PREFIX_.'attribute_lang', pSQL($dataLang[$lang['iso_code']]), (!$exists ? false : pSQL($exists[$field])));
                        }
                        break;
                    }
                }
