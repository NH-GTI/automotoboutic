...
$allowed_format_extension = SCI::getAllowedFileExtension('image');
if ($action == 'delete')
{
    foreach ($id_attribute as $v)
    {
        ...
        {
            if (file_exists(_PS_COL_IMG_DIR_.(int)$v.'.'.$extension))
            {
                @unlink(_PS_COL_IMG_DIR_.(int)$v.'.'.$extension);
                break;
            }
        }
    }
...
if ($action == 'add')
{
    $id_attribute = $id_attribute[0]; ?><!doctype html>
    <html lang="en">
    <head>
        <meta charset="UTF-8" />
        <style>
