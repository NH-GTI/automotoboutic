...
foreach(explode(',',$productIdsToTransform) as $productIdToTransform){
...
foreach ($productsToTransform as $key => $productToTransform) {
    ...
    if (version_compare(_PS_VERSION_, '1.7.8.0', '>=')) {
        $allowed = $productToTransform->getProductType() === PrestaShop\PrestaShop\Core\Domain\Product\ValueObject\ProductType::TYPE_STANDARD;
    }
    if (!$allowed) {
        $forbiddenProducts[]  = ['id' => $productToTransform->id, 'name' => $productToTransform->name[$sc_agent->getIdLang()]];
        unset($productsToTransform[$key]);
        continue;
    }
    $convertData[$productToTransform->id] = (object) [];

}
$convertData = json_encode($convertData, true);
$forbiddenProducts = json_encode($forbiddenProducts, true);

echo '<script>'; ?>
const winConvertProductToCombinationLayout = wConvertProductToCombination.attachLayout("2U");
const attribute_col_prefix = 'attr_';
let errors = [];
let idShop = <?php echo (int)SCI::getSelectedShop() ?>;
// prepare json object for convert_data
let forbiddenProducts = JSON.parse('<?php echo $forbiddenProducts; ?>');
let convertData = JSON.parse('<?php echo $convertData; ?>');
...
winConvertProductToCombinationsPreview_cell.attachURL('index.php?act=cat_win-convertproducttocombination_preview&id_lang=' + SC_ID_LANG + '&ajax=1', true, {'product_ids': JSON.stringify(convertData), 'forbiddenProducts': JSON.stringify(forbiddenProducts)});
    ...
    if(id === 'b'){
        ...
        winConvertProductToCombinationsPreview_cell.cell.querySelector('#create_combinations').addEventListener('click', function() {
            ...
            if(errors.length > 0){
                ...
                convertProductToCombinationForm.send("index.php?ajax=1&act=cat_win-convertproducttocombination_update&idc=<?php echo Tools::getValue('idc','');?>&id_shop="+idShop, convertProductToCombinationsAfterSaved());
            }
        });
    }
})
...
function convertProductToCombinationsAfterSaved(){
    ...
    wConvertProductToCombinationFinalCell.attachURL('index.php?act=cat_win-convertproducttocombination_final&ajax=1', true);
    return true;
}
...
function removeGroupFromSelection(e){
    ...
    const convertDataField = winConvertProductToCombinationsPreview_cell.cell.querySelector('input[name="convert_data"]');
    let convertData = JSON.parse(convertDataField.value);
    for (const [id_product, values] of Object.entries(convertData)) {
        for (const [id_feature, value] of Object.entries(values)) {
            if(id_feature === e.dataset.groupid){
                delete convertData[id_product][id_feature];
            }
        }
    }
    convertDataField.value = JSON.stringify(convertData);
