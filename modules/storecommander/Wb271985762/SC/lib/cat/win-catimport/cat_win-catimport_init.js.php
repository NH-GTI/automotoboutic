...
// IMPORT
    ...
    wCatImport.gridFilesDataProcessor.attachEvent("onBeforeDataSending", function(id, state, data){
...
<?php
    ...
    wCatImport.gridFilesDataProcessor.attachEvent("onBeforeUpdate",function(id,status){
        if (wCatImport.gridFiles.cells(id,wCatImport.gridFiles.getColIndexById('fieldsep')).getValue() === wCatImport.gridFiles.cells(id,wCatImport.gridFiles.getColIndexById('valuesep')).getValue())
            {
...
<?php
    ...
    {
        echo 'wCatImport.gridMapping.cells(rId,idxOptions).setValue("'.$languages[0]['iso_code'].'");return false;';
    }
...
?>
    ...
    wCatImport.tbProcess.setItemToolTip('check','<?php echo _l('Is your import ready? See the Checklist!'); ?>');
    ...
    wCatImport.tbProcess.attachEvent("onClick",
        function(id){
            if (id=='check')
            {
                window.open("<?php echo getScExternalLink('support_csv_import_category_checklist'); ?>");
            }
            if (id=='go_process')
            {
