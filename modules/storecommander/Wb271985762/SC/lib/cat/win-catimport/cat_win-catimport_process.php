...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    switch ($action){
        ...
        case 'mapping_process':
            ...
            if (SCMS && !sc_in_array('id_shop_list', $mappingData['DBArray'], 'catWinCatImportProcess_DBArray'))
            {
            ...
            for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA) - 1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'], $DATA[$current_line])) && ($current_line <= $importlimit)); ++$current_line)
            {
                ...
                $newCategorySaved = false;
                try{
                    $newCategorySaved = $newcat->save();
                } catch (Exception $e) {
                ...
                if ($newCategorySaved)
                {
                    ...
                    if (SCMS && $importConfig[$TODOfilename]['forfoundcat'] != 'skip' && ((!sc_array_key_exists($id_category, $cache_id_shop) && count($id_shop_list_in_csv) > 1) || (!empty($cache_id_shop[$id_category]) && count($cache_id_shop[$id_category]) < count($id_shop_list_in_csv) - 1)))
                    {
                        //mise en cache
            ...
            if ((count($CSVData) == 1) || (count($CSVData) == 2 && $CSVData[0] == join('', $CSVData)) || (filesize(SC_CSV_IMPORT_DIR.'category/'.$TODOfilename) == 0))
            {
                ...
                echo '<script>window.top.displayCatOptions();window.top.stopCatAutoImport(true);</script>';
            ...
            else
            {
                ...
                echo '<script>window.top.displayCatOptions();autoCatImportUnit=999999;window.top.prepareCatNextStep('.($stats['created'] + $stats['modified'] + $stats['skipped'] == 0 ? 0 : filesize(SC_CSV_IMPORT_DIR.'category/'.$TODOfilename)).');</script>';
