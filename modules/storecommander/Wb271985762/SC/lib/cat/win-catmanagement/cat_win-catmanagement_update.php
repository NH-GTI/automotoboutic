...
//    $position = Tools::getValue('position', '0');
    ...
    function duplicateCategories($id_category, $id_parent)
    {
        ...
        if (!empty($result[0]['id_category']))
        {
            ...
            $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'category` ('.bqInSQL(implode(',', $names)).')
            ...
            if (!empty($new_id_cat))
            {
                ...
                if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
                {
                    ...
                    foreach ($results as $result)
                    {
                        ...
                        $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'category_shop` ('.bqInSQL(implode(',', $names)).')
                                    VALUES ("'.implode('","', $values).'")';
                        Db::getInstance()->execute($sql_insert);
                    }
                }
                ...
                foreach ($results as $result)
                {
                    ...
                    $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'category_lang` ('.bqInSQL(implode(',', $names)).')
                                VALUES ("'.implode('","', $values).'")';
                    Db::getInstance()->execute($sql_insert);
                }
                ...
                foreach ($results as $result)
                {
                    ...
                    $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'category_group` ('.bqInSQL(implode(',', $names)).')
                                VALUES ("'.implode('","', $values).'")';
                    Db::getInstance()->execute($sql_insert);
                }
    ...
    if (!empty($action) && $action == 'move')
    {
        ...
        if(in_array($idCateg, [Configuration::get('PS_ROOT_CATEGORY'),Configuration::get('PS_HOME_CATEGORY')])){
            return false;
        }
