...
<?php
    ...
    $sql = 'SELECT c.*, cl.*, c.position '.(!empty($id_shop) ? ', cs.position' : '');
    SC_Ext::readCustomCategoriesGridConfigXML('SQLSelectDataSelect');
    $sql .= ' FROM '._DB_PREFIX_.'category c';
    SC_Ext::readCustomCategoriesGridConfigXML('SQLSelectDataLeftJoin');
    $sql .= ' LEFT JOIN '._DB_PREFIX_.'category_lang cl ON (cl.id_category=c.id_category AND cl.id_lang='.(int) $id_lang.(!empty($id_shop) ? " AND cl.id_shop='".(int) $id_shop."'" : '').')
            '.(!empty($id_shop) ? ' INNER JOIN '._DB_PREFIX_."category_shop cs ON (cs.id_category=c.id_category AND cs.id_shop='".(int) $id_shop."') " : '').'
            GROUP BY c.id_category
    ...
    if (count($res) > 0)
    {
        foreach ($res as $r)
        {
	        $bincategory = $r['id_category'];
	        $bincategory_nleft = $r['nleft'];
	        $bincategory_nright = $r['nright'];
	        $bincategory_parent = $r['id_parent'];
