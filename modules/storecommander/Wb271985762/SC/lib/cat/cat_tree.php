...
?>
    ...
    shopselection = 0;
    <?php if(SCMS) { ?>
    shopselection=Cookies.get('sc_shop_selected');

    if(Boolean(shopselection)){
        if(Number(shopselection) === 0){
    ...
    warehouseselection=Cookies.get('sc_warehouse_selected')*1;
    warehouse_list=Cookies.get('sc_warehouse_list');
    lastProductSelID=0;
...
<?php
    ...
    function saveCheckWarehouseSelection()
    {
        ...
        Cookies.set('sc_warehouse_list',cookie_checked, defaultCookieOptions);
    ...
    function displayWarehouseTree(callback) {
        ...
        cat_warehousetree.load("index.php?ajax=1&act=cat_warehouse_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
                ...
                if (warehouseselection!=null && warehouseselection!=undefined && warehouseselection!=0)
                {
                    ...
                    Cookies.set('sc_warehouse_selected',warehouseselection, defaultCookieOptions);
                }
    ...
    <?php $catTreeHelpId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'help'); ?>
    cat_tb.addButton('<?php echo $catTreeHelpId; ?>', 0, "", "fad fa-question-circle blue <?php echo $catTreeHelpId; ?>", "fad fa-question-circle blue");
    cat_tb.setItemToolTip('<?php echo $catTreeHelpId; ?>','<?php echo _l('Help', 1); ?>');

    <?php if (_r('ACT_CAT_EMPTY_RECYCLE_BIN')) { ?>
    <?php $catTreeBinId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'bin'); ?>
    cat_tb.addButton("<?php echo $catTreeBinId; ?>", 0, "", "fa fa-trash-alt red <?php echo $catTreeBinId; ?>", "fa fa-trash-alt red");
    cat_tb.setItemToolTip('<?php echo $catTreeBinId; ?>','<?php echo _l('Empty bin', 1); ?>');
    <?php } ?>

    <?php $catTreeInBinId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'cat_treegrid_in_bin'); ?>
    cat_tb.addButton("<?php echo $catTreeInBinId; ?>", 0, "", "fad fa-trash grey <?php echo $catTreeInBinId; ?>", "fad fa-trash grey");
    cat_tb.setItemToolTip('<?php echo $catTreeInBinId; ?>','<?php echo _l('Put in the bin/trash', 1); ?>');

    <?php if (_r('ACT_CAT_ADD_CATEGORY')) { ?>
        <?php $catTreeAddPsId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'add_ps'); ?>
        cat_tb.addButton("<?php echo $catTreeAddPsId; ?>", 0, "", "fa fa-prestashop <?php echo $catTreeAddPsId; ?>", "fa fa-prestashop");
        cat_tb.setItemToolTip('<?php echo $catTreeAddPsId; ?>','<?php echo _l('Create new category with the PrestaShop form', 1); ?>');

        <?php $catTreeAddId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'add'); ?>
        cat_tb.addButton("<?php echo $catTreeAddId; ?>", 0, "", "fa fa-plus-circle green <?php echo $catTreeAddId; ?>", "fa fa-plus-circle green");
        cat_tb.setItemToolTip('<?php echo $catTreeAddId; ?>','<?php echo _l('Create new category', 1); ?>');
    <?php } ?>

    <?php $catTreeCatManagementId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'cat_management'); ?>
    cat_tb.addButton("<?php echo $catTreeCatManagementId; ?>", 0, "", "fa fa-cog yellow <?php echo $catTreeCatManagementId; ?>", "fa fa-cog yellow");
    cat_tb.setItemToolTip('<?php echo $catTreeCatManagementId; ?>','<?php echo _l('Categories management', 1); ?>');

    cat_tb.addButtonTwoState("<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'fromIDCategDefault'); ?>", 0, "", "fad fa-at", "fad fa-at");
    cat_tb.setItemToolTip('<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'fromIDCategDefault'); ?>','<?php echo _l('If enabled: display products only from their default category', 1); ?>');
    cat_tb.addButtonTwoState("<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'withSubCateg'); ?>", 0, "", "fad fa-link green", "fad fa-link green");
    cat_tb.setItemToolTip('<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'withSubCateg'); ?>','<?php echo _l('If enabled: display products from all subcategories', 1); ?>');
    cat_tb.addButton("<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'refresh'); ?>", 0, "", "fa fa-sync green", "fa fa-sync green");
    cat_tb.setItemToolTip("<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'refresh'); ?>",'<?php echo _l('Refresh tree', 1); ?>');
    cat_tb.addButtonTwoState("<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'hide_disable_cat'); ?>", 0, "", "fa fa-filter", "fa fa-filter");
    cat_tb.setItemToolTip('<?php echo $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_TREE_TOOLBAR, 'hide_disable_cat'); ?>','<?php echo _l('If enabled: hide disable categories', 1); ?>');
    cat_tb.attachEvent("onClick",
        function(id){
            if(id.includes("_forbidden")) {
                alert(id);
                return false;
            }
            if (id=='help'){
            ...
            if (id=='add'){
                if (catselection!=0)
                {
                    if(Boolean(Number(cat_tree.getUserData(catselection,'is_eservices')))) {
                        alert('<?php echo _l('You can not create a category here', 1); ?>');
                        return false;
                    }
...
?>
    cat_tree.attachEvent("onClick",function(idcategory){
            ...
            else
            {
                ...
                {
                    <?php if(_s('CAT_CATEGORY_TREE_AUTO_CLOSE')) { ?>
                    if(this.getParentId(idcategory) !== this.getParentId(catselection))
                    {
                        this.closeAllItems();
                        this.openItem(idcategory);
                    }
                    <?php } ?>
                    catselection=idcategory;
                    segselection = 0;
...
?>
    ...
    cat_tree.attachEvent("onBeforeContextMenu", function(itemId){
        let is_segment = Number(cat_tree.getUserData(itemId,"is_segment"));
        let is_service = Number(cat_tree.getUserData(itemId,"is_eservices"));
        ...
        switch(1)
        {
            case is_segment:
                ...
                break;
            case is_service:
                cat_cmenu_tree.setItemText('object', 'ID'+itemId+': <?php echo _l('Service:'); ?> '+cat_tree.getItemText(itemId));
                cat_cmenu_tree.hideItem('sort');
                cat_cmenu_tree.hideItem('goshop');
                cat_cmenu_tree.hideItem('gopsbo');
                cat_cmenu_tree.hideItem('sort_and_save');
                cat_cmenu_tree.hideItem('enable');
    ...
    {
        openCategoryTreeDynamicLoading();
    })
...
?>
    ...
    function openCategoryTreeDynamicLoading()
    {
        if(enalbeDynamicTreeLoading){
