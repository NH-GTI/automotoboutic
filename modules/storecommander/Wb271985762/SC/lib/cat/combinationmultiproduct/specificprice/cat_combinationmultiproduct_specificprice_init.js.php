                    ...
                    for(const cId of prop_tb._combinationmultiproductSpecificPricesGrid.columnIds) {
                        switch(cId) {
                            ...
                            case 'price':
                                ...
                                break;
                            case 'reduction_tax':
                                newRow.push('<?= _s('CAT_PROD_SPECIFIC_PRICES_DEFAULT_TAX'); ?>');
                                break;
                            default:
                                newRow.push("");
                        }
        ...
        if (itemId=="paste"){
            if (lastColumnRightClicked_CombinationMultiProductSpecificprices!=0 && clipboardValue_CombinationMultiProductSpecificprices!=null && clipboardType_CombinationMultiProductSpecificprices==lastColumnRightClicked_CombinationMultiProductSpecificprices)
            {
                ...
                if (selection!='' && selection!=null)
                {
                    ...
                    for(const rowId of selArray) {
                        let oldValue = prop_tb._combinationmultiproductSpecificPricesGrid.cells(rowId,lastColumnRightClicked_CombinationMultiProductSpecificprices).getValue();
                        prop_tb._combinationmultiproductSpecificPricesGrid.cells(rowId,lastColumnRightClicked_CombinationMultiProductSpecificprices).setValue(clipboardValue_CombinationMultiProductSpecificprices);
                        prop_tb._combinationmultiproductSpecificPricesGrid.cells(rowId,lastColumnRightClicked_CombinationMultiProductSpecificprices).cell.wasChanged=true;
                        prop_tb._combinationmultiproductSpecificPricesGrid.callEvent('onEditCell',[2, rowId, lastColumnRightClicked_CombinationMultiProductSpecificprices, clipboardValue_CombinationMultiProductSpecificprices, oldValue]);
                    }
                }
            }
        }
...
// DISPLAY
    function getCombinationMultiProductSpecificPrices()
    {
        ...
        $.post("index.php?ajax=1&act=cat_combinationmultiproduct_specificprice_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),
            {
                id_product_attribute_list: prop_tb._combinationmultiproductGrid.getSelectedRowId()
            }
            ,function(data)
        {
