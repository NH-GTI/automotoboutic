...
//    }
    ...
    if ((int) SCI::getConfigurationValue('PS_USE_ECOTAX', null, 0, SCI::getSelectedShop()) == 0)
    {
    ...
    // Color groups
    $arrColorGroups = [0 => _l('Do not display')];
    ...
    */
    ...
    {
        $sql = 'SELECT id_product, COUNT(upc.id_ukooparts_engine) as nb_compat
                ...
                INNER JOIN '._DB_PREFIX_.'ukooparts_engine_type_lang upetl ON (upet.id_ukooparts_engine_type = upetl.id_ukooparts_engine_type)
                GROUP BY id_product';
        $res = Db::getInstance()->executeS($sql);
        if (!empty($res))
        {
    ...
    function getProducts($id_category, $segment_params = [])
    {
        global $sql,$col,$tax,$arrTax,$arrManufacturers,$arrSuppliers,$prodWithAttributes,$countAttributes,$id_lang,$cols,$view,$colSettings,$user_lang_iso,$fields,$fields_lang,$forceUpdateCombinations,$fieldsWithHTML,$prodrow,$exportedProducts,$prodWithCompatibilities,$multiStoresFields;
        $link = new Link();
        $col = '';
        $defaultimg = 'lib/img/i.gif';

        $ids_to_update = Tools::getValue('ids_to_update', false);
        ...
        $fields = ['reference', 'wholesale_price', 'price', 'unity', 'unit_price_ratio', 'ecotax', 'weight', 'supplier_reference', 'id_manufacturer', 'id_supplier',
        ...
        // PACK
        if (version_compare(_PS_VERSION_, '1.6.1.12', '>='))
        {
        ...
        $fields_lang = ['name', 'available_now', 'available_later', 'link_rewrite', 'meta_title', 'meta_description', 'meta_keywords', 'description_short', 'description'];
        ...
        ## extensions
        if (_s('APP_FEEDBIZ') && defined('SC_FeedBiz_ACTIVE') && SC_FeedBiz_ACTIVE == 1 && SCI::moduleIsInstalled('feedbiz'))
        {
        ...
        if (Sc\Service\Service::exists('shippingbo') && (
                sc_in_array('id_sbo', $cols, 'cols')
                || sc_in_array('type_sbo', $cols, 'cols')
                || sc_in_array('is_locked', $cols, 'cols')
            )
        ) {
            $selectGuessTypeSbo = Sc\Service\Shippingbo\Repository\Prestashop\BaseRepository::getSelectForGuessedSboType();
            $extension_fields[] = 'IF(ps_relation.id_product_attribute = 0, ps_relation.`id_sbo`,"") as id_sbo';
            $extension_fields[] = 'COALESCE(ps_relation.`type_sbo`, '.$selectGuessTypeSbo.') as type_sbo';
            $extension_fields[] = 'ps_relation.`id_sbo_source` as id_sbo_source';
            $extension_fields[] = 'COALESCE(ps_relation.`is_locked`, true) as is_locked';
            $extension_sql_JOIN[] = ' LEFT JOIN '._DB_PREFIX_.(new Sc\Service\Shippingbo\Model\ShopRelationModel())->getTableName().' ps_relation ON ps_relation.`id_product` = p.`id_product` AND ps_relation.id_shop = '.(int) SCI::getSelectedShop().' ';
        }

        ## extensions array modification
        if (!empty($extension_fields))
        {
            $extension_without_alias = [];
        ...
        $forceUpdateCombinations = ['price_inc_tax', 'price', 'id_tax', 'id_tax_rules_group'];
        $fieldsWithHTML = ['description', 'description_short'];
        $multiStoresFields = [];
        ...
        $blacklistfields = ['margin', 'reduction_price', 'reduction_percent', 'reduction_from', 'reduction_to'];
        ...
        if (SCSG && $is_segment && !empty($id_segment))
        {
            ...
            {
                $params = ['id_lang' => $id_lang, 'id_segment' => $id_segment, 'access' => 'catalog'];
                for ($i = 1; $i <= 15; ++$i)
                {
            ...
            $sql .= ' FROM '._DB_PREFIX_.'product p
                        ...
                        LEFT JOIN '._DB_PREFIX_.'product_supplier ps ON (ps.id_product=p.id_product AND ps.id_product_attribute=0 AND ps.id_supplier=p.id_supplier)
            ...
            $sql .=
                            ...
                            ((!SCMS) ? 'INNER JOIN '._DB_PREFIX_.'product_shop prs ON (prs.id_product=p.id_product AND prs.id_shop = p.id_shop_default)' : '');
            ...
            if ($ids_to_update)
            {
                $sql .= " AND p.id_product IN('".str_replace(',', '\',\'', $ids_to_update)."')";
            }
        ...
        else
        {
            ...
            { // by id_category_default
                ...
                SC_Ext::readCustomGridsConfigXML('SQLSelectDataLeftJoin');
                $sql .= ' WHERE 1=1 '.
                (!empty($id_category) ? ' AND '.((SCMS && isset($selected_shops_id) && $selected_shops_id > 0) || (!SCMS) ? 'prs.' : 'p.').'id_category_default='.(int) $id_category : '').' ';
                if ($ids_to_update)
                {
                    $sql .= " AND p.id_product IN('".str_replace(',', '\',\'', $ids_to_update)."')";
                }
            ...
            else
            {
                ...
                $sql .= ' FROM '._DB_PREFIX_.'category_product cp
                            ...
                            LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (pl.id_product= p.id_product AND pl.id_lang='.(int) $id_lang.(SCI::getSelectedShop() > 0 ? ' AND pl.id_shop='.(int) SCI::getSelectedShop() : ' AND pl.id_shop=p.id_shop_default ').') 
                            LEFT JOIN '._DB_PREFIX_.'product_supplier ps ON (ps.id_product=p.id_product AND ps.id_product_attribute=0 AND ps.id_supplier=p.id_supplier)
                             LEFT JOIN '._DB_PREFIX_.'stock_available sa ON (sa.id_product=p.id_product AND sa.id_product_attribute=0 '.($shop_group->share_stock ? 'AND sa.id_shop_group='.(int) $shop_group->id.' AND sa.id_shop=0' : 'AND sa.id_shop='.(SCI::getSelectedShop() > 0 ? (int) SCI::getSelectedShop() : 'p.id_shop_default')).')
                                '.(SCMS ? '    LEFT JOIN '._DB_PREFIX_.'supplier_shop ss ON (ss.id_supplier = ps.id_supplier AND ss.id_shop='.(SCI::getSelectedShop() > 0 ? (int) SCI::getSelectedShop() : 'p.id_shop_default').')' : '');
        ...
        $value_marketplace_options = [0 => _l('No'), 1 => _l('Yes'), 2 => _l('Mixed')];
        $cache_amazon_present = $cache_cdiscount_present = $cache_feedbiz_present = $cache_feedbiz_amazon_present = $cache_feedbiz_cdiscount_present = [];
        if (_s('APP_FEEDBIZ') && defined('SC_FeedBiz_ACTIVE') && SC_FeedBiz_ACTIVE == 1 && SCI::moduleIsInstalled('feedbiz'))
        {
        ...
        if (_s('APP_AMAZON') && defined('SC_Amazon_ACTIVE') && SC_Amazon_ACTIVE == 1 && SCI::moduleIsInstalled('amazon'))
        {
        ...
        if (_s('APP_CDISCOUNT') && defined('SC_Cdiscount_ACTIVE') && SC_Cdiscount_ACTIVE == 1 && SCI::moduleIsInstalled('cdiscount'))
        {
            $id_product_list = [];
            foreach ($res as $r)
            {
        ...
        if (Sc\Service\Service::exists('shippingbo'))
        {
            ...
            $sboDataIndexByProductId = $shippingboService->getSboDataForProducts(array_column($res, 'id_product'));
        }
        ...
        foreach ($res as $prodrow)
        {
            ...
            ## avoir useless warning
            $prodrow['price'] = (isset($prodrow['price']) ? $prodrow['price'] : 0);
            $prodrow['active'] = (isset($prodrow['active']) ? $prodrow['active'] : 0);
            $prodrow['ecotax'] = (isset($prodrow['ecotax']) ? $prodrow['ecotax'] : 0);
            ...
            $reductionColor = '';
            $reductionNameColor = '';
            $user_data = ['id_specific_price'];
            if (sc_in_array('reduction_from', $cols, 'cols'))
            {
                $prodrow['price_ttc'] = (float) ($prodrow['price'] * ($tax[(int) $prodrow['id_tax']] / 100 + 1)) + (_s('CAT_PROD_ECOTAXINCLUDED') && $prodrow['ecotax'] > 0 ? $prodrow['ecotax'] * SCI::getEcotaxTaxRate() : 0);
                $prodrow['price_with_reduction'] = (float) $prodrow['price_ttc'];
                $prodrow['price_with_reduction_percent'] = (float) $prodrow['price_ttc'];
                ...
                $sql_specific_price = 'SELECT *
                                    ...
                                    WHERE id_product = '".$prodrow['id_product']."'
                                         ...
                                         AND id_product_attribute = 0  AND id_cart = 0
            ...
            $product_is_adv_pack = false;
            if ($is_adv_pack && $product_is_adv_pack = (bool) (in_array($prodrow['id_product'], $is_adv_pack)))
            {
                echo '<userdata name="isAdvancedPack">1</userdata>';
            }
            if (Sc\Service\Service::exists('shippingbo', true))
            {
                if(!isset($sboDataIndexByProductId[(int)$prodrow['id_product']])) {
                    $sboDataIndexByProductId[(int)$prodrow['id_product']] = [
                        'id_sbo' => '',
                        'type_sbo' => '',
                        'id_sbo_source' => '',
                    ];
                }
                $sboData = $sboDataIndexByProductId[(int)$prodrow['id_product']];

                echo '<userdata name="id_sbo">'.$sboData['id_sbo'].'</userdata>'."\n";
                echo '<userdata name="type_sbo">'.$sboData['type_sbo'].'</userdata>'."\n";
                echo '<userdata name="id_sbo_source">'.$sboData['id_sbo_source'].'</userdata>'."\n";
                $prodrow['type_sbo'] = $sboData['type_sbo'];
            }
            ...
            foreach ($cols as $key => $col)
            {
                switch ($col){
                    ...
                    case 'supplier_reference':
                        if (!empty($prodrow['id_supplier']) && _s('CAT_PROD_REFERENCE_SUPPLIER') == 1)
                        {
                    ...
                    case 'ecotax':
                        ...
                        echo '<cell'.($product_is_adv_pack ? ' bgColor="#D7D7D7" type="ro"' : '').'>'.number_format($prodrow['ecotax'] * SCI::getEcotaxTaxRate(), 6, '.', '').'</cell>';
                        break;
                    case 'price':
                        if ($product_is_adv_pack)
                        {
                            echo '<cell bgColor="#D7D7D7" type="ro">'.number_format($prodrow['price'], 6, '.', '').'</cell>';
                            break;
                        }
                    ...
                    case 'price_inc_tax':
                        ...
                        echo '<cell'.($product_is_adv_pack ? ' bgColor="#D7D7D7" type="ro"' : '').'>'.number_format($value, 6, '.', '').'</cell>';
                        break;
                    case 'unit_price_ratio':
                        ...
                        echo '<cell'.($product_is_adv_pack ? ' bgColor="#D7D7D7" type="ro"' : '').'>'.$temp_val.'</cell>';
                        break;
                    ...
                    case 'description':
                        echo '<cell><![CDATA['.cleanLineBreakForHtmlFieldInGrid($prodrow[$col]).']]></cell>';
                        break;
                    case 'combinations':
                    case 'features':
                    case 'categories':
                    ...
                    case 'compatibilities':
                        ...
                        break;
                    case 'nb_parts_compatibilities':
                        echo '<cell>'.(!empty($prodWithCompatibilities[(int) $prodrow['id_product']]) ? $prodWithCompatibilities[(int) $prodrow['id_product']] : '0').'</cell>';
                        break;
                    case 'parts_compatibilities':
                        echo '<cell><![CDATA[parts_compatibilities_'.(int) $prodrow['id_product'].']]></cell>';
                        break;
                    case 'amazon':
                    ...
                    case 'product_type':
                        ...
                        break;
                    case 'type_sbo':
                        echo '<cell><![CDATA['.ucfirst(_l($prodrow['type_sbo'])).']]></cell>';
                        break;
                    case 'is_locked':
                        echo '<cell id="is_locked">'.$prodrow['is_locked'].'</cell>';
                        break;
                    case 'id_tax_rules_group':
                        if ($product_is_adv_pack)
                        {
                            echo '<cell bgColor="#D7D7D7" type="ro">'.$arrTax[$prodrow[$col]].'</cell>';
                            break;
                        }
                        echo '<cell>'.$prodrow[$col].'</cell>';
                        break;
                    default:
                        ...
                        {
                            $prodrow[$col] = !isset($prodrow[$col]) ? '' : $prodrow[$col];
                            if ($prodrow[$col] == '' || $prodrow[$col] === 0 || $prodrow[$col] === 1)
                            { // opti perf is_numeric($prodrow[$col]) ||
                                echo '<cell>'.$prodrow[$col].'</cell>';
                            }
                            elseif ($colSettings[$col]['type'] == 'dhxCalendarA')
                            {
                                $dateTime = new DateTime($prodrow[$col]);
                                echo '<cell>'.($prodrow[$col] !== '0000-00-00' ? $dateTime->format('Y-m-d H:i:s') : '').'</cell>';
                            }
                            else
                            {
                                echo '<cell><![CDATA['.$prodrow[$col].']]></cell>';
    ...
    if (SCSG && substr($current_id_category, 0, 4) == 'seg_')
    {
        ...
        echo '<userdata name="id_segment"><![CDATA['.$segment_params['segment_object']->id.']]></userdata>'."\n";
        if(Sc\Service\Service::exists('shippingbo')) {
            $sboSegmentIds = \Sc\Service\Shippingbo\Repository\Prestashop\SegmentRepository::getSegmentsIds(SC_Agent::getInstance());
            if(in_array($segment_params['segment_object']->id,$sboSegmentIds)){
                echo '<userdata name="is_sbo_segment">1</userdata>'."\n";
            }
        }
    }
