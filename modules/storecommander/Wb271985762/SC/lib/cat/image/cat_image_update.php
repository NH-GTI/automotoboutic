...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    $id_products = Tools::getValue('id_product_list', 0);
    ...
    foreach ($id_image_array as $id_image)
    {
        switch ($action){
            ...
            case 'shop':
                ...
                $is_cover = (int) Tools::getValue('is_cover', 0);
                if (!empty($id_shop))
                {
                    $image = new Image((int) $id_image);
                    if (!$image->isAssociatedToShop($id_shop) && $value == '1')
                    {
                        ...
                        $isImageCoverExistsInDestinationShop = Db::getInstance()->getValue('SELECT COUNT(*) 
                                                                                                FROM '._DB_PREFIX_.'image_shop 
                                                                                                WHERE id_product = '.(int)$id_prd.'
                                                                                                AND id_shop = '.(int)$id_shop.'
                                                                                                AND cover = 1');
                        if (empty($is_cover) || $isImageCoverExistsInDestinationShop)
                        {
            ...
            case 'thumbnail_regeneration':
                ...
                if(isset($json_reponse[$id_image]['error']))
                {
