...
?>
    ...
    prop_tb.addButton('image_exportcsv',1000, '', 'fad fa-file-csv green', 'fad fa-file-csv green');
    prop_tb.setItemToolTip('image_exportcsv','<?php echo _l('Export grid to clipboard in CSV format for MSExcel with tab delimiter.', 1); ?>');

    <?php if (SC_DEMO || (defined('SUB9EHS4PLUS') && SUB9EHS4PLUS == '1')) { ?>
    var opts = [
    ...
    getTbSettingsButton(prop_tb, {'grideditor':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':<?php echo (int)_r('MEN_TOO_SETTINGS'); ?>}, 'prop_image_',1000);
...
<?php } ?>
                        ...
                        if (lastColumnRightClicked_Images != 0 && clipboardValue_Images != null && clipboardType_Images == lastColumnRightClicked_Images) {
                            ...
                            if (selection != '' && selection != null) {
                                for(const row_id of selection.split(',')){
                                    ...
                                    switch(prop_tb._imagesGrid.getColType(lastColumnRightClicked_Images))
                                    {
                                        case 'ch':
    ...
    function setPropertiesPanel_images(id){
        if (id=='images')
        {
            ...
            prop_tb.showItem('image_exportcsv');
...
?>
        ...
        if (id=='image_add'){
            ...
            if (products_ids != undefined && products_ids!=0)
            {
                if (!dhxWins.isWindow("wProductImages"+products_ids))
                {
                    ...
                    ll_toolbar.setItemState('auto_upload', (Cookies.get('sc_cat_img_auto_upload')==1?1:0));
                    ...
                    ll_toolbar.attachEvent("onStateChange", function(id,state){
                            if (id=='auto_upload'){
                                ...
                                Cookies.set('sc_cat_img_auto_upload',auto_upload, defaultCookieOptions);
                            }
                        });
            ...
            if (prop_tb._imagesGrid.getSelectedRowId()==null)
            {
                ...
                if (lastProductSelID!=0)
                {
                    if (confirm('<?php echo _l('Are you sure you want to change the legend of the selected image(s) with the product name for the current language?', 1); ?>'))
                    {
                        ...
                        $.post("index.php?ajax=1&act=cat_image_update&action=image_fill_legend_current_lang&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'id_product_list': cat_grid.getSelectedRowId(), 'list_id_image':prop_tb._imagesGrid.getSelectedRowId()},function(){
            ...
            if (prop_tb._imagesGrid.getSelectedRowId()==null)
            {
                ...
                if (lastProductSelID!=0)
                {
                    if (confirm('<?php echo _l('Are you sure you want to change the legend of the selected image(s) with the product name for all languages?', 1); ?>'))
                    {
                        ...
                        idxLegend=prop_tb._imagesGrid.getColIndexById('legend');
                        idxProductName=cat_grid.getColIndexById('name');
                        $.post("index.php?ajax=1&act=cat_image_update&action=image_fill_legend_all_lang&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'id_product_list': cat_grid.getSelectedRowId(), 'list_id_image':prop_tb._imagesGrid.getSelectedRowId()},function(){
        ...
        if (id=='image_exportcsv'){
            displayQuickExportWindow(prop_tb._imagesGrid,1);
        }
    }
...
function displayImages(callback)
{
    ...
    $.post("index.php?ajax=1&act=cat_image_get",{
        id_product_list:cat_grid.getSelectedRowId(),
        id_lang:SC_ID_LANG,
...
{
    prop_tb._imageFastUpload.attachURL("index.php?ajax=1&act=cat_image_upload&id_lang="+SC_ID_LANG, null, {product_list:cat_grid.getSelectedRowId()});
}
...
function callbackCutOut(opts)
{
    ...
    cat_grid_tb.attachEvent("onClick",function(item){
        switch(item)
        {
            case 'selectall':
                if (propertiesPanel === 'images' && cat_grid.getSelectedRowId()){
                    ...
                    displayFastUpload();
                    displayFastMobileTabletUpload();
                    images_current_id = selection[selection.length - 1];
                    if (dhxWins.isWindow("wQrCodeImageImporter")) {
                        // refresh qrcode
                        $.get("index.php?ajax=1&act=cat_image_qrcode_init&id_lang="+SC_ID_LANG,function(data){
                            $('#jsExecute').html(data);
