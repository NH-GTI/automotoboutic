...
<script>
    ...
    vaultObject.events.on("BeforeAdd", function(item){
        ...
        if(authorized_extensions.indexOf(extension) >= 0) {
            ...
            let limitSize = <?php echo $size_limit; ?>;
            if (fileSizeMo > limitSize) {
                dhx.message({
                    text: "<?php echo _l('The file is too large. Maximum size allowed is: %1$d Mo. The file you are trying to upload is ', 1, array($size_limit)); ?> " + fileSizeMo.toFixed(2) + " Mo",
                    css: "dhx-error",
                    expire: 4000
                });
                return false;
