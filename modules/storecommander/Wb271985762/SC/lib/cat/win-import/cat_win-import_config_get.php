        ...
        foreach ($files as $file)
        {
            ...
            if (strtolower(substr($file, strlen($file) - 4, 4)) == '.csv' && strpos($file, '&') === false)
            {
                ...
                $urlParams = array(
                    'source' => 'import',
                    'detail' => 'catalog',
                    'retrieve' => generateToken(SC_CSV_IMPORT_DIR.$file)
                );
                $url = SC_ORK_EXTERNAL_URL.'cron/cron_init.php?'.http_build_query($urlParams).'&'.time();
                echo '<userdata name="fileUrl"><![CDATA['.$url.']]></userdata>';
...
<?php
    echo '<userdata name="uisettings">'.UISettings::getSetting('cat_win_import').'</userdata>'."\n";
    getFiles();
    echo '</rows>';
?>