...
$sql = 'SELECT s.id_supplier, s.`name`
        ...
        LEFT JOIN `'._DB_PREFIX_.'supplier_shop` `ss` ON s.`id_supplier` = ss.`id_supplier` AND ss.`id_shop` = '.(int) SCI::getSelectedShop().' 
...
// IMPORT
    ...
    arrayFieldOption=Array('feature','id_feature_value','feature_custom','attribute','attribute_multiple','supplier_reference','wholesale_price','supplier_currency'<?php if (SCAS) { echo ",'quantity','location','add_quantity','remove_quantity','quantity_on_sale'"; } echo SC_Ext::readImportCSVConfigXML('definitionForOptionField'); ?>);
    var comboArray = null;
    ...
    getTbSettingsButton(wImport.tbOptions, {'grideditor':0,'settings':<?php echo (int)_r('MEN_TOO_SETTINGS'); ?>}, 'win_import_', 170);
    ...
    wImport.tbOptions.attachEvent("onClick",
        function(id){
            ...
            if (id=='download')
            {
                window.open(wImport.gridFiles.getUserData(wImport.gridFiles.getSelectedRowId(),'fileUrl'));
            }
            if (id=='delete')
            {
            ...
            if(id=='readEditCsv')
            {
                if(typeof idxFilename != 'undefined' && idxFilename != ''){
                    ...
                    var postData = "filename="+filename+"&fieldsep="+fieldSep+"&utf8="+forceUTF8+"&nbrowstart="+nbRowEStart+"&nbrowend="+nbRowEnd;
    ...
    // UISettings
    wImport.gridFiles._uisettings_prefix='cat_win_import';
    wImport.gridFiles._uisettings_name = wImport.gridFiles._uisettings_prefix;
    wImport.gridFiles._first_loading = 1;
    initGridUISettings(wImport.gridFiles);
    ...
    wImport.gridFilesDataProcessor.attachEvent("onBeforeDataSending", function(id, state, data){
        let key = Object.keys(data)[0];
        delete data[key].fileUrl
            ...
            if (id=='saveasbtn')
            {
                if (wImport.tbMapping.getValue('saveas')=='')
                {
                    ...
                    wImport.gridMapping.forEachRow(function(id){
                            ...
                            {
                                mapping.push([
                                    wImport.gridMapping.cells(wImport.gridMapping.getRowIndex(id),1).getValue(),
                    ...
                    $.post('index.php?ajax=1&act=cat_win-import_mapping_update&action=mapping_saveas',
                    {
                        filename: wImport.tbMapping.getValue('saveas'),
...
<?php SC_Ext::readImportCSVConfigXML('importMappingCheckGrid'); ?>
    ...
    function onEditCellMapping(stage,rId,cInd,nValue,oValue){
        if(stage==1 && (cInd==2 || cInd==3)){
            this.editor.list.setAttribute('size', (this.editor.combo.size() > 15 ? 15 : this.editor.combo.size()+1));
...
function displayOptions(callback)
{
    ...
    {
        // UISettings
        loadGridUISettings(wImport.gridFiles);
        wImport.gridFiles._first_loading = 0;
...
<?php
    ...
    {
        $array[_l('show condition', 1)] = "comboDBField.put('show_condition','".mb_strtolower(_l('Show condition', 1), 'UTF-8')."');";
    ...
    {
        $array[mb_strtolower(_l('Additional delivery times', 1), 'UTF-8')] = "comboDBField.put('additional_delivery_times','".mb_strtolower(_l('Additional delivery times', 1), 'UTF-8')."');";
    ...
    uksort($array, function($a, $b) {
        return strnatcmp(Tools::replaceAccentedChars($a), Tools::replaceAccentedChars($b));
    });
...
<?php echo '</script>'; ?>
<div id="alertbox" style="width:400px;height:200px;color:#FFFFFF;display:none" onclick="stopAlert();">Click here to close alert.</div>
