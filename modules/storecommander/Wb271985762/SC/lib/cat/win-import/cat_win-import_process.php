...
if (!defined('STORE_COMMANDER'))
{
    ...
    set_exception_handler('getReadableError');
    ...
    switch ($action){
        case 'conf_delete':
            ...
            break;
        case 'mapping_load':
            echo loadMapping(basename(Tools::getValue('filename', '')));
            break;
        case 'addSupplier':
        ...
        case 'mapping_process':
            ...
            $getIDlangByISO = [];
            $presentIdShopListInShop = Shop::getShops(false, null, true);

            foreach ($languages as $lang)
            {
                $getIDlangByISO[$lang['iso_code']] = $lang['id_lang'];
            }
            $files = array_diff(scandir(SC_CSV_IMPORT_DIR), array_merge(['.', '..', 'index.php', '.htaccess', SC_CSV_IMPORT_CONF]));
            ...
            if ($CRON)
            {
                ...
                $mapping = loadMapping(($forcemapping ? $forcemapping : $importConfig[$filename]['mapping']), true);
            }
            ...
            if ($importConfig[$filename]['fornewproduct'] == 'create' && $importConfig[$filename]['idby'] == 'idproduct')
            {
                exit(_l('id_product cannot be the identifier for product creation.'));
            }
            ...
            if (SCMS && !sc_in_array('id_shop_list', $mappingData['DBArray'], 'catWinImportCheck_idShopList_in_DBArray'))
            {
            ...
            $dataArray_VAT = [];
            $dataArray_supplier = [];
            $dataArray_manufacturer = [];
            $dataArray_feature = [];
            $dataArray_multiplefeature = [];
            $dataArray_attributegroup = [];
            $dataArray_attributegroup = [];
            $err = '';
            ...
            for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA) - 1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'], $DATA[$current_line])) && ($current_line <= $importlimit)); ++$current_line)
            {
                ...
                if (sc_in_array('id_shop_list', $mappingData['DBArray'], 'catWinImportProcess_DBArray'))
                {
                    $tmpIdShopList = array_map('intval', explode($importConfig[$TODOfilename]['valuesep'], findCSVLineValue('id_shop_list')));
                    $shopIdNotPresentInShop = array_diff($tmpIdShopList, $presentIdShopListInShop);
                    if ($shopIdNotPresentInShop)
                    {
                        $err .= _l('Error on line ').($current_line + 1).'<br>'._l('Field')._l(':').'id_shop_list<br>'._l('These id shops do not exist on your store: %s', null, [implode(',', $shopIdNotPresentInShop)]).'<br/>';
                    }
                }
                if (sc_in_array('id_shop_default', $mappingData['DBArray'], 'catWinImportProcess_DBArray'))
                {
                    $tmpIdShopList = (int) findCSVLineValue('id_shop_default');
                    if (!in_array($tmpIdShopList, $presentIdShopListInShop))
                    {
                        $err .= _l('Error on line ').($current_line + 1).'<br>'._l('Field')._l(':').'id_shop_default<br>'._l('These id shops do not exist on your store: %s', null, [$tmpIdShopList]).'<br/>';
                    }
                }
        ...
        WHERE trg.active=1';
            ...
            // visibility check
            $isErr = false;
            $dataDB_visibilityByName = [
                strtolower(trim(_l('Both'))) => 'both',
                strtolower(trim(_l('Everywhere'))) => 'both',
                strtolower(trim(_l('Catalog'))) => 'catalog',
                strtolower(trim(_l('Catalog only'))) => 'catalog',
                strtolower(trim(_l('Search'))) => 'search',
                strtolower(trim(_l('Search only'))) => 'search',
                strtolower(trim(_l('None'))) => 'none',
                strtolower(trim(_l('Nowhere'))) => 'none',
            ];
            $dataDB_visibility = array_unique(array_values($dataDB_visibilityByName));

            // manufacturer check
            $isErr = false;
            $dataDB_manufacturer = [];
            $dataDB_manufacturerByName = [];
            ...
            {
                if (sc_in_array($mappingData['CSV2DB'][$CSVFieldName], ['feature', 'feature_custom', 'feature_add', 'feature_delete'], 'catWinImportProcess_featurefields'))
                {
            ...
            else
            {
                ...
                $newcategory->position = SCI::getLastPositionFromCategory($newcategory->id_parent);
                $newcategory->save();
                refreshCacheCategory();
            ...
            $refsupp_option = '';
            if (sc_in_array($importConfig[$TODOfilename]['idby'], ['supref'], 'catWinImportProcess_supref'))
            {
            ...
            $stats = ['created' => 0, 'modified' => 0, 'skipped' => 0, 'no_wholesaleprice' => 0];
            ...
            $updated_products = [];
            $id_shop_list = [];
            $id_shop_list_default = [1];
            $id_shop_list_default = [(int) SCI::getConfigurationValue('PS_SHOP_DEFAULT')];
                if ($is_adv_pack_import)
                {
                    $advp_isl = $id_shop_list_default;
                }
            $cache_id_shop = [];
            $cache_current_line_for_combination = [];
            $imageList = [];
            $productsWithTagUpdatedList = [];
            $duplicateCache = [];
            for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA) - 1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'], $DATA[$current_line])) && ($current_line <= $importlimit)); ++$current_line)
            {
                if ($DATA[$current_line] == '')
                ...
                switch ($importConfig[$TODOfilename]['idby'])
                {
                        ...
                        if (!empty($name))
                        {
                            $sql = 'SELECT p.id_product,p.date_upd,p.id_supplier
                                    ...
                                    WHERE pl.name='".pSQL($name)."'
                                        ".(empty($filter_supplier_id) ? '' : ' AND p.id_supplier='.(int) $filter_supplier_id).' '.
                        ...
                        if (!empty($id_product_find))
                        {
                            $sql = 'SELECT p.id_product,p.date_upd,p.id_supplier
                                    ...
                                    WHERE p.id_product='.(int) $id_product_find.'
                                        '.(empty($filter_supplier_id) ? '' : ' AND id_supplier='.(int) $filter_supplier_id).' '.
                                        (!empty($where_shop_list) ? ' AND EXISTS (SELECT * FROM '._DB_PREFIX_.'product_shop ps WHERE p.id_product=ps.id_product AND id_shop IN ('.pSQL($where_shop_list).'))' : '').'
                                    LIMIT 1';
                        }
                        else
                    ...
                    case 'prodref':
                        ...
                        if (!empty($search_reference))
                        {
                            ...
                            {
                                // TODO revoir toute la partie identification pour que ce soit plus lisible/somple a coder et plus efficace

                                $sql = (new DbQuery())
                                    ->select('p.`id_product`, p.`id_supplier`')
                                    ->select('pa.`id_product_attribute`, pa.`date_upd`')
                                    ->from('product', 'p')
                                ...
                                {
                                    $sql
                                        ->innerJoin('product_attribute_shop', 'pas', 'p.`id_product` = pas.`id_product` AND pas.`id_shop` IN ('.pInSQL($where_shop_list).')')
                                ...
                                {
                                    $sql->where('p.id_supplier='.(int) $filter_supplier_id);
                                }
                                $res = Db::getInstance()->executeS($sql);
                                if (!$res)
                                {
                                    $sql = (new DbQuery())
                                        ->select('p.`id_product`, p.`date_upd`, p.`id_supplier`')
                                        ->from('product', 'p')
                                    ...
                                    if ($where_shop_list)
                                    {
                                        $subQuery = (new DbQuery())
                                            ...
                                            ->where('ps.`id_shop` IN ('.pInSQL($where_shop_list).')')
                                        ;
                                        $sql->where('EXISTS ('.$subQuery->build().')');
                                    }
                                    if (!empty($filter_supplier_id))
                                    {
                                        $sql->where('p.`id_supplier` = '.(int) $filter_supplier_id);
                    ...
                    case 'supref':
                        ...
                        if (empty($search_ref))
                        {
                            ...
                            {
                                $search_supp_ref = '';
                                if ($refsupp_option != 'suppref_product')
                                {
                        ...
                        if ($refsupp_option != 'suppref_product')
                        {
                            ## if mapping option supplier ID specified
                            ## and Import supplier ID is specified too but not the same
                            ## ignore row
                            $refsupp_SupplierId = (int) str_replace('suppref_supp_', '', $refsupp_option);
                            if ($filter_supplier_id > 0 && $refsupp_SupplierId !== $filter_supplier_id)
                            {
                                break;
                            }
                            elseif ($filter_supplier_id === 0)
                            {
                                $filter_supplier_id = $refsupp_SupplierId;
                            }

                            ## search ref into product_supplier table for product_attribute?
                            $sql = (new DbQuery())
                                ->select('pa.`id_product`, pa.`id_product_attribute`, pa.`date_upd`')
                            ...
                            if (!empty($filter_supplier_id))
                            {
                                $sql->where('ps.`id_supplier` = '.(int) $filter_supplier_id);
                            }
                            if ($where_shop_list)
                            {
                                $sql
                                    ->innerJoin('product_attribute_shop', 'pas', 'ps.`id_product` = pas.`id_product` AND pas.`id_shop` IN ('.pInSQL($where_shop_list).')')
                                    ->leftJoin('product_attribute', 'pa', 'pas.`id_product_attribute` = pa.`id_product_attribute`')
                                ;
                            }
                            else
                            {
                                $sql->innerJoin('product_attribute', 'pa', 'ps.`id_product_attribute` = pa.`id_product_attribute`');
                            }
                            $res = Db::getInstance()->executeS($sql);
                            ...
                            ## search ref into product_supplier table for product ?
                            $sql = (new DbQuery())
                            ...
                            {
                                $sql
                                    ->innerJoin('product_shop', 'pshop', 'ps.`id_product` = pshop.`id_product` AND pshop.`id_shop` IN ('.pInSQL($where_shop_list).')')
                                    ->leftJoin('product', 'p', 'pshop.`id_product` = p.`id_product`')
                                ;
                            }
                            else
                            {
                        ...
                        ## search ref into product_attribute table for product_attribute (default supplier) ?
                        $sql = (new DbQuery())
                            ->select('p.`id_product`, p.`id_supplier`')
                            ->select('pa.`id_product_attribute`, pa.`date_upd`')
                            ->from('product', 'p')
                        ...
                        {
                            $sql
                                ->innerJoin('product_shop', 'pshop', 'p.`id_product` = pshop.`id_product` AND pshop.`id_shop` IN ('.pInSQL($where_shop_list).')')
                                ->leftJoin('product_attribute', 'pa', 'pshop.`id_product` = pa.`id_product`')
                            ;
                        }
                        else
                        {
                        ...
                        ## search ref into product table for product_attribute (default supplier) ?
                        $sql = (new DbQuery())
                            ->select('p.`id_product`, p.`id_supplier`, p.`date_upd`')
                            ->from('product', 'p')
                            ->where('p.`supplier_reference` = "'.pSQL($search_ref).'"')
                            ->limit(1)
                        ;
                        if (!empty($filter_supplier_id))
                        {
                        ...
                        if ($where_shop_list)
                        {
                            $sql->innerJoin('product_shop', 'pshop', 'p.`id_product` = pshop.`id_product` AND pshop.`id_shop` IN ('.pInSQL($where_shop_list).')');
                        }
                        $res = Db::getInstance()->executeS($sql);
                    break;
                    case 'prodrefandsupref':
                                ...
                                if ($refsupp_option != 'suppref_product')
                                {
                                    $sql = 'SELECT ps.id_product,ps.id_product_attribute,pa.date_upd,p.id_supplier
                                            ...
                                            WHERE pa.reference='".pSQL($search_reference)."'
                                                ...
                                                ".(empty($filter_supplier_id) ? '' : ' AND p.id_supplier='.$filter_supplier_id).' '.
                                                (!empty($where_shop_list) ? ' AND EXISTS (SELECT * FROM '._DB_PREFIX_.'product_shop ps WHERE pa.id_product=ps.id_product AND id_shop IN ('.pSQL($where_shop_list).'))' : '').
                                            ' LIMIT 1';
                                    $res = Db::getInstance()->executeS($sql);
                                    if (count($res) == 0)
                            ...
                            if ($has_view)
                            {
                                ...
                                $sql = 'SELECT ii.id_product,ii.id_product_attribute,ii.date_upd,ii.id_supplier FROM '._DB_PREFIX_.'sc_import_index ii
                                    ...
                                    WHERE
                                        ...
                                        ".(empty($filter_supplier_id) ? '' : ' AND ii.id_supplier='.$filter_supplier_id).' '.
                ...
                if (!empty($id_product))
                {
                    ...
                    elseif ($importConfig[$TODOfilename]['forfoundproduct'] == 'update')
                    {
                        if (SCMS)
                        {
                            ...
                            if ($id_shop == null)
                            {
                                $id_shop_list = [$newprod->id_shop_default];
                            }
                        }
                        else
                        {
                    ...
                    elseif ($importConfig[$TODOfilename]['forfoundproduct'] == 'create')
                    {
                        ...
                        elseif (!isset($duplicateCache[$id_product]))
                        {
                            ...
                            $duplicateCache[$id_product] = $newprod->id;
                            $id_product = $newprod->id;
                            ++$stats['created'];
                        }
                        else
                        {
                            $id_product = $duplicateCache[$id_product];
                        }
                    }
                }
                // NOUVEAU PRODUIT
                ...
                if ($newprod->id && _s('CAT_IMPORT_FORCE_PROD_PRICE_TO_FIRST_COMBI') == 1)
                {
                    ...
                    $newprod->unit_price = round($newprod->unit_price_ratio != 0 ? $newprod->price / $newprod->unit_price_ratio : 0, 6);
                }
                if ($scdebug)
                {
                    echo 'b';
                ...
                if (SCMS)
                {
                    ...
                    {
                        $newprod->id_shop_list = [$newprod->id_shop_list];
                    }
                }

                // CACHE VAT
                $cache_VAT = ['id_tax' => 0, 'rate' => 1, 'id_tax_rules_group' => 0];
                if (sc_in_array('VAT', $mappingData['DBArray'], 'catWinImportProcess_DBArray'))
                {
                    ...
                    if (!empty($vat_string) || (empty($vat_string) && $vat_string == '0') || !empty($vat_number))
                    {
                        ...
                        {
                            exit(_l('Error: tax doesn\'t exist: ').$value);
                        }
                                    ...
                                    case 'ActionDeleteAllCombinations':
                                        if (is_null($cache_id_shop[$id_product.'-'.$id_product_attribute]))
                                        {
                                            ...
                                            if (!empty($attributes))
                                            {
                                                ...
                                                $TODO_actions[] = 'DELETE FROM `'._DB_PREFIX_.'specific_price` WHERE `id_product_attribute` NOT IN (SELECT id_product_attribute FROM `'._DB_PREFIX_.'product_attribute` WHERE `id_product` = '.(int) $newprod->id.')';
                    ...
                    if (sc_array_key_exists($key, $firstLineData) && sc_in_array($firstLineData[$key], $mappingData['CSVArray'], 'catWinImportProcess_CSVArray'))
                    {
                        ...
                        switch ($switchObject)
                        {
                            ...
                            case 'visibility':
                                ...
                                if (array_key_exists($value, $dataDB_visibilityByName))
                                {
                                    $newprod->visibility = $dataDB_visibilityByName[$value];
                                }
                                elseif (array_key_exists($value, $dataDB_visibility))
                                {
                                    $newprod->visibility = $dataDB_visibility[$value];
                                }
                                else
                                {
                                    $newprod->visibility = 'both';
                                }
                                break;
                            case 'manufacturer':
                            ...
                            case 'supplier_reference':
                                ...
                                if ((!empty($id_supplier) && $id_supplier == 'product') || empty($id_supplier))
                                {
                                    if (isCombination($line) || isCombinationWithID())
                                    {
                            ...
                            case 'reduction_price':
                                if ($value != '')
                                {
                                    ...
                                    {
                                        $setReductionTax = '';
                                        if (!isset($mappingData['CSV2DB']['reduction_tax']))
                                        {
                ...
                if ($is_adv_pack_import)
                {
                    $newprod->id_shop_list = [$newprod->id_shop_default];
                }
                ...
                if (empty($newprod->id))
                {
                    ...
                    $product_action_result = $newprod->add();
                    ## pour éviter tout doublon superflu
                    $duplicateCache[$newprod->id] = $newprod->id;
                    if (_s('APP_DEBUG_CATALOG_IMPORT'))
                    {
                        $time_end = microtime(true);
                        $time = $time_end - $time_start;
                ...
                if ($product_action_result)
                {
                    ...
                    $attributesList = [];
                    $imagesListFromCSVLine = []; // contains images to link to the current combination
                    $imagesListFromDB = []; // contains images of current product
                        ...
                        if (sc_array_key_exists($key, $firstLineData) && sc_in_array($firstLineData[$key], $mappingData['CSVArray'], 'catWinImportProcess_CSVArray'))
                        {
                            ...
                            switch ($switchObject)
                            {
                                ...
                                case 'supplier_default':
                                    ...
                                    break;
                                case 'out_of_stock':
                                    $arr = [_l('Deny orders') => 0, _l('Allow orders') => 1, _l('Default(Pref)') => 2];
                                ...
                                case 'advanced_stock_management':
                                    ...
                                    {
                                        $type = (int) $value;
                                        if (!sc_in_array($type, [1, 2, 3], 'catWinImportProcess_asmtypes') && empty($id_product))
                                        {// si 0 et création
                                            $type = _s('CAT_ADVANCEDSTOCK_DEFAULT');
                                        }
                                ...
                                case 'wholesale_price':
                                    ...
                                    if (isCombinationWithID())
                                    {
                                        if (!empty($id_supplier) && ((!empty($id_supplier) && $id_supplier == 'product') || empty($id_supplier)))
                                        {
                                            $TODO_SET[] = "wholesale_price='".importConv2Float($value)."'";
                                            $TODO_SET_SHOP[] = "wholesale_price='".importConv2Float($value)."'";
                                        }
                                        elseif (!empty($id_supplier) && $id_supplier != 'product')
                                        {
                                    ...
                                    if (!isCombination($line) && !isCombinationWithID() && $id_product_attribute == 0)
                                    {
                                ...
                                case 'name':
                                    if (isCombination($line) || isCombinationWithID())
                                    {
                                        ...
                                        if (is_array($id_shop_list) && count($id_shop_list))
                                        {
                                ...
                                case 'specific_price_from_quantity':
                                    if ($value != '')
                                    {
                                        if ($newprod->id)
                                        {
                                            ...
                                            if (findCSVLineValue('specific_price_price') == '')
                                            {
                                            ...
                                            foreach ($specific_price_id_shop as $sp_shop_id)
                                            {
                                                if (!SCI::checkSpecificPriceexists((int) $newprod->id, $id_product_attribute, $specific_price_price, $specific_price_reduction, $specific_price_reduction_type, $sp_shop_id, $specific_price_id_group, $specific_price_id_country, $specific_price_id_currency, $specific_price_id_customer, (_s('APP_COMPAT_MODULE_PPE') ? floatval($specific_price_from_quantity) : (int) $specific_price_from_quantity), !$specific_price_from ? '0000-00-00 00:00:00' : str_replace('/', '-', $specific_price_from), !$specific_price_to ? '0000-00-00 00:00:00' : str_replace('/', '-', $specific_price_to)))
                                                {
                                                    ...
                                                    $specific_price_reduction_tax = (int) (in_array('specific_price_reduction_tax', $mappingData['CSV2DB']) ? findCSVLineValue('specific_price_reduction_tax') : _s('CAT_PROD_SPECIFIC_PRICES_DEFAULT_TAX'));
                                ...
                                case 'customization_field_type':
                                    ...
                                    if (!empty($value))
                                    {
                                        ...
                                        if ($newprod->id)
                                        {
                                            if (!sc_in_array($cfrequired, [0, 1], 'catWinImportProcess_cfrequired'))
                                            {
                                                $cfrequired = 0;
                                            }
                                            if (!sc_in_array($value, [0, 1], 'catWinImportProcess_cfvalue'))
                                            {
                                                $cfvalue = 0;
                                            }
                                ...
                                case 'product_advanced_pack':
                                    ...
                                    if (!empty($value))
                                    {
                                        ...
                                        $advp_sql = $advp_insert_products_sql = $advp_rows_products = $advp_rows_attributes = $advp_ipa_by_ip = [];
                                        ...
                                        foreach ($advp_isl as $r_id_shop)
                                        {
                                            ...
                                            if (!empty($advp_insert_shop_sql))
                                            {
                                                ...
                                                {
                                                    $advp_TODO_after['create'][(int) $r_id_shop][(int) $newprod->id] = [
                                                        'sql' => $advp_sql_string,
...
//                                 $default, $location = null, $upc = null, $minimal_quantity = 1, array $id_shop_list = array())
                            ...
                            {
                                SCI::hookExec('updateProductAttribute', ['id_product_attribute' => (int) $id_product_attribute, 'product' => $newprod]);
                            }
                        }
                        if (!fieldInMapping('attribute_default_on'))
                        {
                            ...
                            if ($has_default_attribute_in_shop)
                            {
                    ...
                    {
                        SpecificPriceRule::applyAllRules([(int) $newprod->id]);
                    }
                            ...
                            if (sc_array_key_exists($key, $firstLineData) && sc_in_array($firstLineData[$key], $mappingData['CSVArray'], 'catWinImportProcess_CSVArray'))
                            {
                                ...
                                switch ($switchObject) {
                                    case 'supplier_reference':
                                        ...
                                        $id_supplier = (str_replace('suppref_', '', $mappingData['CSV2DBOptions'][$firstLineData[$key]]));
                                        if (!empty($id_supplier) && $id_supplier != 'product')
                                        {
                                            $id_supplier = (int) str_replace('supp_', '', $id_supplier);
                                            if (!empty($id_supplier))
                                            {
                                    ...
                                    case 'attribute_default_on':
                                        if (isCombinationWithID() && !empty($id_product_attribute) && getBoolean($value))
                                        {
                                            if ($value)
                                            {
                                                ...
                                                $ado_sql[] = 'UPDATE '._DB_PREFIX_.'product_attribute_shop SET default_on=NULL WHERE id_shop IN ('.implode(',', $id_shop_list).') AND id_product = '.(int) $id_product;
                    ...
                    if (SCMS && $importConfig[$TODOfilename]['forfoundproduct'] != 'skip' && ((!sc_array_key_exists($id_product.'-'.$id_product_attribute, $cache_id_shop) && count($id_shop_list_in_csv) > 1) || (!empty($cache_id_shop[$id_product.'-'.$id_product_attribute]) && count($cache_id_shop[$id_product.'-'.$id_product_attribute]) < count($id_shop_list_in_csv) - 1)))
                    {
                        //mise en cache
                    ...
                    {
                        $cache_id_shop[$id_product.'-'.$id_product_attribute] = [];
                        unset($CSVData[$current_line]);
                            ...
                            foreach ($advp_todo_row as $advp_id_shop => $advp_create_list)
                            {
                                ...
                                foreach ($advp_create_list as $advp_id_base_product => $advp_create_param)
                                {
                                    ...
                                    if (array_key_exists('need_create_product', $advp_create_param))
                                    {
                                        ...
                                        $advp_new_product->id_shop_list = [$advp_id_shop];
            ...
            $stats['total_stats'] = (int) $stats['created'] + (int) $stats['modified'] + (int) $stats['skipped'];
            ...
            if ((count($CSVData) == 1) || (count($CSVData) == 2 && $CSVData[0] == join('', $CSVData)) || (filesize(SC_CSV_IMPORT_DIR.$TODOfilename) == 0))
            {
                ...
                echo '<script>window.top.displayOptions();window.top.stopAutoImport(true);</script>';
                $msg2 = 'All products have been imported.';
            }
            else
            {
                if ((int) $stats['total_stats'] === 0)
                {
                    echo _l('Your file contains line breaks')._l(':').'<br>';
                    echo '<a href="'.getScExternalLink('support_csv_import_3_0').'" target="_blank"><i class="fa fa-link"></i> '._l('See corresponding article').'</a>';
                }
                else
                {
                    echo _l('Some lines of your CSV file have been ignored for one the reasons explained in this article')._l(':').'<br>';
                    echo '<a href="'.getScExternalLink('support_csv_import_ignored_lines').'" target="_blank"><i class="fa fa-link"></i> '._l('See corresponding article').'</a>';
                }
                echo '<br/><br/>';
                ...
                echo '<script>window.top.displayOptions();autoImportUnit=999999;window.top.prepareNextStep('.($stats['created'] + $stats['modified'] + $stats['skipped'] == 0 ? 0 : filesize(SC_CSV_IMPORT_DIR.$TODOfilename)).');</script>';
