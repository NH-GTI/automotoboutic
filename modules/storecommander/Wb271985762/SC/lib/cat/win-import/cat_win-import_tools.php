...
if (!defined('STORE_COMMANDER'))
{
    ...
    {
        if (sc_in_array(Tools::strtoupper($value), ['1', 'YES', 'TRUE', 'VRAI', 'OUI', 'ON'], 'catWinImportProcess_getboolean'))
        {
            return true;
        }
            ...
            else
            {
                if (strpos($categList, '>') != false)
                {
                    ...
                    foreach ($categ as $k => $c)
                    {
                        ...
                        if (!sc_array_key_exists(forceCategoryPathFormat($pathSliced), $categoryIDByPath))
                        {
                            ...
                            $categories[trim($c)] = ['id_category' => $newCateg->id, 'id_parent' => $id_parent];
                            $categoryNameByID[$newCateg->id] = $c;
                            $categoriesProperties[$newCateg->id] = ['id_category' => $newCateg->id, 'id_parent' => $id_parent];
                ...
                else
                {
                    ...
                    if (!sc_in_array($categList, $categoriesFirstLevel, 'catWinImportProcess_categoriesFirstLevel'))
                    {
                        ...
                        $categories[$categList] = ['id_category' => $newCateg->id, 'id_parent' => 1];
                        $categoryNameByID[$newCateg->id] = $categList;
                        $categoriesProperties[$newCateg->id] = ['id_category' => $newCateg->id, 'id_parent' => 1];
                        $categoryIDByPath[getCategoryPath($newCateg->id)] = $newCateg->id;
        ...
        foreach ($line as $k => $v)
        {
            if (sc_in_array($firstLineData[$k], $mappingData['CSVArray'], 'catWinImportProcess_CSVArray') && sc_array_key_exists($firstLineData[$k], $mappingData['CSV2DB']) && $mappingData['CSV2DB'][$firstLineData[$k]] == $valueToFind)
            {
                if ($valueToFind == 'attribute_multiple')
                {
                    ...
                    {
                        @$arrayToFill[] = ['object' => $firstLineData[$k],
                                'value' => trim($val),
    ...
    function copyImg($id_entity, $id_image, $url, $entity = 'products')
    {
        ...
        if (isset($parsed_url['scheme']) && in_array($parsed_url['scheme'], ['http', 'https']))
        {
            ...
            if ($headers != false && !in_array($code_header, [200, 301, 302]))
            {
                return false;
            }
        }
        ...
        if ($copy)
        {
            ...
            {
                SCI::hookExec('watermark', ['id_image' => $id_image, 'id_product' => $id_entity]);
            }
        }
        else
        {
    ...
    */
            ...
            if ($featureValue != '')
            {
                ...
                if ($fv[0] != '' && $fv[1] != '')
                {
                    ...
                    if ($fv[1] != '')
                    {
                        ...
                        $size = (array_key_exists('size', FeatureValue::$definition['fields']['value']) ? (int) FeatureValue::$definition['fields']['value']['size'] : 255);
    ...
    function addFeature($data)
    {
        ...
        foreach ($features as $feature)
        {
            if ($feature != '')
            {
                ...
                $size = (array_key_exists('size', Feature::$definition['fields']['name']) ? (int) Feature::$definition['fields']['name']['size'] : 128);
    ...
    function isCombination($lineToCheck = null)
    {
        ...
        if ($id_product_attribute == 0 && (sc_in_array('attribute', $mappingData['DBArray'], 'catWinImportProcess_DBArray') || sc_in_array('attribute_multiple', $mappingData['DBArray'], 'catWinImportProcess_DBArray')))
        {
            ...
            else
            {
                ...
                {
                    if (sc_array_key_exists($firstLineData[$key], $mappingData['CSV2DB']) && sc_in_array($mappingData['CSV2DB'][$firstLineData[$key]], ['attribute', 'attribute_multiple'], 'catWinImportProcess_attrfields') && $val != '')
                    {
                        return true;
                    }
                }
    ...
    function isCombinationWithID()
    {
        ...
        if ((sc_in_array('id_product_attribute', $mappingData['DBArray'], 'catWinImportProcess_DBArray') && findCSVLineValue('id_product_attribute') > 0) || $id_product_attribute > 0)
        {
            return true;
        }
    ...
    function loadMapping($filename, $toString = false)
    {
        global $sc_agent;
        if ($filename == '')
        {
        ...
        if (file_exists(SC_CSV_IMPORT_DIR.$filename) && $feed = simplexml_load_file(SC_CSV_IMPORT_DIR.$filename))
        {
            ...
            {
                $option = '';
                if (!sc_in_array(trim((string) $map->dbname), ['attribute', 'attribute_multiple'], 'catWinImportProcess_attrfields')
                        || (sc_in_array(trim((string) $map->dbname), ['attribute', 'attribute_multiple'], 'catWinImportProcess_attrfields') && sc_in_array(trim((string) $map->options), $groupsName, 'catWinImportProcess_groupsName'))
                        || (SCAS && ($map->dbname == 'quantity' || $map->dbname == 'location')))
                {
                    $option = trim((string) $map->options);
                }
                $content[] = [
                    trim((string) $map->csvname),
                    trim((string) $map->dbname),
                    $option
                ];
            }
        }
    ...
    function refreshCacheCategory()
    {
        ...
        foreach ($res as $categ)
        {
            ...
            $categories[trim($categ['name'])] = ['id_category' => $categ['id_category'], 'id_parent' => $categ['id_parent']];
            $categoryNameByID[$categ['id_category']] = $categ['name'];
            $categoriesProperties[$categ['id_category']] = ['id_category' => $categ['id_category'], 'id_parent' => $categ['id_parent']];
    ...
    function getReadableError($exception)
    {
