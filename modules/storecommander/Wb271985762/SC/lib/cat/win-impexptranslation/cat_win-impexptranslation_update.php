...
$content = rawurldecode(Tools::getValue('content'));
        ...
        switch ($tab) {
            ...
            case 'attribute_value':
                ...
                if ($temp_table_creaded)
                {
                    ...
                    //missing translations to insert
                    $create_missing_attributes_lang_update = createMissingValueWithName();

                    if ($create_missing_attributes_lang_update && $final_update)
                    {
                        Db::getInstance()->Execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'temp_translate_attribute_value');
...
function createMissingValueWithName()
{
    $return = true;
    $missing_attributes_lang = Db::getInstance()->executeS('SELECT tmp.* FROM '._DB_PREFIX_.'temp_translate_attribute_value tmp 
LEFT JOIN '._DB_PREFIX_.'attribute_lang al ON tmp.id_attribute = al.id_attribute AND tmp.id_lang = al.id_lang
WHERE al.id_attribute IS NULL AND tmp.name != ""');

    if(!empty($missing_attributes_lang)){
        foreach($missing_attributes_lang as $missing_attribute_lang) {
            $create_missing_attribute_lang_sql = 'INSERT INTO '._DB_PREFIX_.'attribute_lang (`id_attribute`,`id_lang`,`name`)
                    VALUES ('.pSQL($missing_attribute_lang["id_attribute"]).','.pSQL($missing_attribute_lang["id_lang"]).',\''.pSQL($missing_attribute_lang["name"]).'\')';
            $insert =  Db::getInstance()->execute($create_missing_attribute_lang_sql);
            if(!$insert){
                $return = false;
            }
        }
