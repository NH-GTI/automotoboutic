...
function getRowsFromDB()
{
    ...
    foreach ($combArray as $id_product_attribute => $product_attribute)
    {
        ...
        if (Sc\Service\Service::exists('shippingbo')) {
            echo '<userdata name="id_sbo">' . $product_attribute['id_sbo'] . '</userdata>' . "\n";
            echo '<userdata name="type_sbo">' . $product_attribute['type_sbo'] . '</userdata>' . "\n";
            echo '<userdata name="id_sbo_source">' . $product_attribute['id_sbo_source'] . '</userdata>' . "\n";
        }
        ...
        foreach ($sourcecols as $key => $col)
        {
            switch ($col) {
                ...
                case 'quantity_real':
                    ...
                    $value = SCI::getProductRealQuantities(
                        (int) $id_product,
...
function getColSettingsAsXML()
{
    ...
    $pa_cols = [];
    $excludedFields = ['ATTR', 'quantityupdate', 'pprice', 'ppriceextax', 'priceextax', 'margin', 'pweight', 'quantity_physical', 'quantity_usable', 'quantity_real', 'available_later', 'unit_price_impact_inc_tax', 'location_new', 'supplier_reference', 'position', 'id_sbo', 'type_sbo', 'id_sbo_source', 'out_of_stock'];
    ...
    $sql = '
        SELECT pa.id_product_attribute,pa.id_product'.(version_compare(_PS_VERSION_, '1.5.0.1', '>=') ? ', a.position, ag.position as group_position' : '').((!empty($pa_cols)) ? ',pa.`'.implode(
        '`,pa.`',
        $pa_cols
    ) : '').'`, ag.`id_attribute_group`, agl.`name` AS group_name, al.`name` AS attribute_name, a.`id_attribute`, p.`price` AS pprice, p.`weight` AS pweight, p.`ecotax` AS pecotax, sa.`out_of_stock`'.
    ...
    (version_compare(_PS_VERSION_, '1.5.0.0', '>=') && !empty($shop_id) ? ',IF(pa_shop.wholesale_price IS NULL or pa_shop.wholesale_price = 0, IF(ps.product_supplier_price_te IS NULL,0,ps.product_supplier_price_te), pa_shop.wholesale_price) AS wholesale_price,pa_shop.price,pa_shop.ecotax,pa_shop.weight,pa_shop.unit_price_impact,pa_shop.minimal_quantity,pa_shop.default_on,pa_shop.available_date' : '').
    ...
    if (Sc\Service\Service::exists('shippingbo'))
    {
        $sql .= ',ps_relation.id_sbo as id_sbo, ps_relation.type_sbo as type_sbo, ps_relation.id_sbo_source as id_sbo_source';
    }
    ...
    */
    ...
    if (Sc\Service\Service::exists('shippingbo'))
    {
        ...
        $sql .= ' LEFT JOIN `'._DB_PREFIX_.$shippingboService->getShopRelationRepository()->sboShopRelationTable.'` ps_relation ON (ps_relation.`id_product_attribute` = pa.`id_product_attribute`) ';
    }
    ...
    $multiStoresFields = ['wholesale_price', 'price', 'ecotax', 'weight', 'unit_price_impact', 'minimal_quantity', 'default_on', 'available_date'];
    $combArray = [];
    foreach ($results as $combinaison)
    {
        ...
        $combArray[$combinaison['id_product_attribute']]['out_of_stock'] = (int) $combinaison['out_of_stock'];
        if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
        {
        ...
        if (Sc\Service\Service::exists('shippingbo'))
        {
            $combArray[$combinaison['id_product_attribute']]['id_sbo'] = $combinaison['id_sbo'];
            $combArray[$combinaison['id_product_attribute']]['type_sbo'] = $combinaison['type_sbo'];
            $combArray[$combinaison['id_product_attribute']]['id_sbo_source'] = $combinaison['id_sbo_source'];
        }
        ...
        if (!empty($combinaison['id_attribute_group']))
        {
            if (version_compare(_PS_VERSION_, '1.5.0.1', '>='))
            {
                $combArray[$combinaison['id_product_attribute']]['attributes'][] = [$combinaison['group_name'], $combinaison['attribute_name'], $combinaison['id_attribute'], $combinaison['id_attribute_group'], $combinaison['group_position'].$combinaison['position']];
            }
            else
            {
                $combArray[$combinaison['id_product_attribute']]['attributes'][] = [$combinaison['group_name'], $combinaison['attribute_name'], $combinaison['id_attribute'], $combinaison['id_attribute_group']];
            }
        }
    ...
    for ($i = 0; $i < $groups; ++$i)
    {
        if (!_s('CAT_PROD_COMBI_METHOD'))
        {
            ...
            $sql = 'SELECT *
                        ...
                        LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = '.(int) $id_lang.')'.
                    (version_compare(_PS_VERSION_, '1.5.0.0', '>=') && !empty($shop_id) ? 'LEFT JOIN `'._DB_PREFIX_.'attribute_shop` ats ON (a.`id_attribute` = ats.`id_attribute` AND ats.id_shop = "'.$shop_id.'")' : '').'
                        WHERE a.`id_attribute_group` = '.(int) $attributeGroupsIDs[$i].'
                        ORDER BY '.(version_compare(_PS_VERSION_, '1.5.0.1', '>=') ? 'a.position' : 'al.name');
            ...
            $colSettings['attr_'.$i.'_'.$attributeGroupsIDs[$i]] = ['text' => $name_group, 'width' => 90, 'align' => 'left', 'type' => 'coro', 'sort' => 'str', 'color' => '', 'filter' => '#text_filter', 'options' => $options];
        }
        else
        {
            $name_group = $attributeGroupsNames[$i];
            $name_group = str_replace('&', _l('and'), $name_group);
            $colSettings['attr_'.$i.'_'.$attributeGroupsIDs[$i]] = ['text' => $name_group, 'width' => 90, 'align' => 'left', 'type' => 'ro', 'sort' => 'str', 'color' => '', 'filter' => '#text_filter'];
        }
...
echo '<userdata name="productecotax">'.number_format($productecotax, 6, '.', '').'</userdata>'."\n";
