...
var alertMassCombinationTreshold = 20;

<?php if (Combination::isFeatureActive() && _r('GRI_CAT_PROPERTIES_GRID_COMBI')) { ?>
...
getTbSettingsButton(prop_tb, {'grideditor':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':0}, 'prop_combi_',1000);
    ...
    { ?>
        ...
        function onGridCombiContextButtonClick(itemId){
            ...
            if (itemId=="remove_sbo_relation"){
                selection=prop_tb._combinationsGrid.getSelectedRowId();
                idxIdSbo=prop_tb._combinationsGrid.getColIndexById('id_sbo');
                ...
                if (confirm(confirmMessage)) {
                    if (selection!='' && selection!=null)
                    {
                        selArray=selection.split(',');
                        for(i=0 ; i < selArray.length ; i++) {
                            onEditCellCombi(2, selArray[i], idxIdSbo, null, prop_tb._combinationsGrid.cells(selArray[i],idxIdSbo).getValue());
                            prop_tb._combinationsGrid.cells(selArray[i],idxIdSbo).setValue(null)
                        }
                    }
                }
            }
            <?php if(Sc\Service\Service::exists('shippingbo')) { ?>
                if (itemId=="open_in_shippingbo"){
                    ...
                    let idSbo = prop_tb._combinationsGrid.getUserData(lastCombiSelID, "id_sbo");
                    if(prop_tb._combinationsGrid.getUserData(lastProductSelID, "type_sbo") === '<?php echo Sc\Service\Shippingbo\Shippingbo::SBO_PRODUCT_TYPE_BATCH; ?>'){
                        sboLinkPattern = '<?php echo Sc\Service\Shippingbo\Shippingbo::LINK_BATCH_URL_PATTERN; ?>';
                        idSbo = prop_tb._combinationsGrid.getUserData(lastCombiSelID, "id_sbo_source");
                    } else if(prop_tb._combinationsGrid.getUserData(lastProductSelID, "type_sbo") === '<?php echo Sc\Service\Shippingbo\Shippingbo::SBO_PRODUCT_TYPE_PACK; ?>'){
...
<?php
if (_r('MEN_CAT_ATTRIBUTES_GROUPS'))
    {
        ?>
...
function onEditCellCombi(stage,rId,cInd,nValue,oValue){
    ...
    if (nValue!=oValue){
        ...
        let need_global_ecotax = <?php echo version_compare(_PS_VERSION_, '1.7.7.6', '>=')? 'true' : 'false'; ?>;
...
<?php
    ...
    else
    {
        ...
        if (getCombinationsNum()==0 && id=='combi_add')
        {
            ...
            if(nb > alertMassCombinationTreshold){
                let confir_combi_mass_create_message ='<?php echo _l("Are you sure you want to create {nb} combinations ?",1); ?>';
                let confirm_combi_mass_create = confirm(confir_combi_mass_create_message.replace('{nb}', nb));
                if(!confirm_combi_mass_create){
                    return;
                }
            }
...
function colorExistCombi()
{
    <?php if (_s('CAT_COLOR_SAME_COMBI')) { ?>
        ...
        var nbCols = prop_tb._combinationsGrid.getColumnsNum();
        if(prop_tb._combinationsGrid.getRowsNum() <= 0)
        {
            return;
        }
        let combinationGridAllIds = prop_tb._combinationsGrid.getAllRowIds().split(',');
        for(const rid of combinationGridAllIds){
