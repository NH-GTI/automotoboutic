...
$('document').ready(function(){
    ...
    if (sc_qs_filter === false) {
        ...
        const TopSearchQuery = $("#search_query");
        TopSearchQuery.on( "dblclick", function() {
            $(this).autocomplete('search');
        });
        const TopSearchCache = {};
        TopSearchQuery.autocomplete({
            minLength: 1,
            appendTo: "#search_innner",
            source: function (request, response) {
                sc_qs_filter['q'] = request.term;
                sc_qs_filter['shopselection'] = shopselection;

                const TopSearchKey = JSON.stringify(sc_qs_filter);

                if (TopSearchKey in TopSearchCache) {
                    response(TopSearchCache[TopSearchKey]);
                    return;
                }

                $.ajax({
                    type: 'POST',
                    url: 'index.php?ajax=1&act=cat_quicksearch_get',
                    dataType: "json",
                    data: sc_qs_filter,
                    success: function (data) {
                        ...
                        TopSearchCache[TopSearchKey] = formattedData;
            ...
            select: function (event, ui) {
                ...
                if (typeof data!='undefined' && Boolean(data.found))
                {
                    ...
                    if(enalbeDynamicTreeLoading) {
                        catselection_path = data.cat_parent_list;
                        openCategoryTreeDynamicLoading();
                    }
                    lastProductSelID = data.id_product;
                    displayProducts('id_product_attributeToSelect='+Number(data.id_product_attribute)+';lastProductSelID;idxProductID=cat_grid.getColIndexById("id");oldFilters["id"]="'+data.id_product+'";cat_grid.getFilterElement(idxProductID).value="'+data.id_product+'";cat_grid.filterByAll();cat_grid.selectRowById('+data.id_product+',false,true,true);');
                }
                return false;
            }
        });
