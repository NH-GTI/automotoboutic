...
function generateValue($col, $row, $p, $pa = null, $params = [], $row_color = null)
{
    ...
    switch ($col){
        ...
        case 'manufacturer':
            ...
            break;
        case 'supplier':
            $return .= '<cell><![CDATA['.(!empty($arrSuppliers[$p->id_supplier]) ? $arrSuppliers[$p->id_supplier] : '').']]></cell>';
            break;
        case 'price':
            $return .= '<cell>'.($row['price'] != -1 ? number_format($row['price'], 2) : '-1').'</cell>';
            break;
...
function getXml($id_category)
{
    ...
    if (!empty($id_category))
    {
        ...
        $sql = '
            SELECT sp.*, p.reference, p.on_sale '.(SCMS ? ',ps.on_sale' : '').(SCMS ? ' ,ps.id_shop as product_id_shop' : ' ,'.$id_shop.' as product_id_shop').' ';
        ...
        foreach ($res as $specific_price)
        {
            ...
            $params['price_with_reduction_tax_excl'] = $item_price * 1;
            $params['price_with_reduction_tax_incl'] = $item_price * ($tax[(int) $product->id_tax_rules_group] / 100 + 1) * (_s('CAT_PROD_ECOTAXINCLUDED') && $product->ecotax > 0 ? $product->ecotax * $ecotax_tax_rate : 1);
