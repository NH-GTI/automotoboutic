...
if (_r('GRI_CAT_PROPERTIES_GRID_ATTACHEMENT')) { ?>
    ...
    if (needInitAttachments)
    {
        ...
        function onEditCellAttachments(stage,rId,cInd,nValue,oValue){
            ...
            if (cName.substr(0,4) == 'name'){
                if (stage==2)
                    $.post("index.php?ajax=1&act=cat_attachment_update&action=updateName", {attachment_list:rId, [cName]:nValue}, function(data){});
            }
...
function setPropertiesPanel_attachments(id){
    ...
    if (id=='attachment_add'){
        if (!dhxWins.isWindow("wCatAddAttachment"))
        {
            ...
            ll.cells('a').attachURL("index.php?ajax=1&act=cat_attachment_upload&id_lang="+SC_ID_LANG+"&"+new Date().getTime(), null, {product_list:cat_grid.getSelectedRowId()});
    ...
    if (id=='attachment_edit')
    {
        ...
        let idxFilename=attachment_grid.getColIndexById('file_name');
        let name = attachment_grid.cells(attachment_grid.getSelectedRowId(),idxFilename).getValue();
        prop_tb._attachmentsLayout.cells('b').setText('<?php echo _l('Edit document file', 1); ?> "'+name+'"');
        prop_tb._attachmentsLayout.cells('b').attachURL("index.php?ajax=1&act=cat_attachment_upload_file", null, {
            id_attachment : ids,
            id_lang : SC_ID_LANG
        });
    }
