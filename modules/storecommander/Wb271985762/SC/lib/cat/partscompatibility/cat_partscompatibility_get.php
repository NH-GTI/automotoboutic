...
function getCompatibilities($all_compatibilities, $multiple)
{
    foreach ($all_compatibilities as $id_compat => $compat)
    {
        ...
        $return .= '<cell><![CDATA['.$compat['active'].']]></cell>';
...
<rows>
    <head>
        <beforeInit>
            <call command="attachHeader"><param><![CDATA[#numeric_filter,<?php if ($multiple) echo '#text_filter,' ?>#select_filter,#select_filter,#select_filter,#text_filter,#select_filter]]></param></call>
        </beforeInit>
        ...
        <column id="modele_is_active" width="80" type="coro" align="center" sort="int"><?php echo _l('Model active'); ?>
