...
?>
    ...
    <?php if (_r('GRI_CAT_VIEW_GRID_PACK') && version_compare(_PS_VERSION_, '1.6.1.12', '>=')) { ?>gridnames['grid_pack']='<?php echo _l('Pack', 1); ?>';<?php } ?>
    ...
    <?php if ((defined('SC_UkooParts_ACTIVE') && SC_UkooParts_ACTIVE == 1) && SCI::moduleIsInstalled('ukooparts') && _r('GRI_CAT_ENABLE_UKOO_PARTS_FEATURES')) { ?>
    gridnames['grid_everyparts']='<?php echo _l('EveryParts', 1); ?>';<?php } ?>
    <?php if (Sc\Service\Service::exists('shippingbo') && _r('GRI_CAT_VIEW_SHIPPINGBO')) { ?>
    gridnames['grid_shippingbo']='<?php echo _l('Shippingbo', 1); ?>';<?php } ?>
    <?php
    ...
    opts.push(['sep01', 'sep', '', '']);
    opts.push(['grideditor', 'obj', '<?php echo _l('Customize'); ?>']);
    <?php } ?>
    if (opts.length > 25)
    ...
    <?php $catGridLangId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'lang'); ?>
    ...
    <?php if (_r('ACT_CAT_DELETE_PRODUCT_COMBI')) { ?>
        <?php $catGridDeleteId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'delete'); ?>
        cat_grid_tb.addButton("<?php echo $catGridDeleteId; ?>",100, "", "fa fa-minus-circle red <?php echo $catGridDeleteId; ?>", "fa fa-minus-circle red");
        cat_grid_tb.setItemToolTip('<?php echo $catGridDeleteId; ?>','<?php echo _l('This will permanently delete the selected products everywhere in the shop.'); ?>');
    <?php } ?>
    ...
    <?php $catGridCopyToCategoryId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'copytocateg'); ?>
    cat_grid_tb.addButtonTwoState("<?php echo $catGridCopyToCategoryId; ?>",100, "", "fad fa-folder-plus green <?php echo $catGridCopyToCategoryId; ?>", "fad fa-folder-plus green");
    cat_grid_tb.setItemToolTip('<?php echo $catGridCopyToCategoryId; ?>','<?php echo _l('If enabled: link products in the target category when you drag and drop products. Not enabled: move products'); ?>');
    ...
    <?php if (_r('ACT_CAT_BARCODE')) { ?>
        <?php $catGridBarCodeId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'barcode'); ?>
        cat_grid_tb.addButton("<?php echo $catGridBarCodeId; ?>", 100, "", "fad fa-scanner yellow <?php echo $catGridBarCodeId; ?>", "fad fa-scanner yellow");
        cat_grid_tb.setItemToolTip('<?php echo $catGridBarCodeId; ?>','<?php echo _l('Update quantity with barcode'); ?>');
    <?php } ?>
    <?php if (_r('ACT_CAT_FAST_EXPORT')) { ?>
        <?php $catGridExportCsvId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'exportcsv'); ?>
        cat_grid_tb.addButton("<?php echo $catGridExportCsvId; ?>", 100, "", "fad fa-file-csv green <?php echo $catGridExportCsvId; ?>", "fad fa-file-csv green");
        cat_grid_tb.setItemToolTip('<?php echo $catGridExportCsvId; ?>','<?php echo _l('Export grid to clipboard in CSV format for MSExcel with tab delimiter.'); ?>');
    <?php } ?>

    <?php $catGridSetPositionId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'setposition'); ?>
    cat_grid_tb.addButton("<?php echo $catGridSetPositionId;?>", 100, "", "fa fa-list-ol green <?php echo $catGridSetPositionId;?>", "fa fa-list-ol grey");
    cat_grid_tb.setItemToolTip('<?php echo $catGridSetPositionId;?>','<?php echo _l('Save product positions in the grid as category positions'); ?>');

    <?php $catGridShufflepositionId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'shuffleposition'); ?>
    cat_grid_tb.addButton("<?php echo $catGridShufflepositionId; ?>", 100, "", "fa fa-random <?php echo $catGridShufflepositionId; ?>", "fa fa-random");
    cat_grid_tb.setItemToolTip('<?php echo $catGridShufflepositionId; ?>','<?php echo _l('Shuffle positions', 1); ?>');

    <?php $catGridPrintId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'print'); ?>
    cat_grid_tb.addButton("<?php echo $catGridPrintId; ?>", 100, "", "fad fa-print <?php echo $catGridPrintId; ?>", "fad fa-print");
    cat_grid_tb.setItemToolTip('<?php echo $catGridPrintId; ?>','<?php echo _l('Print grid', 1); ?>');

    <?php $catGridEditorId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'grideditor'); ?>
    getTbSettingsButton(cat_grid_tb, {'<?php echo $catGridEditorId; ?>':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':<?php echo (int)_r('MEN_TOO_SETTINGS'); ?>}, '',100);
    <?php if (SCAS) { ?>
    <?php $catGridHelpScasId = $sc_agent->getGrantedId(ScCredentials::TYPE_CAT_GRID_TOOLBAR, 'help_scas'); ?>
        cat_grid_tb.addButton("<?php echo $catGridHelpScasId;?>", 1200, "", "fad fa-info-circle blue <?php echo $catGridHelpScasId;?>", "fad fa-info-circle blue");
        cat_grid_tb.setItemToolTip('<?php echo $catGridHelpScasId;?>','<?php echo _l('Show help for color codes used for quantities/warehouses management', 1); ?>');
    <?php } ?>
    ...
    function gridToolBarOnClick(id){
            ...
            if (id=='grideditor'){
                ...
                <?php } else { ?>
                    window.open('<?php echo getScExternalLink('interface_customization');?>');
                <?php } ?>
            }
            if (id=='settings'){
                        ...
                        <?php if (version_compare(_PS_VERSION_, '1.7.0.0', '>='))
{
    ...
    cat_grid.attachEvent("onRowDblClicked", function(rId,cInd){
        ...
        if(Boolean(cat_grid.getUserData(rId, 'isAdvancedPack'))) {
            let advPackNotAllowedFieldToUpdate = [
                ...
                cat_grid.getColIndexById('unit_price_inc_tax')
            ];

            <?php if(Sc\Service\Service::exists('shippingbo')){ ?>
            advPackNotAllowedFieldToUpdate.push(cat_grid.getColIndexById('is_locked'));
            <?php } ?>

            if(advPackNotAllowedFieldToUpdate.includes(cInd)) {
    ...
    cat_grid.attachEvent("onBeforeContextMenu", function(rowid,colidx,grid)
    {
            ...
            <?php if(Sc\Service\Service::exists('shippingbo')){ ?>
                if(cat_grid.getUserData("", "is_sbo_segment")){
                    cat_cmenu.showItem('convert_to_combinations');
                } else {
                    cat_cmenu.hideItem('convert_to_combinations');
                }
                cat_cmenu.showItem('remove_sbo_relation');
                cat_cmenu.showItem('open_in_shippingbo');
                if(cat_grid.getUserData(rowid, "id_sbo") == ''){
                    cat_cmenu.setItemDisabled('open_in_shippingbo');
                } else {
                    cat_cmenu.setItemEnabled('open_in_shippingbo');
                }
            <?php } ?>
    ...
    function calculMarginProduct(rId, type_col)
    {
        ...
        else
        {
            if(cat_grid.getColIndexById(type_col)!=undefined && cat_grid.getColIndexById('wholesale_price')!=undefined)
            {
                ...
                if(!idxPriceIncTaxes || !idxPriceWithoutTaxes){
                    return;
                }
                idxWholeSalePrice=cat_grid.getColIndexById('wholesale_price');
    ...
    function onEditCell(stage,rId,cInd,nValue,oValue){
        ...
        if (nValue!=oValue){
            ...
            <?php } ?>
            <?php if(Sc\Service\Service::exists('shippingbo')) { ?>
                var idxSboSync=cat_grid.getColIndexById('is_locked');
                if (cInd == idxSboSync){
                    cat_grid.cells(rId,idxSboSync).setValue(nValue);
                }
            <?php } ?>
...
?>
            ...
            if (cInd == idxActive && idxProductName){//Active update
                if (nValue==0){
...
?>
    ...
    {
        this.editor.list.setAttribute('size', (this.editor.combo.size() > 15 ? 15 : this.editor.combo.size()));
        var pos = this.getPosition(this.editor.cell);
        var y = document.body.offsetHeight-pos[1];
        if(y < this.editor.list.offsetHeight)
            this.editor.list.style.top = (pos[1] - this.editor.list.offsetHeight)+'px';
    }
    }
    ...
    function onGridCatContextButtonClick(itemId){
        ...
        if (itemId=="goshop"){
            ...
            if (sel)
            {
                ...
                for (var i=0;i<tabId.length;i++) {
                    if (k > <?php echo _s('CAT_PROD_OPEN_URL'); ?>) {
                        break;
                    }

                    productArray.push({
                        id_product: Number(tabId[i]),
                        active: Number((idxActive ? Number(cat_grid.cells(tabId[i], idxActive).getValue()) : 0))
                    })
                    k++;
                }

                if(productArray.length > 0) {
                    $.post('index.php?ajax=1&act=cat_product_info', {
                        action: 'getProductUrl',
                        id_lang: SC_ID_LANG,
                        productList: JSON.stringify(productArray),
                    }, function(response) {
                        let previewUrlList = JSON.parse(response)
                        if(Boolean(previewUrlList) && previewUrlList.length > 0) {
                            for(const url of previewUrlList) {
                                window.open(url);
                            }
                        }
                    });
        ...
        if (itemId=="update_yes"){
            ...
            if (selection!='' && selection!=null)
            {
                ...
                for(i=0 ; i < selArray.length ; i++)
                {
                    let value = '1';
                    <?php if(Sc\Service\Service::exists('shippingbo')) { ?>
                    if (cat_grid.getColIndexById('is_locked') === lastColumnRightClicked){
                        value = '0';
                    }
                    <?php } ?>
                    cat_grid.cells(selArray[i],lastColumnRightClicked).setValue(value);
                    cat_grid.cells(selArray[i],lastColumnRightClicked).cell.wasChanged=true;
                    onEditCell(null,selArray[i],lastColumnRightClicked,value,null);
                }
            }
        }
        if (itemId=="update_no"){
        ...
        <?php if(Sc\Service\Service::exists('shippingbo')) { ?>
        if (itemId=="convert_to_combinations"){
            let selection=cat_grid.getSelectedRowId();
            if (selection!='' && selection!=null){
                if (!dhxWins.isWindow("wConvertProductToCombination"))
                {
                    wConvertProductToCombination = dhxWins.createWindow("wConvertProductToCombination", 20, 20, $(window).width()-40, $(window).height()-40);
                    wConvertProductToCombination.setText('<?php echo ucfirst(_l('convert to combinations', 1)); ?>');
                    let idShopParam = '';
                    if(shopselection != undefined && shopselection!=0){
                        idShopParam = '&id_shop=<?php echo SCI::getSelectedShop() ?>';
                    }
                    wConvertProductToCombination.attachURL("index.php?ajax=1&act=cat_win-convertproducttocombination_init&idc="+catselection+idShopParam,true,{'product_ids':selection});
                    pushOneUsage('grid-bo-link-adminproducts_convertproduct','cat');
                    wConvertProductToCombination.attachEvent("onClose", function(win){
                        displayProducts();
                        return true;
                    });
                }
        ...
        if (itemId=="remove_sbo_relation"){
            let selection=cat_grid.getSelectedRowId();
            if (selection!='' && selection!=null){
                let confirmMessage = '<?php echo _l('Are you sure you want to scrap the Shippingbo link?', 1); ?>';
                if(selection.split(',').length > 1){
                    confirmMessage = '<?php echo _l('Are you sure you want to scrap the Shippingbo links?', 1); ?>';
                }
                if (confirm(confirmMessage)){
                    var params = {"field": "remove_sbo_relation", 'todo':true};
                    massUpdateInQueue(selection, params)
                }
            }
        }
        if (itemId=="open_in_shippingbo"){
            let sboLinkPattern = '<?php echo Sc\Service\Shippingbo\Shippingbo::LINK_PRODUCT_URL_PATTERN; ?>';
            let idSbo = cat_grid.getUserData(lastProductSelID, "id_sbo");
            if(cat_grid.getUserData(lastProductSelID, "type_sbo") == '<?php echo Sc\Service\Shippingbo\Shippingbo::SBO_PRODUCT_TYPE_BATCH; ?>'){
                sboLinkPattern = '<?php echo Sc\Service\Shippingbo\Shippingbo::LINK_BATCH_URL_PATTERN; ?>';
                idSbo = cat_grid.getUserData(lastProductSelID, "id_sbo_source");
            } else if(cat_grid.getUserData(lastProductSelID, "type_sbo") == '<?php echo Sc\Service\Shippingbo\Shippingbo::SBO_PRODUCT_TYPE_PACK; ?>'){
                sboLinkPattern = '<?php echo Sc\Service\Shippingbo\Shippingbo::LINK_PACK_URL_PATTERN; ?>';
            }

            let sboLink = sboLinkPattern.replace('{sbo_id_product}', idSbo);
            window.open(sboLink);
        }
    ...
    var contextMenuXML='<menu absolutePosition="auto" mode="popup" maxItems="8"  globalCss="contextMenu" globalSecondCss="contextMenu" globalTextCss="contextMenuItem">'+
        ...
        <?php } ?>
        <?php if(Sc\Service\Service::exists('shippingbo')) { ?>
        '<item img="contextmenu shippingbo" imgdis="contextmenu shippingbo" text="<?php echo ucfirst(_l('convert to combinations', 1)); ?>..." id="convert_to_combinations"/>'+
        '<item img="contextmenu shippingbo" imgdis="contextmenu shippingbo" text="<?php echo ucfirst(_l('remove Shippingbo link', 1)); ?>..." id="remove_sbo_relation"/>'+
        '<item img="contextmenu shippingbo" imgdis="contextmenu shippingbo" text="<?php echo ucfirst(_l('open in Shippingbo', 1)); ?>..." id="open_in_shippingbo"/>'+
        <?php } ?>
    '</menu>';
...
function displayProducts(callback)
{
    if (catselection!=undefined && catselection!=null && catselection!="" && catselection!=0)
    {
        ...
        ajaxPostCalling(cat_productPanel, cat_grid, loadUrl,function(data)
        {
            ...
            if(cat_grid_tb.getPosition('setposition')){
