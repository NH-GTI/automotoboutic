...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    getTbSettingsButton(prop_tb, {'grideditor':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':0}, 'prop_specificprice_',1000);
        ...
        if (id=='specificprice_add')
        {
            if (lastProductSelID==0){
                ...
                // INSERT
                    ...
                    foreach ($cols as $col)
                    {
                        switch ($col) {
                            case 'id_specific_price':
                                ...
                                break;
                            case 'id_product':
                                $insert[] = 'cat_grid.getSelectedRowId()';
                                break;
                            ...
                            case 'price':
                                ...
                                break;
                            case 'reduction_tax':
                                $insert[] = (int) _s('CAT_PROD_SPECIFIC_PRICES_DEFAULT_TAX');
                                break;
                            default:
                                $insert[] = '""';
                        }
                    ...
                    ?>
                    newRow = [<?php echo implode(',', $insert); ?>];
                    ...
                    prop_tb._specificpricesGrid.forEachCell(newId,function(cellObj,ind){
                        let col = prop_tb._specificpricesGrid.getColumnId(ind);
                        if(col === 'id_product')
                        {
                            col = 'id_product_list';
                        }
                        params.params[col] = prop_tb._specificpricesGrid.cells(newId,ind).getValue();
                    });
                    // USER DATA
    ...
    function displaySpecificPrices()
    {
        ...
        $.post("index.php?ajax=1&act=cat_specificprice_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'id_product_list': cat_grid.getSelectedRowId()},function(data)
        {
