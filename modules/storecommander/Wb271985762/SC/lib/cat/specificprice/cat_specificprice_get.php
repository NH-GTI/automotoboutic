...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    function generateValue($col, $row, $p, $pa = null)
    {
        ...
        switch ($col){
            ...
            case 'from_quantity':
                ...
                break;
            case 'price':
                $return .= ('<cell>'.($row['price'] != -1 ? number_format($row['price'], $round) : '-1').'</cell>');
                break;
            case 'reduction':
            ...
            case 'price_with_reduction_tax_incl':
                ...
                {
                    $p_price = $p->price;
                    $pa_price = ($pa ? $pa->price : 0) + $p_price;
                }
    ...
    function getRowsFromDB()
    {
        ...
        $allowed_shops = SC_Agent::getInstance()->getAllowedShops();
        $allowed_shops[] = 0;
        ...
        AND sp.id_shop IN ('.pInSQL(implode(',',$allowed_shops)).')
        AND sp.id_cart = 0 
