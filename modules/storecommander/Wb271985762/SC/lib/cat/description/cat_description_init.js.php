...
if (_r('GRI_CAT_PROPERTIES_GRID_DESC')) { ?>
    ...
    function initDescriptions(){
        if (needInitDescriptions)
        {
            ...
            if(Boolean(getParamUISettings('desc_translation'))) {
                defaultWidth = getParamUISettings('desc_translation');
            }
            prop_tb._descriptionsLayout.cells('b').setWidth(defaultWidth);
    ...
    prop_tb.attachEvent("onStateChange",function(id,state){
        switch(id) {
            case 'desc_twodesc':
                prop_tb._descriptionsLayout.cells('a').getFrame().contentWindow.hideOrShowAllDesc();
                break;
            case 'desc_3cols':
                if (state)
                {
