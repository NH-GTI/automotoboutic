...
if (Tools::getValue('rows') || $action == 'insert')
{
    ...
    if (is_array($rows) && count($rows) > 0)
    {
        ...
        foreach ($rows as $num => $row)
        {
            if (!empty($log_ids[$num]))
            {
                ...
                elseif (!empty($action) && $action == 'delete' && !empty($gr_id))
                {
                    ...
                    $updated_products[] = (int)Tools::getValue('id_product');
                }
        ...
        {
            $updated_products = implode(',', array_unique($updated_products));
            /**
             * Recalcule du champ customizable
             * Uniquement sur les produits impactés
             */
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'product` p
                    ...
                    WHERE id_product IN ('.pInSQL($updated_products).')');
