            ...
            if (!empty($log_ids[$num]))
            {
                ...
                elseif (!empty($action) && $action == 'update' && !empty($gr_id))
                {
                    ...
                    if (isset($_POST['price_inc_tax']) || isset($_POST['ecotax']))
                    {
                        ...
                        if (((int) SCI::getConfigurationValue('PS_USE_ECOTAX', null, 0, SCI::getSelectedShop()) == 1)) {
                        ...
                        if (isset($_POST[$field]))
                        {
                            if (sc_in_array($field, array('reduction_price', 'reduction_percent', 'reduction_from', 'reduction_to', 'reduction_tax', 'unit_price_ratio', 'unit_price_inc_tax', 'price'), 'catProductUpdateQueue_specialFields'))
                            {
                                ...
                                switch ($field.$versSuffix){
                                    case 'id_supplier':
                                        ...
                                        $combinations = Product::getProductAttributesIds((int) $id_product);
                                        if (!empty($combinations))
                                        {
                                            foreach ($combinations as $combination)
                                            {
                                                ...
                                                $id_product_attr = (int)$combination['id_product_attribute'];

                                                $combination = new Combination($id_product_attr, null, SCI::getSelectedShop());
                                                $combination->wholesale_price = 0;
                                                $combination->supplier_reference = '';
                                                $combination->update();

                                                $id_product_supplier = (int) ProductSupplier::getIdByProductAndSupplier((int) $id, (int) $id_product_attr, (int) $value);
                                                if (empty($id_product_supplier))
                                                {
                                                    ...
                                                    $product_supplier_entity->id_supplier = (int) $value;
                                    ...
                                    case 'unit_price_ratio15':
                                        if (floatval(Tools::getValue($field)) > 0)
                                        {
                                            ...
                                            $sql = 'UPDATE '._DB_PREFIX_.'product SET `unit_price_ratio`= 0 WHERE id_product='.(int) $id_product;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (($def = ObjectModel::getDefinition('Product'))
                    ...
                    if (Sc\Service\Service::exists('shippingbo')){
                        if(isset($_POST['is_locked'])) {
                            ...
                            if (!$sboShopRelations) {
                                $psProduct = new Product($id_product, false, null, $id_shop);
                                if ($psProduct->hasCombinations()) {
                                    foreach ($psProduct->getAttributeCombinations() as $product_attribute) {
                                        $newShopRelation = new Sc\Service\Shippingbo\Entity\ShopRelation(null, $shippingboService);
                                        $newShopRelation->setIdProduct($id_product);
                                        $newShopRelation->setReference($product_attribute['reference']);
                                        $newShopRelation->setIdProductAttribute($product_attribute['id_product_attribute']);
                                        $newShopRelation->setIdShop($id_shop);
                                        $sboShopRelations[] = $newShopRelation;
                                    }

                                } else {
                                    $newShopRelation = new Sc\Service\Shippingbo\Entity\ShopRelation(null, $shippingboService);
                                    $newShopRelation->setIdProduct($id_product);
                                    $newShopRelation->setReference($psProduct->reference);
                                    $newShopRelation->setIdProductAttribute(0);
                                    $newShopRelation->setIdShop($id_shop);
                                    $sboShopRelations = [$newShopRelation];
                                }
                            ...
                            foreach ($sboShopRelations as $sboShopRelation) {
                                $is_locked = (bool)Tools::getValue('is_locked', true);
                                $sboShopRelation->setIsLocked($is_locked);
                                if($is_locked){
                                    $sboShopRelation->setIdSbo(null);
                                }
                                $sboShopRelation->save();
                            }
                        }
                    }
                    ...
                    if (isset($_POST['combinations']) && substr(Tools::getValue('combinations'), 0, 13) == 'combinations_')
                    {
                        ...
                        if ($id_productsource != $id_product)
                        {
                            ...
                            $p = new Product($id_product, false, $id_lang, $selected_shop_id);

                            if($combination_images = Product::duplicateAttributes($id_productsource, $p->id))
                            {
                                if ($p->hasAttributes()) {
                                    Product::updateDefaultAttribute($p->id);
                                }
                                Image::duplicateProductImages($id_productsource, $p->id, $combination_images);
                            }
                        }
                    }
                        ...
                        if ($id_productsource != $id_product)
                        {
                            ...
                            if (!empty($source_features))
                            {
                                ...
                                foreach ($source_features as $row)
                                {
                                    ...
                                    $isCustom = Db::getInstance()->getValue('SELECT custom FROM '._DB_PREFIX_.'feature_value WHERE id_feature_value='.(int) (int) $row['id_feature_value']);

                                    if(isset($isCustom) && (bool) $isCustom) {
                                        // duplicate custom feature
                                        Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'feature_value SELECT NULL as id_feature_value,id_feature,custom FROM ' . _DB_PREFIX_ . 'feature_value WHERE id_feature_value = ' . (int)$row['id_feature_value']);
                                        // last inserted feature value id
                                        $feature_value_id = (int)Db::getInstance()->Insert_ID();
                                        // duplicate custom feature lang
                                        Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'feature_value_lang SELECT ' . $feature_value_id . ' as id_feature_value, id_lang,value FROM ' . _DB_PREFIX_ . 'feature_value_lang WHERE id_feature_value = ' . (int)$row['id_feature_value']);
                                        $sql_values[] = '('.(int) $row['id_feature'].','.(int) $id_product.','.$feature_value_id.')';
                                    } else {
                    ...
                    {
                        include SC_DIR.'lib/cat/partscompatibility/cat_partscompatibility_update_queue.php';
                    }
                    ...
                    if (count($todo_lang))
                    {
                        ...
                        $sql2 = 'UPDATE '._DB_PREFIX_.'product_lang SET '.join(' , ', $todo_lang).' WHERE id_product='.(int) $id_product.' AND id_lang='.(int) $id_lang.' AND id_shop IN ('.pInSQL(SCI::getSelectedShopActionList(true)).')';
                        if ($debug)
                        {
                            $dd .= $sql2."\n";
                        }
                        ...
                        if (isset($_POST['name']))
                        {
                            $sql3 = 'UPDATE '._DB_PREFIX_.'product_lang SET name=\''.pSQL(Tools::getValue('name')).'\' WHERE id_product='.(int) $id_product.' AND name=\'new\' AND id_shop IN ('.pInSQL(SCI::getSelectedShopActionList(true)).')';
