...
<script>
    ...
    //##################
    //##################
    @$files = getCatProperties($sc_agent);

    $credentials = $sc_agent->getCredentials();
            ...
            if (is_dir(SC_TOOLS_DIR . 'lib/cat/' . $item) && substr($item, 0, 4) != 'win-') {
                ...
                if (in_array($item, $have_sub_properties)) {
                    ...
                    foreach ($sub_files as $sub_item) {
                        if ($sub_item != '.' && $sub_item != '..') {
                            if (is_dir(SC_TOOLS_DIR . 'lib/cat/' . $item . '/' . $sub_item)
                                && file_exists(SC_TOOLS_DIR . 'lib/cat/' . $item . '/' . $sub_item . '/cat_' . $item . '_' . $sub_item . '_init.js.php')
                                && $credentials->getAllowedSubProperty($item, $sub_item)
                            ) {
                    ...
                    foreach ($sub_files as $sub_item) {
                        if ($sub_item != '.' && $sub_item != '..' && !in_array($sub_item, $sub_items_override)) {
                            if (is_dir(SC_DIR . 'lib/cat/' . $item . '/' . $sub_item)
                                && file_exists(SC_DIR . 'lib/cat/' . $item . '/' . $sub_item . '/cat_' . $item . '_' . $sub_item . '_init.js.php')
                                && $credentials->getAllowedSubProperty($item, $sub_item)
                            ) {
                ...
                if (in_array($item, $have_sub_properties)) {
                    ...
                    foreach ($sub_files as $sub_item) {
                        if ($sub_item != '.' && $sub_item != '..') {
                            if (is_dir(SC_DIR . 'lib/cat/' . $item . '/' . $sub_item)
                                && file_exists(SC_DIR . 'lib/cat/' . $item . '/' . $sub_item . '/cat_' . $item . '_' . $sub_item . '_init.php')
                                && $credentials->getAllowedSubProperty($item, $sub_item)
                            ) {
    ...
    <?php if (Combination::isFeatureActive()) { ?>
    prop_tb.attachEvent("onClick", function (id) {
        if (id == 'combinations') {
            ...
            switch ("<?php echo _s('CAT_PROD_COMBIMULTIPRODUCT_DEFAULT_SUBCOMBI'); ?>") {
                case "image":
