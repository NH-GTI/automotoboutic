...
<script>
    ...
    getTbSettingsButton(dhxlAffPartners.toolbar, {'grideditor':<?php echo (int)_r('MEN_TOO_GRIDSSETTINGS'); ?>,'settings':0}, 'partners_grid_', 100);
...
function displayAffiliation()
{
    ...
    dhxlAffPartners.grid.load("index.php?ajax=1&act=all_win-affiliation_manager_get&id_lang="+SC_ID_LANG,function()
        {
...
function displayAffiliates(ids)
{
    ...
    dhxlAffAffiliates.grid.load("index.php?ajax=1&act=all_win-affiliation_manager_affiliates_get&ids="+ids+"&id_lang="+SC_ID_LANG,function()
    {
...
function displayCommissions(ids)
{
    ...
    dhxlAffCommissions.grid.load("index.php?ajax=1&act=all_win-affiliation_manager_commissions_get&ids="+ids+"&id_lang="+SC_ID_LANG,function()
    {
...
function displayAffOrders(ids = 0)
{
    oldFilters=new Array();
    ...
    if(Boolean(dhxlAffPartners.grid.getSelectedId()) && ids!=undefined && ids!=null)
    {
        $.post("index.php?ajax=1&act=all_win-affiliation_manager_orders_get&id_lang="+SC_ID_LANG,
        {
            id_partner: Number(dhxlAffPartners.grid.getSelectedRowId()),
            ids: ids
        }
        ,function(data)
        {
...
function colorCommissionsConcernedByAffiliate(affiliate_ids)
{
    ...
    $.post("index.php?ajax=1&act=all_win-affiliation_manager_commissions_concerned_by_affiliate&id_lang="+SC_ID_LANG,{'ids':affiliate_ids},function(data){
        if(data.liste!="")
        {
...
function colorCommissionsAndAffiliateConcernedByOrder(order_ids)
{
    ...
    $.post("index.php?ajax=1&act=all_win-affiliation_manager_commissions_concerned_by_order&id_lang="+SC_ID_LANG,{'ids':order_ids},function(data){
        if(data.liste!="")
        {
            $.each( data.liste, function( num, id ) {
                if(dhxlAffCommissions.grid.getRowIndex(id)>=0)
...
function colorOrderAndAffiliateConcernedByCommission(id_commissions)
{
    ...
    $.post("index.php?ajax=1&act=all_win-affiliation_manager_concerned_by_commission&id_lang="+SC_ID_LANG,{'ids':id_commissions},function(data){
        if(data.listecom!="")
        {
...
function askCouponCode(id_partner, prefix)
{
    ...
    {
        $.post("index.php?ajax=1&act=all_win-affiliation_manager_reductioncode_exist&id_lang="+SC_ID_LANG,{'id_partner':id_partner,'code':code},function(data){
            if(data=="OK")
            {
                $.post("index.php?ajax=1&act=all_win-affiliation_manager_reductioncode_update&id_lang="+SC_ID_LANG,{'id_partner':id_partner,'code':code},function(data){
