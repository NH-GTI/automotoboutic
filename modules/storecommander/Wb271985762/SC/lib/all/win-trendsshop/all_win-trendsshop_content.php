...
if ($filters != null)
{
    ...
    {
        $modulePayementCache = [];
        $modulePayementFound = Db::getInstance()->executeS('SELECT DISTINCT `module` FROM '._DB_PREFIX_.'orders');
        if ($modulePayementFound)
        {
            foreach ($modulePayementFound as $modulePayement)
            {
                $modulePayementCache[$modulePayement['module']] = Module::getModuleName($modulePayement['module']);
            }
        }
        foreach ($orders as &$order)
        {
            if (isset($modulePayementCache[$order['module']]))
            {
                $order['payment'] = $modulePayementCache[$order['module']];
            }
        }
    }
    ...
    ## Traductions
    $month_full = [_l('January'), _l('February'), _l('March'), _l('April'), _l('May'), _l('June'), _l('July'), _l('August'), _l('September'), _l('October'), _l('November'), _l('December')];
    $month_short = [_l('Jan'), _l('Feb'), _l('Mar'), _l('Apr'), _l('May'), _l('Jun'), _l('Jul'), _l('Aug'), _l('Sep'), _l('Oct'), _l('Nov'), _l('Dec')];
    $day_full = [_l('Sunday'), _l('Monday'), _l('Tuesday'), _l('Wednesday'), _l('Thursday'), _l('Friday'), _l('Saturday')];
    $day_short = [_l('Sun.'), _l('Mon.'), _l('Tue.'), _l('Wed.'), _l('Thu.'), _l('Fri.'), _l('Sat.')];
    $quarter = [_l('Q1'), _l('Q2'), _l('Q3'), _l('Q4')];

    ## Champs disponibles
    $field_list = [
        ['id' => 'id_order', 'label' => _l('Order'), 'sortDir' => 'desc'],
        ['id' => 'date_add', 'label' => _l('Order creation date'), 'type' => 'date', 'format' => '%Y-%m-%d %H:%i:%s', 'group' => 'dateByYear', 'sortDir' => 'desc'],
        ['id' => 'payment', 'label' => _l('Payment method')],
        ['id' => 'language', 'label' => _l('Language')],
        ['id' => 'country', 'label' => _l('Country'), 'aliases' => $countries_name],
    ];
    $field_list[] = ['id' => 'id_default_group', 'label' => _l('Customer group'), 'aliases' => $groups_name];

    if (version_compare(_PS_VERSION_, '1.5.0.2', '>='))
    {
        $field_list[] = ['id' => 'carrier', 'label' => _l('Carrier'), 'aliases' => $carriers_name];
        $field_list[] = ['id' => 'total_shipping_tax_excl', 'label' => _l('Total shipping Tax excl')];
        $field_list[] = ['id' => 'total_shipping_tax_incl', 'label' => _l('Total shipping Tax incl')];
        $field_list[] = ['id' => 'total_paid_tax_incl', 'label' => _l('Total Paid Tax incl.')];
        $field_list[] = ['id' => 'total_paid_tax_excl', 'label' => _l('Total Paid Tax excl.')];
        $field_list[] = ['id' => 'id_order_for_margin', 'label' => _l('Order margin'), 'aliases' => $order_margin];
    }
    else
    {
        $field_list[] = ['id' => 'total_paid', 'label' => _l('Total Paid Tax incl.')];
    }
    if (SCMS)
    {
        $field_list[] = ['id' => 'shop', 'label' => _l('Shop')];
    }
        ...
        foreach ($conditions as $field => $condition)
        {
            ...
            switch ($field) {
                ...
                case 'date_end':
                    ...
                    break;
                case 'payment_method':
                    $return .= ' AND o.module IN ("'.str_replace(',', '","', pSQL($ids)).'")';
                    break;
                case 'carrier':
...
<head>
<meta charset="UTF-8">
    <link rel="stylesheet" href="lib/js/pivot/pivot.min.css?<?php echo date('Ymd'); ?>">
    <style>
        html,body{
            height:100%;
    ...
    <script src="lib/js/pivot/pivot.min.js?<?php echo date('Ymd'); ?>"></script>
</head>
<body>
<?php if ($filters == null) { ?>
    <script>
