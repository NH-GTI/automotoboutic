...
echo '<script>'; ?>
    ...
    dhxlTrendsShopFilters_Tb.attachEvent("onClick",function(id){
        ...
        if(id=='all') {
            dhxlTrendsShopFilters_Tb.callEvent("onClick",["refresh"]);
        }
    ...
    function displayTrendsShopContent(pivotFields)
    {
        ...
        dhxlTrendsShopContent.attachURL('index.php?ajax=1&act=all_win-trendsshop_content&id_lang='+SC_ID_LANG,null,{filters : filter_arr.filter(n => n), fields : fields});
    ...
    function fillFilterArray(f_arg, f_array)
    {
        ...
        $.each(filters, function(index, id) {
            if(id.substr(0,4) !== "none") {
                switch (id){
                    case 'date_start':
                    case 'date_end':
                        let data = dhxlTrendsShopFilters_Tree.getItemText(id).split(' ')
                        let date = data[1]
                        f_array.push(id+'#'+date);
                        break;
                    default:
                        f_array.push(id);
                }
    ...
    {
        if (!!ts_filter_views_list[ts_view]) {
            var ts_data = ts_filter_views_list[ts_view][0];
            var ts_splitedData = ts_data.split(',');
            $.each(ts_splitedData, function(index, id) {
                if(!!ts_view && id.includes('date_start') || id.includes('date_end')){
                    let newId = id.split('#');
                    id = newId[0];
                    let dateItemText = dhxlTrendsShopFilters_Tree.getItemText(id).replace(/(.*)\ (.*)/gi,  '$1 '+newId[1]);
                    dhxlTrendsShopFilters_Tree.setItemText(id,dateItemText);
                }
