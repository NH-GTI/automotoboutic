...
// data is recovered into fileList array so $_FILES is useless now
// bypass symfony httpFoundation/Request
$_FILES = [];
...
function dofileProcess($objectPs, $submittedFile)
{
    ...
    if (in_array($objectPs, ['attachment', 'productdownload']))
    {
    ...
    if (!$out)
    {
        exitResponseJson(103, _l('Failed to open output stream: %s<br/>This folder must be writeable', false, [implode('<br/>', explode('/', $targetDir.$fileName))]), $objectPs);
    }
...
function insertIntoPs($targetDir, $objectPs, $submittedFile)
{
    ...
    switch ($objectPs) {
                    ...
                    case 'edit_file':
                        ...
                        if ($id_attachment)
                        {
                            ...
                            if (file_exists($targetDir.$attachment->file)
                                && !unlink($targetDir.$attachment->file))
                            { ## delete old file
                    ...
                    case 'edit_file':
                        ...
                        if ($id_product_download)
                        {
                            ...
                            if (file_exists($targetDir.$download->filename)
                                && !unlink($targetDir.$download->filename))
                            { ## delete old file
        ...
        case 'image':
            ...
            {
                $highPos = Image::getHighestPosition((int) $id_product);
                $image = new Image();
                ...
                if (version_compare(_PS_VERSION_, '8', '>=') && $infoFile['extension'] == 'webp')
                {
                    if (!$new_path = $image->getPathForCreation())
                    {
                        exitResponseJson(2000, _l('PS: An error occurred while attempting to create a new folder.'), $objectPs);
                    }

                    if (!copy($tmpName, $new_path.'.'.$image->image_format))
                    {
                        exitResponseJson(2001, _l('PS: An error occurred while copying image source'), $objectPs);
                    }
                    ...
                    foreach ($imagesTypes as $imageType)
                    {
                        ...
                        {
                            exitResponseJson(2001, _l('PS: An error occurred while copying image'), $objectPs);
                        }
                    }
                }
                else
                {
                    ...
                    case 0:
                        ...
                        {
                            if ((int) $imageType['width'] < 1 || (int) $imageType['height'] < 1)
                            {
                                exitResponseJson(10511, _l('PS: image format %s has one or two wrong dimensions. Height:%s Width:%s', false, [$imageType['name'], (int) $imageType['height'], (int) $imageType['width']]), $objectPs);
                                break;
                            }
                            ...
                            {
                                exitResponseJson(1052, _l('PS: An error occurred while copying image %s', false, [stripslashes($imageType['name'])]), $objectPs);
                            }
                            else
                            {
                    ...
                    case 1:
                        ...
                        foreach ($imagesTypes as $k => $imageType)
                        {
                            ...
                            {
                                exitResponseJson(10531, _l('PS: An error occurred while copying image %s', false, [stripslashes($imageType['name'])]), $objectPs);
                            }
                            else
                            {
                                if ($generate_hight_dpi_images)
                    ...
                    case 2:
                        ...
                        foreach ($imagesTypes as $k => $imageType)
                        {
                            ...
                            {
                                exitResponseJson(10551, _l('PS: An error occurred while copying image %s', false, [stripslashes($imageType['name'])]), $objectPs);
                            }
                            ...
                            {
                                exitResponseJson(10552, _l('PS: An error occurred while copying image %s', false, [stripslashes($imageType['name'])]), $objectPs);
                            }
                            else
                            {
                                if ($generate_hight_dpi_images)
                ...
                SCI::hookExec('watermark', ['id_image' => $id_image, 'id_product' => $id_product]);

                if (!Image::getCover((int) $image->id_product))
                {
