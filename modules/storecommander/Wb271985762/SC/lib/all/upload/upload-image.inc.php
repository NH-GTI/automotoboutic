...
function imageResize($sourceFile, $destFile, $destWidth = null, $destHeight = null, $fileType = 'jpg')
{
    ...
    else
    {
        ...
        $createdImage = img_create_from(($fileType == 'png' ? 'image/png' : $mime), $destWidth, $destHeight, $sourceFile);
        if (!$createdImage)
        {
            return false;
        }
        list($rsrc, $img) = $createdImage;

        $bgcolor_settings = explode(',', _s('CAT_PROD_IMG_RESIZE_BGCOLOR'));
        if (count($bgcolor_settings) != 3)
        {
            $bgcolor_settings = [255, 255, 255];
        }
        $bgcolor = imagecolorallocate($rsrc, (int) $bgcolor_settings[0], (int) $bgcolor_settings[1], (int) $bgcolor_settings[2]);
        ...
        switch ($fileType) {
            ...
            case 'gif':
                ...
                if ($t_indx > 0)
                {
                    ...
                    $t_indx = imagecolorallocate($rsrc, (int) $t_color['red'], (int) $t_color['green'], (int) $t_color['blue']);
                    imagefill($rsrc, 0, 0, (int) $t_indx);
                    imagecolortransparent($rsrc, (int) $t_indx);
                }
                elseif ($fileType == 'png')
                {
                    imagealphablending($rsrc, false);
                    $color = imagecolorallocatealpha($rsrc, 0, 0, 0, 127);
                    imagefill($rsrc, 0, 0, (int) $color);
                    imagesavealpha($rsrc, true);
                }
                break;
            default:
                imagefilledrectangle($rsrc, 0, 0, (int) $destWidth, (int) $destHeight, (int) $bgcolor);
        }
        ...
        SCI::hookExec('actionOnImageResizeAfter', ['dst_file' => $destFile, 'file_type' => $fileType]);
        @imagedestroy($rsrc);
        @imagedestroy($img);
 ...
 * @return array|false
 */
function img_create_from($mime, $width, $height, $filename)
{
    switch ($mime) {
        case 'image/jpg':
        case 'image/jpeg':
        case 'image/jpe':
        case 'image/pjpeg':
            $rsrc = @imagecreatetruecolor($width, $height);
