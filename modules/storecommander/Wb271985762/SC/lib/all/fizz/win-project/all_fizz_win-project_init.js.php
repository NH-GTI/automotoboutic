...
echo '<script>'; ?>
    ...
    eSP_projects_tb.attachEvent("onClick",function(id){
        ...
        if(id=="archive")
        {
            let idxStatus=eSP_projects_grid.getColIndexById('status');
            let id_project = Number(eSP_projects_grid.getSelectedRowId());
            if(id_project > 0) {
                let status = Number(eSP_projects_grid.cells(id_project,idxStatus).getValue());
                if(status === 555)
                {
                    dhtmlx.message({text:'<?php echo _l('You can\'t archive a permanent project', 1); ?>' ,type:'error',expire:10000});
                    return false;
                }
