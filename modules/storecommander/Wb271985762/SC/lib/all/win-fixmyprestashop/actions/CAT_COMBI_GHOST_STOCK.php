...
{
    $sql = 'SELECT st.id_product, st.id_product_attribute
            FROM '._DB_PREFIX_.'stock st 
            WHERE st.id_product_attribute > 0 
                AND st.id_product_attribute NOT IN (SELECT pa.id_product_attribute FROM '._DB_PREFIX_.'product_attribute pa)
            LIMIT 1500';
