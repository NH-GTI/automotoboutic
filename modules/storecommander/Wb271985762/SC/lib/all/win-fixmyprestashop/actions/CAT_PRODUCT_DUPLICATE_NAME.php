...
if (!empty($post_action) && $post_action == 'do_check')
{
    ...
    if (!empty($name_found))
    {
        ...
        foreach ($name_found as $row)
        {
            ...
            $second_res = Db::getInstance()->ExecuteS($sql);
            if(!$second_res)
            {
                $second_res = array();
            }
            $res = array_merge($res, $second_res);
        }
    }
    ...
    if (!empty($name_found) && count($res) > 0)
    {
        ...
        <script>
            ...
            gridProductSameName.setHeader("ID <?php echo _l('product'); ?>,<?php echo _l('Name'); ?><?php if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) echo ","._l('Shop'); ?>,<?php echo _l('Language'); ?>,<?php echo _l('Active'); ?>");
            ...
            xml = xml+'      <cell><![CDATA[<?php echo (isset($shops[$row['id_shop']]) ? addslashes($shops[$row['id_shop']]) : '0'); ?>]]></cell>';
