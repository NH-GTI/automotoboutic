...
if (!empty($post_action) && $post_action == 'do_check')
{
    ...
    if (!empty($itemFound))
    {
        ...
        ob_start(); ?>
        <script>
            var rowsProductCombiSameREFERENCE = dhxlSCExtCheck.tabbar.cells("table_<?php echo $action_name; ?>").attachLayout("2E");

            var occurencesProductCombiSameREFERENCE_layout = rowsProductCombiSameREFERENCE.cells('a');
            occurencesProductCombiSameREFERENCE_layout.setText('<?php echo _l('Results'); ?>');

            var detailProductCombiSameREFERENCE_layout = rowsProductCombiSameREFERENCE.cells('b');
            detailProductCombiSameREFERENCE_layout.setText('<?php echo _l('Details of the selected rows'); ?>');

            // results
            var tbProductCombiSameREFERENCE = occurencesProductCombiSameREFERENCE_layout.attachToolbar();
            ...
            var gridProductCombiSameREFERENCE = occurencesProductCombiSameREFERENCE_layout.attachGrid();
            ...
            var referenceXml = '<rows>';
            <?php foreach ($itemFound as $item => $occurences) { ?>
            referenceXml = referenceXml+'   <row id="<?php echo htmlspecialchars($item); ?>">';
            ...
            function getSbProductCombiSameREFERENCE(){
                var filteredRows=gridProductCombiSameREFERENCE.getRowsNum();
                var selectedRows=(gridProductCombiSameREFERENCE.getSelectedRowId()?gridProductCombiSameREFERENCE.getSelectedRowId().split(',').length:0);
            ...
            // details
            var tbDetailProductCombiSameREFERENCE = detailProductCombiSameREFERENCE_layout.attachToolbar();
                ...
                switch (id) {
                    case 'gotocatalog':
                        if (lastIdProductCombiSameREFERENCE !== '') {
                            var path = gridDetailsProductCombiSameREFERENCE.getUserData(lastIdProductCombiSameREFERENCE, "path_pdt");
                            var url = "?page=cat_tree&open_cat_grid=" + path;
                            window.open(url, '_blank');
                        }
                        break;
                    case 'gotoCombicatalog':
                        if (lastIdProductCombiSameREFERENCE !== '') {
                            var path = gridDetailsProductCombiSameREFERENCE.getUserData(lastIdProductCombiSameREFERENCE, "path_combi");
                            var url = "?page=cat_tree&open_cat_grid=" + path;
                            window.open(url, '_blank');
                        }
                        break;
            ...
            var gridDetailsProductCombiSameREFERENCE = detailProductCombiSameREFERENCE_layout.attachGrid();
            ...
            gridDetailsProductCombiSameREFERENCE.setInitWidths("100,60,200,150,100,60,100,200");
            ...
            {
                var selection = gridProductCombiSameREFERENCE.getSelectedRowId();
                if(!selection)
                {
                    return;
                }
        ...
        foreach ($products as $pRow)
        {
            foreach($productAttributes as $paRow)
            {
                if ((!isset($pRow[$itemToFind]) || !isset($paRow[$itemToFind]))
                    || ($pRow[$itemToFind] != $paRow[$itemToFind])) {
                    continue;
                }
