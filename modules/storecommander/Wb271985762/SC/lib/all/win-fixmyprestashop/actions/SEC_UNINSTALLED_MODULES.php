...
if (!empty($post_action) && $post_action == 'do_check')
{
    ...
    if (!empty($uninstalledModuleToDelete))
    {
        ...
        <script>
            ...
            foreach($uninstalledModuleToDelete as $uniqueId => $moduleName)
            {
                $forJson['rows'][] = [
                    'id' => $uniqueId,
                    'data'=> [
            ...
            ?>
            gridUninstalledModules.json = <?php echo json_encode($forJson); ?>;
            gridUninstalledModules.parse(gridUninstalledModules.json, 'json');
            ...
            function deleteUninstalledModules()
            {
                ...
                if(selectedModules)
                {
                    $.post("index.php?ajax=1&act=all_win-fixmyprestashop_actions&check=<?php echo $current_control_filename; ?>&id_lang="+SC_ID_LANG, {
                        ...
                        {
                            gridUninstalledModules.deleteRow(idRow);
                        }
                        let numberOrRowRemain = gridUninstalledModules.getAllRowIds().split(',').length;
