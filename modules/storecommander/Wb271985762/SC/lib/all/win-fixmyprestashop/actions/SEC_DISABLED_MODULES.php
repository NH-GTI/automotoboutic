...
if (!empty($post_action) && $post_action == 'do_check')
{
    ...
    if (!empty($moduleDatabase))
    {
        ...
        <script>
            ...
            foreach($moduleDatabase as $module)
            {
                $active = _l('No');
                $activeValue = 0;
                if ($module['active'] == 1
                ...
                $forJson['rows'][] = [
                    ...
                    'userdata'=> [
                           'isEnable' => $activeValue
                    ],
                    'data'=> [
                        ['value' => $module['name'] , 'class' => $activeValue?'':'disabled'],
                        ['value' => $active , 'class' => $activeValue?'':'disabled'],
                    ]
                ];
            ...
            gridDisabledModules.json = <?php echo json_encode($forJson); ?>;
            gridDisabledModules.parse(gridDisabledModules.json, 'json');
            ...
            function uninstallAndDeleteDisabledModules()
            {
                ...
                if(selectedModules)
                {
                    $.post("index.php?ajax=1&act=all_win-fixmyprestashop_actions&check=<?php echo $current_control_filename; ?>&id_lang="+SC_ID_LANG, {
                        ...
                        for(const idRow of selectedModules.split(','))
                        {
                            gridDisabledModules.deleteRow(idRow);
                        }
                        let numberOrRowRemain = gridDisabledModules.getAllRowIds().split(',').length;
                        if(numberOrRowRemain <=0) {
