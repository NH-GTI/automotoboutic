...
if (!empty($post_action) && $post_action == 'do_check')
{
    ...
    $sql = 'SELECT DISTINCT c.id_category, cl.name, c.date_add, COUNT(cp.id_product) as nb_found 
            FROM `' . _DB_PREFIX_ . 'category` c
            LEFT JOIN `' . _DB_PREFIX_ . 'category_lang` cl 
                ON (c.id_category = cl.id_category AND cl.id_lang = '.(int) $id_lang .(version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? ' AND cl.id_shop=c.id_shop_default' : '').' )
    ...
    if (!empty($res) && count($res) > 0)
    {
        ...
        <script>
            ...
            xml = xml+'      <cell><![CDATA[<?php echo  $row['nb_found']; ?>]]></cell>';
