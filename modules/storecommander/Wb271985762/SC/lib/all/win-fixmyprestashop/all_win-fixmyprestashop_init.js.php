...
echo '<script>'; ?>
<?php
$defaultSolution = Tools::getValue('defaultSolution', false);
    ## defines js controls
    ...
    foreach ($controls as &$control)
    {
        ...
        {
            $nameDetail = (isset($control['name'][1]) ? $control['name'][1] : []);
            $control['name'] = substr(_l($control['name'][0], null, $nameDetail), 0, 250);
        }
        else
        {
        ...
        {
            $descriptionDetail = (isset($control['description'][1]) ? $control['description'][1] : []);
            $description = _l($control['description'][0], null, $descriptionDetail);
        }
        else
        {
...
?>
    var js_controls = <?php echo $js_controls; ?>;
    var defaultSolution = <?php echo $defaultSolution?'"'.$defaultSolution.'"':'false';?>;
    ...
    dhxlSCExtCheck.tbSolutions.attachEvent("onClick",function(id) {
        switch(id){
            case 'refresh':
                actions_selected='';
                displaySolutionList(defaultSolution);
    ...
    function displaySolutionList(defaultSolution)
    {
        ...
        $.post("index.php?ajax=1&act=all_win-fixmyprestashop_solutions",{action: "get_list"},function(data){
            ...
            if(defaultSolution){
                dhxlSCExtCheck.colSolutions.list.select(defaultSolution)
            }
        });
    }
    ...
    dhxlSCExtCheck.gridChecks.attachEvent("onFilterEnd", function(filterElements) {
        getFixGridStat();

        for (var colIndex=0; colIndex<dhxlSCExtCheck.gridChecks.getColumnsNum(); colIndex++) {
            let colId = dhxlSCExtCheck.gridChecks.getColumnId(colIndex);
            if ((colId == 'tools' && filterElements[0][0].value == "" ) || (colId == 'section' && filterElements[1][0].value == "")) {
                var selectFilter = dhxlSCExtCheck.gridChecks.getFilterElement(colIndex);
                if (selectFilter && selectFilter.tagName && selectFilter.tagName.toLowerCase() == "select") {
                    // Gather all unique values from the column after filtration
                    var uniqueValues = new Set();
                    let arrayIds = dhxlSCExtCheck.gridChecks.getAllRowIds().split(',');
                    for(const rId of arrayIds) {
                        var cellValue = dhxlSCExtCheck.gridChecks.cells(rId, colIndex).getValue();
                        uniqueValues.add(cellValue);
                    }

                    // Run through all options from select and hide if value not in column
                    for (var i = 0; i < selectFilter.options.length; i++) {
                        var option = selectFilter.options[i];
                        if (uniqueValues.has(option.value) || option.value === "") {
