...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    foreach ($controls as $row)
    {
        ...
        if ($good_version)
        {
            ...
            {
                $nameDetail = (isset($row['name'][1]) ? $row['name'][1] : []);
                $name = _l($row['name'][0], null, $nameDetail);
            }
            else
            {
                $name = _l($row['name']);
            ...
            {
                $descriptionDetail = (isset($row['description'][1]) ? $row['description'][1] : []);
                $description = _l($row['description'][0], null, $descriptionDetail);
            }
            else
            {
