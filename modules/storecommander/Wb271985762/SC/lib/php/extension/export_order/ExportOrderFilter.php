...
class ExportOrderFilter extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'description' => ['type' => self::TYPE_STRING, 'validate' => 'isCleanHtml'],
            'static_definition' => ['type' => self::TYPE_STRING, 'validate' => 'isCleanHtml'],
            'dynamic_definition' => ['type' => self::TYPE_STRING, 'validate' => 'isCleanHtml'],
    ...
    public function getSqlQuery($aliases, $id_lang, $orderby = null)
    {
        ...
        */
        ...
        $includes = ['includeCustomers' => $includeCustomers,
        ...
        if ($this->dynamic_definition)
        {
            ...
            $sqlInClause = $this->getTableClause(['Order_Id'], $includes, false, $id_lang);
    ...
    public function getDisplayOrderFilter($id_lang)
    {
        ...
        if (ExportOrderTools::isSCMS())
        {
            $return = $this->getList(['Order_Id', 'Shop_Id', 'Order_Display_Customer', 'Order_Display_Total', 'Order_Module', 'Address_Delivery_Country', 'Carrier', 'Order_Date_Add', 'Order_Invoice_Number'], $id_lang, 'o.`date_add` DESC');
        }
        elseif (ExportOrderTools::isNewerPs15x())
        {
            $return = $this->getList(['Order_Id', 'Order_Display_Customer', 'Order_Display_Total', 'Order_Module', 'Address_Delivery_Country', 'Carrier', 'Order_Date_Add', 'Order_Invoice_Number'], $id_lang, 'o.`date_add` DESC');
        }
        else
        {
            $return = $this->getList(['Order_Id', 'Order_Display_Customer', 'Order_Display_Total', 'Order_Module', 'Address_Delivery_Country', 'Carrier', 'Order_Date_Add'], $id_lang, 'o.`date_add` DESC');
        }
    ...
    private function analyzeRules($rules)
    {
        ...
        */
        ...
        foreach ($parts as $rule)
        {
            ...
            if ($rule == 'AND' || $rule == 'OR')
            {
                $ruleDescriptors[] = ['type' => 'operator', 'operator' => $rule];
            }
            else
            { // or a rule ?
                $subparts = explode('=>', $rule);
                $ruleName = $subparts[0];
                $ruleDescriptors[] = ['type' => 'rule', 'name' => $ruleName, 'descriptor' => $subparts[1]];
            }
        }
            ...
            else
            {
                ...
                switch ($rd['name'])
                {
                    ...
                    case 'a_d_address':
                        ...
                        $sqlClause = $this->getSqlRule(['ad.`address1`', 'ad.`address2`', 'ad.`other`'], $ruleDescriptor[1], $ruleDescriptor[2]);
                        break;
                    case 'a_d_postcode':
                    ...
                    case 'a_i_address':
                        // filter on either address1, address2 or other
                        $sqlClause = $this->getSqlRule(['ai.`address1`', 'ai.`address2`', 'ai.`other`'], $ruleDescriptor[1], $ruleDescriptor[2]);
                        break;
                    case 'a_i_postcode':
                        // c_a_postcode=>NONE|BEGINS|64
    ...
    private function getSqlRule($fields, $valueOperator1, $value1, $valueOperator2 = null, $value2 = null)
    {
        ...
        if (in_array($valueOperator1, ['like', 'begin', 'end', 'equal']))
        {
    ...
    public static function _getProductQty($id_product, $id_product_attribute = null, $id_warehouse = null)
    {
        ...
        { // Si PS>=1.5 & Stock Avancé & possède des déclinaisons & est dans l'entrepot
            $query = new DbQuery();
        ...
        elseif (SCAS && $type_advanced_stock_management == 2 && !$has_combination && !$not_in_warehouse)
        { // Si PS>=1.5 & Stock Avancé & n'a pas de déclinaison & est dans l'entrepot
            $query = new DbQuery();
        ...
        elseif (SCAS && $type_advanced_stock_management == 2 && $not_in_warehouse)
        { // Si PS>=1.5 & Stock Avancé & n'a pas de déclinaison & n'est pas l'entrepot
            $return = 0;
        }
        elseif (/* !SCAS && */
    ...
    public static function _getProductAttributeQty($id_product, $id_product_attribute = null)
    {
        ...
        { // Si PS>=1.5 & Stock Avancé & est dans l'entrepot
            $query = new DbQuery();
            $query->select('SUM(st.usable_quantity) as usable_quantity');
            $query->from('stock', 'st');
            $query->where('st.id_product = '.(int) $id_product.'');
        ...
        elseif (SCAS && $type_advanced_stock_management == 2 && $not_in_warehouse)
        { // Si PS>=1.5 & Stock Avancé & n'est pas l'entrepot
            $return = 0;
        }
        elseif ((!SCAS || (SCAS && $type_advanced_stock_management == 1)) && version_compare(_PS_VERSION_, '1.5.0.0', '>='))
        { // Si PS>=1.5 & pas Stock Avancé
            $query = new DbQuery();
            $query->select('SUM(quantity)');
            $query->from('stock_available');
            $query->where('id_product = '.(int) $id_product);
    ...
    public function _fillTaxRates()
    {
        ...
        $exist_tax_rates = [
            ['rate' => '100.0', 'min' => (100 - $margin), 'max' => (100 + $margin)],
            ['rate' => '0.0', 'min' => (0 - $margin), 'max' => $margin],
        ];
        ...
        foreach ($rates as $rate)
        {
            ...
            {
                $exist_tax_rates[] = ['rate' => $rate['rate'], 'min' => ($rate['rate'] - $margin), 'max' => ($rate['rate'] + $margin)];
            }
        }
    ...
    public function fillTax($id_order_list)
    {
        ...
        if ($data)
        {
            foreach ($data as $row)
            {
                ...
                {
                    $group_reduction = 1;
                    if (!empty((float)$row['group_reduction']))
                    {
                    ...
                    if ((float) $row['group_reduction'] > 0)
                    {
        ...
        if ($data)
        {
            foreach ($data as $row)
            {
                $divider = (float) $row['unit_price_tax_excl'];
                ...
                $rate = number_format((($row['unit_price_tax_incl'] / $divider) - 1) * 100, 1, '.', '');
