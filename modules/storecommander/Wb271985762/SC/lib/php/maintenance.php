...
// ----------------------------------------------------------------------------
    ...
    {
        ## deny file access
        if (!file_exists(SC_CSV_IMPORT_DIR.'.htaccess'))
        {
            file_put_contents(SC_CSV_IMPORT_DIR.'.htaccess', 'Deny from all');
        }
        if (!file_exists(SC_CSV_EXPORT_DIR.'.htaccess'))
        {
            file_put_contents(SC_CSV_EXPORT_DIR.'.htaccess', "Deny from all");
        }
        ...
        if (defined('SC_ExportOrders_ACTIVE') && (int) SC_ExportOrders_ACTIVE == 1 && !file_exists(SC_CSV_EXPORT_DIR.'orders/'))
        {
            mkdir(SC_CSV_EXPORT_DIR.'orders/', 0775);
        }
        if (defined('SC_ExportCustomers_ACTIVE') && (int) SC_ExportCustomers_ACTIVE == 1 && !file_exists(SC_CSV_EXPORT_DIR.'customers/'))
        {
            mkdir(SC_CSV_EXPORT_DIR.'customers/', 0775);
        }
