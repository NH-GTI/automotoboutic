...
class SC_Ext
{
    public static function getAllowedGrids($interface, $fromXmlList)
    {
        switch ($interface)
        {
            case 'cat':
                $views = [
                    'grid_light' => 'GRI_CAT_VIEW_GRID_LIGHT',
                    'grid_large' => 'GRI_CAT_VIEW_GRID_LARGE',
                    'grid_pack' => 'GRI_CAT_VIEW_GRID_PACK',
                    'grid_delivery' => 'GRI_CAT_VIEW_GRID_DELIVERY',
                    'grid_price' => 'GRI_CAT_VIEW_GRID_PRICE',
                    'grid_discount' => 'GRI_CAT_VIEW_GRID_DISCOUNT',
                    'grid_discount_2' => 'GRI_CAT_VIEW_GRID_DISCOUNT',
                    'grid_seo' => 'GRI_CAT_VIEW_GRID_SEO',
                    'grid_reference' => 'GRI_CAT_VIEW_GRID_REFERENCE',
                    'grid_description' => 'GRI_CAT_VIEW_GRID_DESCRIPTION',
                ];
                break;
            case 'cms':
                $views = [
                    'grid_light' => 'GRI_CMS_VIEW_GRID_LIGHT',
                    'grid_large' => 'GRI_CMS_VIEW_GRID_LARGE',
                    'grid_seo' => 'GRI_CMS_VIEW_GRID_SEO'
                ];
                break;
            case 'cus';
                $views = [
                    'grid_light' => 'GRI_CUS_VIEW_GRID_LIGHT',
                    'grid_large' => 'GRI_CUS_VIEW_GRID_LARGE',
                    'grid_address' => 'GRI_CUS_VIEW_GRID_ADDRESS',
                    'grid_convert' => 'GRI_CUS_VIEW_GRID_CONVERT'
                ];
                break;
            case 'ord':
                $views = [
                    'grid_light' => 'GRI_ORD_VIEW_GRID_LIGHT',
                    'grid_large' => 'GRI_ORD_VIEW_GRID_LARGE',
                    'grid_delivery' => 'GRI_ORD_VIEW_GRID_DELIVERY',
                    'grid_picking' => 'GRI_ORD_VIEW_GRID_PICKING'
                ];
                break;
            default:
                return [];
        ...
        $customGrids = [];
        foreach ($fromXmlList->grid as $grid)
        {
            if (isset($views[(string) $grid->name]) && !_r($views[(string) $grid->name])) {
                continue;
            } elseif (!_r('GRI_'.strtoupper($interface).'_VIEW_GRID_EXT_' . (string) $grid->name)) {
                continue;
            }

            $customGrids[(string) $grid->name] = (string) $grid->text->fr;
        }
    ...
    public static function readCustomGridsConfigXML($config, $extData = false)
    {
        ...
        switch ($config){
            case 'getCustomGrids':
                return self::getAllowedGrids('cat', $grids_products_conf->grids);
            case 'permissions':
                global $permissions_list;
                $commun_list = ['grid_light', 'grid_large', 'grid_delivery', 'grid_price', 'grid_discount', 'grid_discount_2', 'grid_seo', 'grid_reference', 'grid_description'];

                foreach ($grids_products_conf->grids->grid as $grid)
                {
                    if (!sc_in_array((string) $grid->name, $commun_list, 'extension_communlist'))
                    {
                        $permissions_list['GRI_CAT_VIEW_GRID_EXT_'.$grid->name] = ['id' => 'GRI_CAT_VIEW_GRID_EXT_'.$grid->name, 'section1' => 'Grid', 'section2' => 'Catalog', 'name' => 'Product grid:'.' '.(string) $grid->text->fr, 'description' => '', 'default_admin' => 1, 'default_value' => 1];
                    }
                }
                break;
            case 'gridnames':
                $views = self::getAllowedGrids('cat', $grids_products_conf->grids);
                if(empty($views)) {
                    break;
                }
                foreach ($views as $id => $name)
                {
                    echo "gridnames['".$id."']='".str_replace("'", "\'", $name)."';";
                }
                break;
            case 'toolbar':
                $lines = [];
            ...
            case 'colSettings':
                ...
                foreach ($grids_products_conf->fields->field as $field)
                {
                    ...
                    {
                        if (isset($colSettings[((string) $field->name)]['footer']))
                        {
                            $footer = $colSettings[((string) $field->name)]['footer'];
                        }
                        if (isset($colSettings[((string) $field->name)]['options']))
                        {
                    ...
                    $colSettings[((string) $field->name)] = ['text' => str_replace("'", "\'", (string) $field->text->fr),
                ...
                break;
            case 'updateSettings':
                global $fields,$fields_lang,$fieldsWithHTML,$forceUpdateCombinations,$idproduct,$id_lang,$multiStoresFields,$cols;
                ...
                foreach ($grids_products_conf->fields->field as $field)
                {
                    if ((isset($cols) && sc_in_array((string) $field->name, $cols, 'cols')) || !isset($cols))
                    {
                        ...
                        elseif (((string) $field->table) == 'product_html')
                        {
                            $fields[] = ((string) $field->name);
                            $fieldsWithHTML[] = ((string) $field->name);
                        }
                        elseif (((string) $field->table) == 'product_lang')
                        {
                            $fields_lang[] = ((string) $field->name);
                        }
                        elseif (((string) $field->table) == 'product_lang_html')
                        {
                            $fields_lang[] = ((string) $field->name);
                            $fieldsWithHTML[] = ((string) $field->name);
                        }
        ...
        switch ($config){
            ...
            case 'definition':
                ...
                foreach ($grids_combinations_conf->fields->field as $field)
                {
                    ...
                    if (empty($definition) && in_array(((string) $field->table), ['product_attribute', 'product_attribute_shop', 'product', 'product_shop', 'product_supplier']))
                    {
    ...
    public static function readCustomOrdersGridsConfigXML($config, $extData = false)
    {
        ...
        switch ($config){
            case 'getCustomGrids':
                return self::getAllowedGrids('ord', $grids_orders_conf->grids);
            case 'permissions':
                global $permissions_list;
                $commun_list = ['grid_light', 'grid_large', 'grid_delivery', 'grid_picking'];
                foreach ($grids_orders_conf->grids->grid as $grid)
                {
                    if (!sc_in_array((string) $grid->name, $commun_list, 'extension_communlist'))
                    {
                        $permissions_list['GRI_ORD_VIEW_GRID_EXT_'.$grid->name] = ['id' => 'GRI_ORD_VIEW_GRID_EXT_'.$grid->name, 'section1' => 'Grid', 'section2' => 'Orders', 'name' => 'Order grid:'.' '.(string) $grid->text->fr, 'description' => '', 'default_admin' => 1, 'default_value' => 1];
                    }
                }
                break;
            case 'gridnames':
                $views = self::getAllowedGrids('ord', $grids_orders_conf->grids);
                if(empty($views)) {
                    break;
                }
                foreach ($views as $id => $name)
            ...
            case 'colSettings':
                ...
                foreach ($grids_orders_conf->fields->field as $field)
                {
                    ...
                    if (!empty($colSettings[((string) $field->name)]))
                    {
                        $footer = isset($colSettings[((string) $field->name)]['footer']) ? $colSettings[((string) $field->name)]['footer'] : '';
                        $options = isset($colSettings[((string) $field->name)]['options']) ? $colSettings[((string) $field->name)]['options'] : '';
                    }
                    if (trim((string) $field->footer) != '')
                    {
                        $footer = (string) $field->footer;
    ...
    public static function readCustomCustomersGridsConfigXML($config, $extData = false)
    {
        ...
        switch ($config){
            case 'getCustomGrids':
                return self::getAllowedGrids('cus', $grids_customers_conf->grids);
            case 'permissions':
                global $permissions_list;
                $commun_list = ['grid_light', 'grid_large', 'grid_address', 'grid_convert'];

                foreach ($grids_customers_conf->grids->grid as $grid)
                {
                    if (!sc_in_array((string) $grid->name, $commun_list, 'extension_commun_list'))
                    {
                        $permissions_list['GRI_CUS_VIEW_GRID_EXT_'.$grid->name] = ['id' => 'GRI_CUS_VIEW_GRID_EXT_'.$grid->name, 'section1' => 'Grid', 'section2' => 'Customers', 'name' => 'Customer grid:'.' '.(string) $grid->text->fr, 'description' => '', 'default_admin' => 1, 'default_value' => 1];
                    }
                }
                break;
            case 'gridnames':
                $views = self::getAllowedGrids('cus', $grids_customers_conf->grids);
                if(empty($views)) {
                    break;
                }
                foreach ($views as $id => $name)
            ...
            case 'onAfterUpdate':
                ...
                break;
            case 'onAfterUpdateSQL':
                global $id_customer,$gr_id,$for_address;
                foreach ($grids_customers_conf->fields->field as $field)
                {
                    eval((string) $field->onAfterUpdateSQL);
                }
    ...
    public static function readCustomImageGridConfigXML($config)
    {
        ...
        case 'updateSettings':
            ...
            case 'onEditCell':
                ...
                foreach ($grid_image_conf->fields->field as $field)
                {
                    if (!empty((string) $field->onEditCell))
                    {
                        echo (string) $field->onEditCell;
                    }
    ...
    public static function readCustomCategoriesGridConfigXML($config, $extData = '')
    {
        ...
        switch ($config){
            ...
            case 'SQLSelectDataSelect':
                ...
                foreach ($grids_categories_conf->fields->field as $field)
                {
                    if (trim((string) $field->SQLSelectDataSelect) != '')
                    {
                        $sql .= eval((string) $field->SQLSelectDataSelect);
                    }
                }
            ...
            case 'SQLSelectDataLeftJoin':
                ...
                foreach ($grids_categories_conf->fields->field as $field)
                {
                    if (trim((string) $field->SQLSelectDataLeftJoin) != '')
                    {
                        $sql .= eval((string) $field->SQLSelectDataLeftJoin);
                    }
                }
    ...
    public static function readCustomCMSGridsConfigXML($config, $extData = false)
    {
        ...
        switch ($config){
            case 'getCustomGrids':
                return self::getAllowedGrids('cms', $grids_cms_conf->grids);
            case 'permissions':
                global $permissions_list;
                $commun_list = ['grid_light', 'grid_large', 'grid_seo'];

                foreach ($grids_cms_conf->grids->grid as $grid)
                {
                    if (!sc_in_array((string) $grid->name, $commun_list, 'extension_commun_list_cms'))
                    {
                        $permissions_list['GRI_CMS_VIEW_GRID_EXT_'.$grid->name] = ['id' => 'GRI_CMS_VIEW_GRID_EXT_'.$grid->name, 'section1' => 'Grid', 'section2' => 'CMS', 'name' => 'CMS grid:'.' '.(string) $grid->text->fr, 'description' => '', 'default_admin' => 1, 'default_value' => 1];
                    }
                }
                break;
            case 'gridnames':

                $views = self::getAllowedGrids('cms', $grids_cms_conf->grids);
                if(empty($views)) {
                    break;
                }
                foreach ($views as $id => $name)
    ...
    public static function addNewField($type, $field, $view = '')
    {
        if (!empty($type) && !empty($field))
        {
            ...
            $alone_grids = ['combinations', 'productsort', 'msproduct', 'mscombination', 'image'];
    ...
    public static function readCustomExportOrderConfigXML($config, &$fieldConfig, $extData = false)
    {
        if (!SC_TOOLS || !file_exists(SC_TOOLS_DIR.'export_order_csv_conf.xml') || !_s('CORE_USE_EXTENSIONS'))
        {
            return false;
        }

        if (!$exportOrderCsvConf = simplexml_load_file(SC_TOOLS_DIR.'export_order_csv_conf.xml'))
        {
            return false;
        }
        ...
        switch ($config){
            ...
            case 'calculated':
            case 'rowDataBefore':
                foreach ($exportOrderCsvConf->fields as $field)
                {
                    foreach ($field as $rowField)
                    {
                        if (isset($rowField->{$config}) && !empty((string) $rowField->{$config}))
                        {
                            eval((string) $rowField->{$config});
                        }
                    }
                }
                break;
    ...
    public static function readCustomExportCustomerConfigXML($config, &$fieldConfig = null, $extData = false)
    {
        if (!SC_TOOLS || !file_exists(SC_TOOLS_DIR.'export_customer_csv_conf.xml') || !_s('CORE_USE_EXTENSIONS'))
        {
            return false;
        }

        if (!$exportCustomerCsvConf = simplexml_load_file(SC_TOOLS_DIR.'export_customer_csv_conf.xml'))
        {
            return false;
        }
        ...
        switch ($config){
            case 'SQLSelectDataSelect':
            case 'SQLSelectDataLeftJoin':
                foreach ($exportCustomerCsvConf->fields as $field)
                {
                    foreach ($field as $rowField)
                    {
                        if ($extData && !in_array((string) $rowField->name, $extData))
                        {
                            continue;
                        }
                        if (isset($rowField->{$config}) && !empty((string) $rowField->{$config}))
                        {
                            $fieldConfig .= eval((string) $rowField->{$config});
                        }
                    }
                }
                break;
            case 'translation':
                foreach ($exportCustomerCsvConf->fields as $field)
                {
                    foreach ($field as $rowField)
                    {
                        if (isset($rowField->name) && !empty((string) $rowField->name))
                        {
                            $fieldConfig[(string) $rowField->name] = [1 => (string) $rowField->text->en, 2 => (string) $rowField->text->fr];
                        }
                    }
                }
                break;
            ...
            case 'miscellaneous':
                foreach ($exportCustomerCsvConf->fields as $field)
                {
                    foreach ($field as $rowField)
                    {
                        if (isset($rowField->{$config}) && !empty((string) $rowField->{$config}))
                        {
                            $fieldConfig[(string) $rowField->name] = (string) $rowField->name;
                        }
                    }
                }
                break;
