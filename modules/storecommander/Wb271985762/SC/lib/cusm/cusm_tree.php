...
<?php
    createMenu();
?>
    cookie_selection = Cookies.get('sc_cusm_filters_selected');
    if(cookie_selection!=null && cookie_selection!="" && cookie_selection!=0)
        filterselection=Cookies.get('sc_cusm_filters_selected');
    else
    {
        filterselection = "st_open";
        Cookies.set('sc_cusm_filters_selected',filterselection, defaultCookieOptions);
    }
...
<?php } ?>
    ...
    cusm_filter.attachEvent("onCheck",function(idfilter, state){
            if(idfilter === 'from_to' && state)
            {
                periodselection=idfilter;
                Cookies.set('sc_cusm_periodselection',periodselection, defaultCookieOptions);

                if (dhxWins.isWindow("wCusmFilterFromTo"))
                    wCusmFilterFromTo.close();

                if (!dhxWins.isWindow("wCusmFilterFromTo")) {
                    wCusmFilterFromTo = dhxWins.createWindow("wCusmFilterFromTo", 170, 150, 350, 230);
...
?>
    ...
    function displayFilters(callback)
    {
        ...
        cusm_filter.load("index.php?ajax=1&act=cusm_filter_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
                ...
                let dateselection = Cookies.get('sc_cusm_fromto_dates');
                if(dateselection !== null && dateselection !== undefined) {
                    cusm_filter.setCheck('from_to', true);

                    let dates = dateselection.split("_");
                    let from = dates[0];
                    let to = "";
                    ...
                    cusm_filter.setItemText('from_to','<?php echo _l('Ord from'); ?> '+from+" <?php echo _l('to'); ?> "+to);
                }
