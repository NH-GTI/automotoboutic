...
switch ($action) {
    ...
    case 'mapping_process':
        ...
        for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA) - 1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'], $DATA[$current_line])) && ($current_line <= $importlimit)); ++$current_line)
        {
            ...
            foreach ($line as $key => $value)
            {
                ...
                if (sc_array_key_exists($key, $firstLineData) && sc_in_array($firstLineData[$key], $mappingData['CSVArray'], 'manWinImportProcess_CSVArray'))
                {
                    ...
                    $line_id_lang = isset($getIDlangByISO[$mappingData['CSV2DBOptions'][$firstLineData[$key]]])?$getIDlangByISO[$mappingData['CSV2DBOptions'][$firstLineData[$key]]]:Configuration::get('PS_LANG_DEFAULT');
                    switch ($switchObject) {
                        // CUSTOMER
                        case 'name':
                            ...
                            break;
                        case 'description':
                            if (is_array($newManufacturer->description))
                            {
                                $newManufacturer->description[$line_id_lang] = psql($value);
                            }
                            else
                            {
                                $newManufacturer->description = psql($value);
                            }
                            break;
                        case 'short_description':
                            if (is_array($newManufacturer->short_description))
                            {
                                $newManufacturer->short_description[$line_id_lang] = psql($value);
                            }
                            else
                            {
                                $newManufacturer->short_description = psql($value);
                            }
                            break;
                        case 'meta_title':
                            if (is_array($newManufacturer->meta_title))
                            {
                                $newManufacturer->meta_title[$line_id_lang] = psql($value);
                            }
                            else
                            {
                                $newManufacturer->meta_title = psql($value);
                            }
                            break;
                        case 'meta_keywords':
                            if (is_array($newManufacturer->meta_keywords))
                            {
                                $newManufacturer->meta_keywords[$line_id_lang] = psql($value);
                            }
                            else
                            {
                                $newManufacturer->meta_keywords = psql($value);
                            }
                            break;
                        case 'meta_description':
                            if (is_array($newManufacturer->meta_description))
                            {
                                $newManufacturer->meta_description[$line_id_lang] = psql($value);
                            }
                            else
                            {
                                $newManufacturer->meta_description = psql($value);
                            }
                            break;
                        case 'id_shop_list':
                        ...
                        case 'imageURL':
                            ...
                            break;
                        case 'active':
                            $newManufacturer->active = (int)getBoolean($value);;
                            break;
                        default:
                    }
                }
        ...
        if ((count($CSVData) == 1) || (count($CSVData) == 2 && $CSVData[0] == join('', $CSVData)) || (filesize(SC_CSV_IMPORT_DIR.'manufacturers/'.$TODOfilename) == 0))
        {
            ...
            echo '<script>window.top.displayOptionsMan();window.top.stopAutoImportMan(true);</script>';
        ...
        else
        {
            ...
            echo '<script>window.top.displayOptionsMan();window.top.prepareNextStepMan('.($stats['created'] + $stats['modified'] + $stats['skipped'] == 0 ? 0 : filesize(SC_CSV_IMPORT_DIR.'manufacturers/'.$TODOfilename)).');</script>';
