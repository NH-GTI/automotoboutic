...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    function duplicateCmsCategories($id_cms_category, $id_parent)
    {
        ...
        if (!empty($result[0]['id_cms_category']))
        {
            ...
            $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'cms_category` ('.bqInSQL(implode(',', $names)).')
            ...
            if (!empty($new_id_cat))
            {
                ...
                if (version_compare(_PS_VERSION_, '1.6.0.12', '>='))
                {
                    ...
                    foreach ($results as $result)
                    {
                        ...
                        $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'cms_category_shop` ('.bqInSQL(implode(',', $names)).')
                                        VALUES ("'.implode('","', $values).'")';
                        Db::getInstance()->execute($sql_insert);
                    }
                }
                ...
                foreach ($results as $result)
                {
                    ...
                    $sql_insert = 'INSERT INTO `'._DB_PREFIX_.'cms_category_lang` ('.bqInSQL(implode(',', $names)).')
                                    VALUES ("'.implode('","', $values).'")';
                    Db::getInstance()->execute($sql_insert);
                }
