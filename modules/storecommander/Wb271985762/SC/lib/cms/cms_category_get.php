...
if (!defined('STORE_COMMANDER')) { exit; }
    ...
    if (!$res)
    {
        ...
        $newCmsCategory->level_depth = SCI::getConfigurationValue('PS_ROOT_CATEGORY');
        $newCmsCategory->active = 0;
        $newCmsCategory->position = CMSCategory::getLastPosition(SCI::getConfigurationValue('PS_ROOT_CATEGORY'));
        foreach ($languages as $lang)
        {
            $newCmsCategory->link_rewrite[$lang['id_lang']] = 'sc_recycle_bin';
    ...
    {
        // fix bug in db
        if ($res['id_cms_category'] == $res['id_parent'])
        {
            Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'cms_category 
                                            SET id_parent = '.(int) SCI::getConfigurationValue('PS_ROOT_CATEGORY').' 
                                            WHERE id_cms_category = '.(int) $res['id_cms_category']);
        }
        $bincategory = $res['id_cms_category'];
    }
    $binPresent = false;
