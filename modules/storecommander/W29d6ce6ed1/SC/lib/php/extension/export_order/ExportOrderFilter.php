...
class ExportOrderFilter extends ObjectModel
{
    ...
    public function getList($aliases, $id_lang, $orderby = null)
    {
        ...
        if (Tools::strlen($sql) > 0)
        {
            ...
            ExportOrderTools::addLog('CList run :'.$this->id.' - get DATAS : '.ExportOrderTools::captureVarDump($datas));
        }
        else
        {
            $datas = null;
                            ...
                            if (!empty($data['id_slips']) && $data['id_slips'] > 0)
                            {
                                ...
                                if (ExportOrderTools::isNewerPs16x())
                                {
                                    $sql = 'SELECT GROUP_CONCAT(DISTINCT(CONCAT(os.`id_order_slip`,"(",DATE_FORMAT(os.`date_add`,"%Y-%m-%d"),")"))) as id_slips,
                                                                                                                (SUM(os.`total_shipping_tax_incl`)) as Calculated_Slip_Shipping_TTC,
                                                        (SUM(os.`total_shipping_tax_excl`)) as Calculated_Slip_Shipping,
                                                         os.`shipping_cost`
                                    ...
                                    $sql2 = 'SELECT (SUM(osd.`amount_tax_excl`)) as Calculated_Slip,
                                                                                                                (SUM(osd.`amount_tax_incl`)) as Calculated_Slip_TTC
                                                                                        FROM `'._DB_PREFIX_.'order_slip` os
                                                                                        LEFT JOIN `'._DB_PREFIX_.'order_slip_detail` osd ON (osd.`id_order_slip`=os.`id_order_slip`)
                                                                                        WHERE os.`id_order` = "'.(int) $data['temp_id_order'].'"';
                                }
                                ...
                                {
                                    $sql = 'SELECT GROUP_CONCAT(DISTINCT(CONCAT(os.`id_order_slip`,"(",DATE_FORMAT(os.`date_add`,"%Y-%m-%d"),")"))) as id_slips,
                                                                                                                (SUM(os.`shipping_cost_amount`)) as Calculated_Slip_Shipping_TTC,
                                                        (SUM((os.`shipping_cost_amount`/ (1+(o.`carrier_tax_rate`/100))))) as Calculated_Slip_Shipping,
                                                        os.`shipping_cost`
                                                                                        FROM `'._DB_PREFIX_.'order_slip` os
                                                                                        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (os.`id_order`=o.`id_order`)
                                                                                        WHERE os.`id_order` = "'.(int) $data['temp_id_order'].'"';
                                    $sql2 = 'SELECT (SUM(osd.`amount_tax_excl`)) as Calculated_Slip,
                                                                                                                (SELECT SUM(`amount`) FROM `'._DB_PREFIX_.'order_slip` WHERE `id_order` = "'.(int) $data['temp_id_order'].'") as Calculated_Slip_TTC
                                                                                        FROM `'._DB_PREFIX_.'order_slip` os
                                                                                        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (os.`id_order`=o.`id_order`)
                                                                                        LEFT JOIN `'._DB_PREFIX_.'order_slip_detail` osd ON (osd.`id_order_slip`=os.`id_order_slip`)
                                                                                        WHERE os.`id_order` = "'.(int) $data['temp_id_order'].'"';
                                ...
                                {
                                    $sql = 'SELECT GROUP_CONCAT(DISTINCT(CONCAT(os.`id_order_slip`,"(",DATE_FORMAT(os.`date_add`,"%Y-%m-%d"),")"))) as id_slips,
                                                                                                                 GROUP_CONCAT(DATE_FORMAT(os.`date_add`,"%Y-%m-%d")) as slips_date,
                                                                                                (SUM(os.`shipping_cost`)) as Calculated_Slip_Shipping_TTC,
                                                                                                (SUM((os.`shipping_cost`/ (1+(o.`carrier_tax_rate`/100))))) as Calculated_Slip_Shipping
                                                                                        FROM `'._DB_PREFIX_.'order_slip` os
                                                                                        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (os.`id_order`=o.`id_order`)
                                                                                        WHERE os.`id_order` = "'.(int) $data['temp_id_order'].'"';
                                    $sql2 = 'SELECT (SUM(osd.`product_quantity`*osd_pd.`sc_qc_product_price`)) as Calculated_Slip,
                                                                                                (SUM(osd.`product_quantity`* ROUND(osd_pd.`sc_qc_product_price` * (osd_pd.`tax_rate`/100+1), 2) ) + SUM(o.`total_shipping`*os.`shipping_cost`)) as Calculated_Slip_TTC
                                                                                        FROM `'._DB_PREFIX_.'order_slip` os
                                                                                        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (os.`id_order`=o.`id_order`)
                                                                                        LEFT JOIN `'._DB_PREFIX_.'order_slip_detail` osd ON (osd.`id_order_slip`=os.`id_order_slip`)
                                                                                                LEFT JOIN `'._DB_PREFIX_.'order_detail` osd_pd ON (osd.`id_order_detail`=osd_pd.`id_order_detail`)
                                ...
                                else
                                {
                                    $sql = 'SELECT GROUP_CONCAT(DISTINCT(CONCAT(os.`id_order_slip`,"(",DATE_FORMAT(os.`date_add`,"%Y-%m-%d"),")"))) as id_slips,
                                                                                        ...
                                                                                        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (os.`id_order`=o.`id_order`)
                                                                                        WHERE os.`id_order` = "'.(int) $data['temp_id_order'].'"';
                                    $sql2 = 'SELECT (SUM(osd.`product_quantity`*osd_pd.`product_price`)) as Calculated_Slip,
                                                                                                (SUM(osd.`product_quantity`*osd_pd.`product_price` * (osd_pd.`tax_rate`/100+1)) + SUM(o.`total_shipping`*os.`shipping_cost`)) as Calculated_Slip_TTC
                                                                                        FROM `'._DB_PREFIX_.'order_slip` os
                                                                                        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (os.`id_order`=o.`id_order`)
                                                                                        LEFT JOIN `'._DB_PREFIX_.'order_slip_detail` osd ON (osd.`id_order_slip`=os.`id_order_slip`)
                                                                                                LEFT JOIN `'._DB_PREFIX_.'order_detail` osd_pd ON (osd.`id_order_detail`=osd_pd.`id_order_detail`)
                                ...
                                $slips2 = Db::getInstance()->executeS($sql2);
                                if (!empty($slips[0]) && !empty($slips[0]['id_slips']))
                                {
                                    // echo "<pre>";print_r($slips);echo "</pre>";die();
                                    $data['id_slips'] = $slips[0]['id_slips'];
                                    if (ExportOrderTools::isNewerPs15x() && !empty($slips[0]['shipping_cost']))
                                    {
                                        $data['Calculated_Slip_Shipping'] = $slips[0]['Calculated_Slip_Shipping'];
                                        $data['Calculated_Slip_Shipping_TTC'] = $slips[0]['Calculated_Slip_Shipping_TTC'];
                                    }
                                }
                                if (!empty($slips2[0]))
                                {
                                    $data['Calculated_Slip'] = $slips2[0]['Calculated_Slip'];
                                    $data['Calculated_Slip_TTC'] = $slips2[0]['Calculated_Slip_TTC'];
                                }
                            }
                            else
                            {
