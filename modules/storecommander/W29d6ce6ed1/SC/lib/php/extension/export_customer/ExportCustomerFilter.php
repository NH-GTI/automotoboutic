        ...
        if ($includeCustomers) {
            ...
            if ($this->dynamic_definition) {
                ...
                if ($where) {
                    ...
                    SC_Ext::readCustomExportCustomerConfigXML('SQLSelectDataSelect', $sql);
                    $sql .= ' FROM `' . _DB_PREFIX_ . 'customer` c ';
                    SC_Ext::readCustomExportCustomerConfigXML('SQLSelectDataLeftJoin', $sql);
            ...
            if ($exportTemplate->export_format == 'CSV' && count($datas) > 0) {
                ...
                foreach ($datas as $data) {
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        $buffer .= $temp_value != '' ? ExportCustomerTools::prepareValueForCsv($temp_value) : ExportCustomerFields::getDefaultValues($key, $id_lang);
                        $i++;
                    }
                    $buffer .= "\n";
                }
