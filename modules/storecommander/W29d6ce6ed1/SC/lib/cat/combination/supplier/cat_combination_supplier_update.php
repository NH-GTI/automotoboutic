    ...
    switch ($action) {
        case 'fields':
            ...
            foreach ($ids as $id)
            {
                if (isset($value))
                {
                    $combination = new Combination((int) $id, null, SCI::getSelectedShop());
                    $sql = '
                        SELECT *
                    ...
                    $product = new Product((int) $combination->id_product, false, (int) $id_lang, (int) SCI::getSelectedShop());
                    ...
                    {
                        $combination->supplier_reference = $new->product_supplier_reference;
                        $combination->save();
                    }
                    ...
                    {
                        $combination->wholesale_price = $new->product_supplier_price_te;
                        $combination->save();
                    }
                    ...
                    if (version_compare(_PS_VERSION_, '1.5.0.10', '>=') && !empty($product->id_supplier) && $product->id_supplier == $id_supplier && _s('CAT_PROD_WHOLESALEPRICE_SUPPLIER') == 1)
                    {
                        ...
                        $sql .= (_s('CAT_PRODPROP_SUPPLIER_WHOLESALEPRICE_SAVING_METHOD') == 1 ? ' AND id_shop IN ('.pInSQL(SCI::getSelectedShopActionList(true, (int)$combination->id_product)).')' : '');
