...
<?php } ?>
        cat_prop_info_grid.attachEvent("onRowSelect", function(id,ind){
            let ids = cat_prop_info_grid.getSelectedRowId();
            if(ids !== null)
            {
                ids = ids.split(',').pop();
<?php if (version_compare(_PS_VERSION_, '8.0.0', '<')) { ?>
...
<?php } ?>
                cat_prop_description_cell.attachURL("index.php?ajax=1&act=cat_win-catmanagement_info_form",null,{field:'description',id_row:ids});

<?php if (version_compare(_PS_VERSION_, '8.0.0', '>=')) { ?>
                cat_prop_additionalDescription_cell.attachURL("index.php?ajax=1&act=cat_win-catmanagement_info_form",null,{field:'additional_description',id_row:ids});
<?php } ?>
            }
        });
}
