                    ...
                    if (isset($_POST['combinations']) && substr(Tools::getValue('combinations'), 0, 13) == 'combinations_')
                    {
                        ...
                        if ($id_productsource != $id_product)
                        {
                            ...
                            foreach ($res as $key => $row)
                            {
                                ...
                                if ((int) $idco)
                                {
                                    $sql = 'SELECT id_attribute FROM '._DB_PREFIX_.'product_attribute_combination WHERE id_product_attribute='.(int) $row['id_product_attribute'];
                                    $res = Db::getInstance()->executeS($sql);
                                    if($res) {
                                        $combi = new Combination($idco);
                                        $combi->id_shop_list = $id_shop_list;
                                        $combi->setAttributes(array_column($res,'id_attribute'));
                                        $combi->save();
                                    }
                                }
                            }
