...
function getFiltersConditions($filters = null)
{
    ...
    foreach ($filters as $field => $filter)
    {
        ...
        switch ($query) {
            case '!!':
                $condition[] = '(pasl.`'.bqSQL($field).'` IS NULL OR pasl.`'.bqSQL($field).'` = "")';
                break;
            default:
                if (!empty($query))
                {
                    $condition[] = 'pasl.`'.bqSQL($field).'` LIKE "%'.pSQL($query).'%"';
                }
        }
    }
...
if ($id_lang)
{
    ...
    $sql = 'SELECT pasl.*
        FROM '._DB_PREFIX_.'pm_advancedsearch_seo_lang pasl
        LEFT JOIN '._DB_PREFIX_.'pm_advancedsearch_seo pas ON pas.id_seo = pasl.id_seo
        WHERE pas.deleted = 0 AND pasl.id_lang = '.(int) $id_lang
        .$filtersConditions.
        ' ORDER BY pasl.id_seo ASC';
...
if ($data)
{
    foreach ($data as $row_data)
    {
        ...
        $row_xml[] = '<cell><![CDATA['.cleanLineBreakForHtmlFieldInGrid($row_data['description']).']]></cell>';
        if (version_compare($advSearchModuleInstance->version, '5.0.0', '>='))
        {
            $row_xml[] = '<cell><![CDATA['.cleanLineBreakForHtmlFieldInGrid($row_data['footer_description']).']]></cell>';
        }
