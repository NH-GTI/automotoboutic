                    ...
                    if ($sub_action != '')
                    {
                        ...
                        switch ($sub_action)
                        {
                            ...
                            case 'default1':
                                ...
                                {
                                    $plist = Tools::getValue('idlist', null);
                                    $id_product_src = [];
                                    if($plist)
                                    {
                                        $sql = 'SELECT id_product FROM `'._DB_PREFIX_.'category_product` WHERE id_category='.(int) $id_category.' AND id_product IN ('.pInSQL($plist).')';
                                        $id_product_alreadyin = [];
                                        if($res = Db::getInstance()->executeS($sql))
                                        {
                                            $id_product_alreadyin = array_column($res,'id_product');
                                        }
                                        $id_product_src = array_diff(explode(',', $plist),$id_product_alreadyin);
                                    }
                                    $sql = 'SELECT MAX(position) AS max FROM '._DB_PREFIX_.'category_product WHERE id_category='.(int) $id_category;
                                    $res = Db::getInstance()->getRow($sql);
                                    $max = (int) $res['max'];
