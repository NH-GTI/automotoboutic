    ...
    function setPropertiesPanel_segments(id){
            ...
            prop_tb._segmentsGrid.attachEvent("onEditCell",function(stage,rId,cInd,nValue,oValue)
            {
                    ...
                    if (cInd == idxUsed){
                        if(stage==1)
                        {
                            ...
                            $.post("index.php?ajax=1&act=ord_segmentation_update&type=order&id_segment="+rId+"&action=present&value="+value+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{ids:ord_grid.getSelectedOrderId()},function(data){
                                displaySegments();
                            });
                        }
                    }
        ...
        if (id=='segment_multi_add')
        {
            if(prop_tb._segmentsGrid.getSelectedRowId()!=null && lastOrderSelID!=null)
            {
                ...
                $.post("index.php?ajax=1&act=ord_segmentation_update&type=order&segments="+segments+"&action=mass_present&value="+value+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{ids:ord_grid.getSelectedOrderId()},function(data){
                    displaySegments();
                });
            }
        }
