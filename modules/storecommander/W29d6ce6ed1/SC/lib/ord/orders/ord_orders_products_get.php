...
{
    global $id_order,$orderStatus, $sc_agent;
    $sql = 'SELECT od.*, p.id_category_default, t.rate as tax_rate, tl.name as tax_name
            FROM '._DB_PREFIX_.'order_detail od 
            LEFT JOIN '._DB_PREFIX_.'order_detail_tax odt ON (odt.id_order_detail=od.id_order_detail)
            LEFT JOIN '._DB_PREFIX_.'tax t ON (t.id_tax = odt.id_tax)
            LEFT JOIN '._DB_PREFIX_.'tax_lang tl ON (tl.id_tax = t.id_tax AND tl.id_lang = '.(int)$sc_agent->id_lang.')
            INNER JOIN '._DB_PREFIX_.'orders o ON (od.id_order=o.id_order)
            INNER JOIN '._DB_PREFIX_.'product_shop p ON (p.id_product=od.product_id AND p.id_shop=o.id_shop)
