...
foreach ($exportCustomerFilterFormObj->customerRules as $ruleKey => $ruleInfos) {
    $panelCustomerRules .= $exportCustomerFilterFormObj->displayFieldRule($ruleKey, $ruleInfos['text'], $ruleDescriptors, $ruleInfos['type'], $ruleInfos['choiceText1'], $ruleInfos['choiceText2'], $ruleInfos['editedList']);
}

$panelCustomerAddressRules = '';
foreach ($exportCustomerFilterFormObj->customerAddressRules as $ruleKey => $ruleInfos) {
    $panelCustomerAddressRules .= $exportCustomerFilterFormObj->displayFieldRule($ruleKey, $ruleInfos['text'], $ruleDescriptors, $ruleInfos['type'], $ruleInfos['choiceText1'], $ruleInfos['choiceText2'], $ruleInfos['editedList']);
}

$panelCustomerGroupRules = '';
foreach ($exportCustomerFilterFormObj->customerGroupRules as $ruleKey => $ruleInfos) {
    $panelCustomerGroupRules .= $exportCustomerFilterFormObj->displayFieldRule($ruleKey, $ruleInfos['text'], $ruleDescriptors, $ruleInfos['type'], $ruleInfos['choiceText1'], $ruleInfos['choiceText2'], $ruleInfos['editedList']);
}

$panelCustomerOrderRules = '';
foreach ($exportCustomerFilterFormObj->customerOrderRules as $ruleKey => $ruleInfos) {
    $panelCustomerOrderRules .= $exportCustomerFilterFormObj->displayFieldRule($ruleKey, $ruleInfos['text'], $ruleDescriptors, $ruleInfos['type'], $ruleInfos['choiceText1'], $ruleInfos['choiceText2'], $ruleInfos['editedList']);
}

$panelCustomerOtherListRules = '';
foreach ($exportCustomerFilterFormObj->customerOtherListRules as $ruleKey => $ruleInfos) {
    $panelCustomerOtherListRules .= $exportCustomerFilterFormObj->displayFieldRule($ruleKey, $ruleInfos['text'], $ruleDescriptors, $ruleInfos['type'], null, null, $exportCustomerFilterObj);
}

$panelNewsletterRules = '';
foreach ($exportCustomerFilterFormObj->customerNewsletterRules as $ruleKey => $ruleInfos) {
    $panelNewsletterRules .= $exportCustomerFilterFormObj->displayFieldRule($ruleKey, $ruleInfos['text'], $ruleDescriptors, $ruleInfos['type'], null, null, $ruleInfos['editedList']);
}
...
<head>
    ...
    <script type="text/javascript">
        ...
        <?php if(version_compare(_PS_VERSION_, '1.7.6.0', '>=')) { ?>

            $.widget('prestashop.scExportCustomerAutocomplete', $.ui.autocomplete, {
            ...
            $("#sc_product_search_query").scExportCustomerAutocomplete({
                source: function (query, response) {
                    $.get('<?php echo $link_search; ?>', {
                            q: query.term
                        }, null, 'json')
                        .then(function (resp) {
                            response(resp);
                ...
                select: function (event, ui) {
                    // keep new value in input
                    ui.item.value = ui.item.name;
                    $('#sc_product_search_query').val(ui.item.name);
                    $('#sc_product_search_id_to_add').val(ui.item.id);
                    $('#sc_product_search_name_to_add').val(ui.item.name);
                },
            });

        <?php } elseif(version_compare(_PS_VERSION_, '1.7.0.0', '>=')) { ?>
        ...
        <?php } else { ?>
            $("#sc_product_search_query")
                .autocomplete(
                    '<?php echo $link_search; ?>', {
                        minChars: 3,
                        max: 10,
                        width: 500,
                        selectFirst: true,
                        ...
                        extraParams: {
                            ajaxSearch: 1,
                            id_lang: <?php echo $id_lang; ?>
                        }
                    }
                )
                .result(function (event, data, formatted) {
                    $('#sc_product_search_query').val(data.pname);
                    $('#sc_product_search_id_to_add').val(data.id_product);
                    $('#sc_product_search_name_to_add').val(data.pname);
                });
        <?php } ?>
