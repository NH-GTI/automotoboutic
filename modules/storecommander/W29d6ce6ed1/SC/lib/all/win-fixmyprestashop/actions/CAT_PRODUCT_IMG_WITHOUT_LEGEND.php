...
if (!empty($post_action) && $post_action == 'do_check')
{
    ...
    $sql = "SELECT DISTINCT ishop.*, il.`id_lang`, il.`legend`, pl.`name`
            ...
            INNER JOIN `"._DB_PREFIX_."image_shop` ishop ON (i.`id_image`=ishop.`id_image` AND ishop.`id_product`!=0 AND ishop.`id_shop`=".SCI::getSelectedShop().")
            INNER JOIN `"._DB_PREFIX_."image_lang` il ON (ishop.`id_image`=il.`id_image`)
            INNER JOIN `"._DB_PREFIX_."product_lang` pl ON (ishop.`id_product`= pl.`id_product`)
            WHERE il.`legend` = '' OR il.`legend` IS NULL LIMIT 1500";
    ...
    if (!empty($res) && count($res) > 0)
    {
        ...
        <script type="text/javascript">
            ...
            gridProductImgWithoutLegend.setImagePath("lib/js/imgs/");
