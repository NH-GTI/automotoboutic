...
class Lm_SurmesureSurmesureModuleFrontController extends ModuleFrontController
{
    ...
    public function processStepOne()
    {
        ...
        if ($marque_selected) {
            ...
            if ($family_selected && $this->_checkIfFamilyMatchBrand($marque_selected, $family_selected)) {
                // if the selected modele do not match with the selected marque, we cancel the model
    ...
    public function processStepTwo()
    {
        ...
        foreach ($models as &$model) {
            ...
            $modelTitle = explode(' - ', $valueWithoutBrand[1]); // Titre du modèle
...
//echo '<pre>';                
				...
				//	var_dump($result['product']);
					...
					if ( in_array($this->context->cookie->custom_product_id_gam, $this->module->_id_gam_elite_carat)) {
						if ($result['id_conf'] == 515) {
							...
							$result['value'] = '2 avants + 1 arrière';
							$result['image'] = 'config05.png';
							$result['description'] = '<ul><li>2 tapis avant sur mesure</li><li>1 tapis arrière sur mesure</li></ul>';
							...
							$result['value'] = '2 avants + 1 arrière + 1 coffre';
							$result['image'] = 'config09.png';
							$result['description'] = '<ul><li>2 tapis avant sur mesure</li><li>1 tapis arrière sur mesure</li><li>1 tapis de coffre sur mesure</li></ul>';
						}
					}
    	...
    	if (
			...
			if (!empty($results)) {
				foreach($results as $result) {
					...
					var_dump($id_product);
					exit();
					if($id_product != null) $result['product'] = new Product($id_product);
					var_dump($result['product']);
					exit();
					if ( in_array($this->context->cookie->custom_product_id_gam, $this->module->_id_gam_elite_carat) ) {
						if ($result['id_conf'] == 515) {
    ...
    {
/*	var_dump($this->context->cookie->custom_product_marque_id);
	var_dump($this->context->cookie->custom_product_model_id);
	var_dump($this->context->cookie->custom_product_id_gam);
	var_dump($this->context->cookie->custom_product_id_conf);
	var_dump($this->context->cookie->custom_product_id);
	var_dump($this->context->cookie->custom_color);
	exit();*/
    	if (
            !empty($this->context->cookie->custom_product_marque_id)
            && !empty($this->context->cookie->custom_product_model_id)
	...
	public function getProductByConfCarroGamme($id_conf, $id_carro, $id_gam)
    {
        ...
        $ids_product = $this->getProductsByFeature($this->module->_id_carro, $id_carro);
        if (!empty($ids_product)) {
            // Filter products by configuration $id_conf
            $ids_product = $this->filterProductsByFeature($ids_product, $this->module->_id_conf, $id_conf);

            if (!empty($ids_product)) {
                // Filter products by gamme $id_gam
                $ids_product = $this->filterProductsByFeature($ids_product, $this->module->_id_gam, $id_gam);
//var_dump($ids_product);
    ...
    private function getProductsByFeature($feature_id, $feature_value)
    {
        return array_map(function ($row) {
            ...
            WHERE `id_feature` = '. (int)$feature_id .' 
                AND id_feature_value = '. (int)$feature_value
        ));
    }

    private function filterProductsByFeature($ids_product, $feature_id, $feature_value)
    {
//	var_dump('SELECT `id_product` FROM `'._DB_PREFIX_.'feature_product` WHERE `id_feature` = '. (int)$feature_id .' AND id_product IN ('. implode(',', $ids_product) .') AND id_feature_value = '. (int)$feature_value);
        return array_map(function ($row) {
            return (int)$row['id_product'];
        }, Db::getInstance()->executeS('
            SELECT `id_product` 
	...
	$sql = "SELECT id_mod FROM "._DB_PREFIX_."customproducts_fam_mod AS cfm
        ...
        "; // Récupère tout les modèles en fonction d'une famille précise
        $modeles = Db::getInstance()->executeS($sql);
