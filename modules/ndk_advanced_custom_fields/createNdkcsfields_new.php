...
//                                                file_put_contents(_PS_ROOT_DIR_.'/temp.log', 'field_reference:'.var_export($field_reference, true)."\n", FILE_APPEND);
    ...
    */
    ...
    if ($customizationPrice > 0 || sizeof($accessoryProdQuantity) > 0) {
        ...
        if (Configuration::get('NDK_ADD_PRODUCT_PRICE') == 1 && !$disabe_product_price) {
            //$myProductPrice = Product::getPriceStatic((int)Tools::getValue('id_product'), false,(int)Tools::getValue('ndkcf_id_combination'), 6, null, false, true, (int)Tools::getValue('qty'), false, (int)$context->customer->id, (int)$context->cart->id);
  ...
  function saveJsonDatas($json_datas)
  {
   ...
   function addTextFieldToProduct($id_product, $link_index, $id_lang, $text)
   {
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addTextFieldToProduct: Starting with id_product=$id_product, link_index=$link_index, id_lang=$id_lang\n", FILE_APPEND);
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addTextFieldToProduct: Text to add: " . $text . "\n", FILE_APPEND);

       $sql = 'INSERT INTO `'._DB_PREFIX_.'customization_field` (`id_product`, `type`, `required`, `is_module`, `is_deleted`)
           VALUES ('.(int)$id_product.', 1, 0, 1, 0)';
       Db::getInstance()->execute($sql);
       $id_customization_field = (int)Db::getInstance()->Insert_ID();
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addTextFieldToProduct: Created customization_field with ID: " . $id_customization_field . "\n", FILE_APPEND);

       $sql = 'INSERT INTO `'._DB_PREFIX_.'customization_field_lang` (`id_customization_field`, `id_lang`, `name`)
           VALUES ('.(int)$id_customization_field.', '.(int)$id_lang.', \''.pSQL($text).'\')';
       $result = Db::getInstance()->execute($sql);
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addTextFieldToProduct: Added customization_field_lang, result: " . ($result ? "success" : "failed") . "\n", FILE_APPEND);

       if ($link_index) {
           $sql = 'INSERT INTO `'._DB_PREFIX_.'customization_field_link` (`id_customization_field`, `link_index`)
               VALUES ('.(int)$id_customization_field.', '.(int)$link_index.')';
           $result = Db::getInstance()->execute($sql);
           file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addTextFieldToProduct: Added customization_field_link with link_index=$link_index, result: " . ($result ? "success" : "failed") . "\n", FILE_APPEND);
       }

       return $id_customization_field;
   }

   function createLabel($languages, $type, $id_product, $labels)
   {
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - createLabel: Starting for product ID: " . $id_product . ", type: " . $type . "\n", FILE_APPEND);
       
       $sql = 'INSERT INTO `'._DB_PREFIX_.'customization_field` (`id_product`, `type`, `required`, `is_module`, `is_deleted`)
           VALUES ('.(int)$id_product.', '.(int)$type.', 0, 1, 0)';
       Db::getInstance()->execute($sql);
       $id_customization_field = (int)Db::getInstance()->Insert_ID();
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - createLabel: Created customization_field with ID: " . $id_customization_field . "\n", FILE_APPEND);

       foreach ($languages as $language) {
           if (isset($labels[$language['id_lang']][0]['name'])) {
               $sql = 'INSERT INTO `'._DB_PREFIX_.'customization_field_lang` (`id_customization_field`, `id_lang`, `name`)
                   VALUES ('.(int)$id_customization_field.', '.(int)$language['id_lang'].', \''.pSQL($labels[$language['id_lang']][0]['name']).'\')';
               $result = Db::getInstance()->execute($sql);
               file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - createLabel: Added label for language " . $language['id_lang'] . ": " . $labels[$language['id_lang']][0]['name'] . ", result: " . ($result ? "success" : "failed") . "\n", FILE_APPEND);
           }
       }
   ...
   function addCustomization($id_product, $id_customization_field, $id_lang, $value)
   {
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addCustomization: Starting for product=$id_product, field=$id_customization_field, lang=$id_lang\n", FILE_APPEND);
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addCustomization: Value to add: " . $value . "\n", FILE_APPEND);

       $myIdCustomization = (int)Context::getContext()->cart->_addCustomization((int)$id_product, 0, $id_customization_field, $value);
       file_put_contents(_PS_ROOT_DIR_ . '/debug_ndk.log', date('Y-m-d H:i:s') . " - addCustomization: Created customization with ID: " . $myIdCustomization . "\n", FILE_APPEND);
       
       return $myIdCustomization;
   }
