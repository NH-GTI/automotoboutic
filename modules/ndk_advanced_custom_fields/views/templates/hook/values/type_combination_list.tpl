				...
				{capture name='itemPrice'}
					{if $value.price > 0}
						...
						{elseif $field.price > 0}
							{$field.price}
						{else}
							{if !$priceDisplay}{$product.price}{else}{$product.price}{/if}
						{/if}
				{/capture}
				{if $value.tags != ''}
				 ...
				 <div style="display:none">
					 <div id="accessory-popup-{$value.id|escape:'intval'}" class="accessory-popup-ndk">
						 	<div class="col-md-6 ndk-img-block">
						 		<img class="img-responsive" style="display:none" src="{$link->getImageLink($product.link_rewrite, $product.id_image, Configuration::get('NDK_LARGE_IMAGE_SIZE'))|escape:'html':'UTF-8'}"/>
						 	</div>
				 			...
				 			<div class="combColumn_list">
				 				...
				 				<span class="comblist_name" title="{$col.attribute_name}">
				 							{if $col.is_color_group == 1}
				 								...
				 								style="background: {if $img_color_exists}url('{$theme_col_dir}{$col.id_attribute|intval}.jpg'){else}{$col.attribute_color|escape:'htmlall':'UTF-8'}{/if};">{$col.attribute_name}</span>
