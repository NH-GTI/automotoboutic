					 ...
					 <div id="accessory-popup-{$value.id|escape:'intval'}" class="accessory-popup-ndk">
					 	{if $value.is_image}
						 	<div class="col-md-6 ndk-img-block">
						 		<img class="img-responsive" src="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/thumbs/{$value.id|escape:'intval'}-{Configuration::get('NDK_IMAGE_LARGE_SIZE')}.jpg"/>
						 	</div>
					 	{/if}
