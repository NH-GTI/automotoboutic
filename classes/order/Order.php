...
class OrderCore extends ObjectModel
{
    ...
    public function addOrderPayment(
        ...
        $order_payment->date_add = ($date ? $date : null);
            ...
            PrestaShopLogger::addLog('PaymentModule::validateOrder - Start validate Order', 1, null, 'Cart', (int) $order_payment->transaction_id, true);
		...
		$this->total_paid_real += $order_payment->amount;
            ...
            PrestaShopLogger::addLog('PaymentModule::validateOrder - Currency', 1, null, 'Cart', (int) $order_payment->transaction_id, true);
        } else {
		$default_currency = Currency::getDefaultCurrencyId();

            PrestaShopLogger::addLog('PaymentModule::validateOrder - Default currency', 1, null, 'Cart', (int) $order_payment->transaction_id, true);
            ...
            PrestaShopLogger::addLog('PaymentModule::validateOrder - Order payment currency = default currency', 1, null, 'Cart', (int) $order_payment->transaction_id, true);
            } else {
            ...
            PrestaShopLogger::addLog('PaymentModule::validateOrder - Round total paid real', 1, null, 'Cart', (int) $order_payment->transaction_id, true);
            }
        }
        ...
        $res = $order_payment->add(null === $order_payment->date_add) && $this->update();
            ...
            PrestaShopLogger::addLog('PaymentModule::validateOrder - Order payment add done', 1, null, 'Cart', (int) $order_payment->transaction_id, true);
        if (!$res) {
            return false;
        }

            PrestaShopLogger::addLog('PaymentModule::validateOrder - Before request', 1, null, 'Cart', $res, true);
        if (null !== $order_invoice) {
            $res = Db::getInstance()->execute('
            INSERT INTO `' . _DB_PREFIX_ . 'order_invoice_payment` (`id_order_invoice`, `id_order_payment`, `id_order`)
            VALUES(' . (int) $order_invoice->id . ', ' . (int) $order_payment->id . ', ' . (int) $this->id . ')');
		var_dump('INSERT INTO `' . _DB_PREFIX_ . 'order_invoice_payment` (`id_order_invoice`, `id_order_payment`, `id_order`)
			1990             VALUES(' . (int) $order_invoice->id . ', ' . (int) $order_payment->id . ', ' . (int) $this->id . ')');
            // Clear cache
            Cache::clean('order_invoice_paid_*');
        }

            PrestaShopLogger::addLog('PaymentModule::validateOrder - After request', 1, null, 'Cart', 1, true);
        return $res;
    }
